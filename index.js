import {
	...
	resolveTimePromptKey,
	resolveTimePromptDisplayTag,
	buildCyoaPrompt,
	DEFAULT_CYOA_SLOTS,
	refreshCyoaConfigToShipped,
	formatTimeOfDay,
} from "./constants.js";
import {
	MODULE_NAME,
	...
	saveChatState,
	getActiveChatId,
	shouldPreserveLiveChatStateOnBoot,
	writeModuleSchemaBackup,
	getPendingModuleSchemaBackup,
	applyModuleSchemaBackup,
	applyDeletedCustomTagTombstones,
	recordDeletedCustomTags,
	clearDeletedCustomTagTombstones,
	saveProfile,
	deleteProfile,
	getEffectiveRouterCampaignPrefix,
	sanitizeCampaignPrefixString,
	buildNpcInstruction,
	LOREBOOK_FULL_AUDIT_INSTRUCTION,
...
import {
	...
	getCurrentCompletionPreset,
	setCompletionPreset,
	syncCombatProfile,
	resetCombatProfileOverride,
	isCombatActive,
} from "./llm-client.js";
import {
	getDiceToolName,
	getDiceCommandName,
	getDiceCommandAliases,
	doDiceRoll,
	...
	onGenerationStarted,
	onGenerationEnded,
	onMapArchitectAssistantMessage,
	handleRelationshipSwipeChange,
	applyStateTrackerRelationshipCommands,
	resetRouterTick,
	getRouterTick,
	getMapUpdaterTick,
	resetRouterAutoTick,
...
import {
	...
	escapeRegex,
	highlightParens,
	cleanToolCallMessage,
	cleanMessageContent,
	getLastUserAction,
	buildLorebookContext,
	buildModulesInstructionText,
	buildModuleFormatInstruction,
	parseQuestsFromMemo,
	syncQuestsFromMemo,
	syncQuestsToMemo,
	writeQuestsToMemo,
	...
	stripArchivedQuestsFromMemo,
	stripCompletedQuestsFromMemo,
	applyQuestSyncAndStripMemo,
	isArchivedQuestStatus,
	removeArchivedQuest,
	parseInWorldTime,
	formatInWorldTime,
...
import {
	renderSubFieldByRule,
	tryRenderMarker,
	renderCustomBlockLine,
	stripMemoHtml,
	escapeHtmlWithColor,
	parseMemoBlocks,
	getPageSize,
...
import {
	...
	captureActiveDungeonMapHistory,
...
import {
	migrateAllEmbeddedPortraits,
	countEmbeddedPortraitDataUrls,
	purgeAllPortraitData,
	resolvePortraitDisplaySrc,
	lookupCustomPortraitSrc,
	collectAllPortraitRefs,
	isManagedPortraitPath,
	isPortraitMigrationLocked,
	setPortraitMigrationLocked,
	PORTRAIT_STORAGE_FOLDER,
...
} from "./src/ui/game-master-card.js";
import {
...
import {
	openGameSystemWizard,
	openManageGameSystems,
	openSystemPromptControlRoom,
	syncAllNarratorTogglesForUnlockState,
	extractTopLevelSections,
	normalizeSectionOrder,
	getSectionRowDescriptor,
	transformBaseSectionContent,
	isBlankSectionContent,
	isSectionUnlocked,
	isEffectiveSectionEnabled,
} from "./game-systems.js";
import {
	isCyoaEnabled,
	isLorebookAgentRuntimeActive,
	setLocationMappingEnabled,
...
import {
	applyMapArchitectOpenerToUi,
	normalizeMapArchitectOpener,
	syncMapArchitectOpenerNestedVisibility,
} from "./map-architect-opener.js";
import {
	openManageGameCartridges,
	promptAndSaveCurrentAsCartridge,
} from "./game-cartridges.js";
import {
...
import {
	branchCampaignChat,
	isBranchSeedInProgress,
} from "./src/features/chat/branch-campaign.js";
...
import {
	captureXpGainAnimationState,
	playXpGainAnimation,
} from "./src/ui/panel/xp-gain-animation.js";
import {
	captureBarChangeAnimationState,
	playBarChangeAnimations,
...
RENDERING_TAGS_LIBRARY.push(
	...getMarkerLibraryKeys().map(
		(key) => `((${key})) ${MARKER_TYPE_MAP[key].example}`,
	),
);
...
const memoRecovery = LEGACY_LOCAL_RECOVERY_ENABLED
	? createMemoRecoveryManager({
			getSettings,
	...
	: null;
const snapshotMemoToLocalStorage = (...args) =>
	memoRecovery?.snapshotMemoToLocalStorage(...args);
const ensureLocalMemoRecovery = (...args) =>
	memoRecovery?.ensureLocalMemoRecovery(...args);
const confirmLocalSettingsRecovery = (...args) =>
	memoRecovery?.confirmLocalSettingsRecovery(...args);
const markMemoPersistedByCurrentBrowser = (...args) =>
	memoRecovery?.markMemoPersistedByCurrentBrowser(...args);
...
globalThis._rpgRefreshImmersionView = () => {
	void runtimeState.refreshImmersionView();
};
globalThis._rpgCheckRealtimeSceneArt = () => {
	void runRealtimeSceneArtCheck().catch(() => {});
};
...
export function substituteDisplayMacros(text) {
	...
	try {
		...
		return typeof substituteParams === "function"
...
function renderRelTierBadge(type, value, max) {
	const tier =
		type === "friendship"
			? getFriendshipTier(value, max)
			: getAffectionTier(value, max);
...
function notifyMapEvolutionPassResult(result) {
	...
	if (skipped === "location_mapping_off" || skipped === "dungeon_reality_off")
		...
		skipped === "no_maps" ||
		skipped === "no_active_map" ||
		skipped === "no_matching_sites" ||
		skipped === "no_selection"
	)
		...
		toastr["info"](
			"Interval baseline stamped. Evolution will fire after the interval elapses.",
			"Map Evolution",
		);
...
function persistMapEvolutionSelectedRootsFromUi() {
	const roots = [];
	$('#rpg_map_evolution_selected_list input[type="checkbox"]:checked').each(
		function () {
...
function persistMapEvolutionIntervalOverrideFromUi(siteRoot, rawValue) {
	...
	void saveSettings(true);
	if (settings.chatLinkEnabled && runtimeState.currentChatId)
...
function formatMapEvolutionCadence(hours) {
	...
	return [wholeHours ? `${wholeHours}h` : "", minutes ? `${minutes}m` : ""]
		.filter(Boolean)
		.join(" ");
}
...
async function refreshMapEvolutionSelectedList() {
	...
	try {
		...
		console.warn(
			"[RPG Tracker] Failed to list mapped sites for Map Evolution:",
			error,
		);
	}
	if (!sites.length) {
		$list.append(
			$("<i>")
				.css({ opacity: "0.6", fontSize: "0.85em" })
				.text("No mapped sites in this chat."),
		);
	...
	const selected = new Set(
		(getSettings().mapEvolutionSelectedRoots || []).map((root) =>
			String(root || "")
				.trim()
				.toLowerCase(),
		),
	...
	for (const site of sites) {
		...
		const overrideHours = getSiteEvolutionIntervalOverride(
			overrides,
			site.siteRoot,
		);
		...
		const inheritText =
			inherited === 0
				? site.current
					? "Automatic: skip (current map)"
					: "Automatic: skip (other maps)"
				: site.current
					? `Automatic: ${formatMapEvolutionCadence(inherited)} (current map)`
					: `Automatic: ${formatMapEvolutionCadence(inherited)} (other maps)`;
		const $inherit = $("<div>")
			.css({
				fontSize: "0.75em",
			...
			.text(
				hasOverride
					? `Override · was ${formatMapEvolutionCadence(inherited)}`
					: inheritText,
			);
		const $hours = $(
			'<input type="text" inputmode="numeric" pattern="[0-9]*" class="text_pole">',
		)
			.attr({
		...
		$hours.on("change", function () {
			persistMapEvolutionIntervalOverrideFromUi(
				site.siteRoot,
				String($(this).val() || ""),
			);
		...
		const $name = $("<span>")
			...
			.append($("<div>").text(label), $inherit);
		const $item = $('<label class="checkbox_label">')
			.css({
				fontSize: "0.9em",
				display: "flex",
				alignItems: "flex-start",
		...
		const $interval = $("<div>")
			...
			.append(
				$hours,
				$("<span>").css({ fontSize: "0.75em", opacity: "0.55" }).text("h"),
			);
...
function currentMemoMinutes() {
	const s = getSettings();
	const timeMatch = (s.currentMemo || "").match(
		/\[TIME\]([\s\S]*?)\[\/TIME\]/i,
	);
...
function updateMapEvolutionScheduleDisplay(currentRoot = "") {
	...
	const schedule = summarizeMapEvolutionSchedule(
	...
	const lastText =
		schedule.lastMins >= 0 ? formatInWorldTime(schedule.lastMins) : "Never";
	$("#rpg_map_evolution_last_fired").text(lastText);
	$("#rpg_map_evolution_last_report_val").text(lastText);
	$("#rpg_map_evolution_next_report_val").text(
		schedule.nextMins >= 0 ? formatInWorldTime(schedule.nextMins) : "—",
	);
...
function applyMapEvolutionTickSettingsToUi(settings) {
	...
	$("#rpg_map_evolution_tick_randomize").prop(
		"checked",
...
runtimeState.applyMapEvolutionTickSettingsToUiRef =
	applyMapEvolutionTickSettingsToUi;
...
runtimeState.isLoreOrMapAgentBusyRef = () =>
	isRouterRunning() || isMapUpdaterRunning() || isMapEvolutionRunning();

function applyMapRuntimeConnectionSettingsToUi(settings) {
	const s = settings || getSettings();
	$("#rpg_map_runtime_connection_source").val(
		s.mapRuntimeConnectionSource || "default",
	);
	$("#rpg_map_runtime_connection_profile").val(
		s.mapRuntimeConnectionProfileId || "",
	);
	$("#rpg_map_runtime_completion_preset").val(
		s.mapRuntimeCompletionPresetId || "",
	);
	$("#rpg_map_runtime_ollama_url").val(
		s.mapRuntimeOllamaUrl || "http://localhost:11434",
	);
	...
	$("#rpg_map_runtime_profile_group").toggle(
		s.mapRuntimeConnectionSource === "profile",
	);
	$("#rpg_map_runtime_ollama_group").toggle(
		s.mapRuntimeConnectionSource === "ollama",
	);
	$("#rpg_map_runtime_openai_group").toggle(
		s.mapRuntimeConnectionSource === "openai",
	);
}

function applyMapEvolutionConnectionSettingsToUi(settings) {
	const s = settings || getSettings();
	$("#rpg_map_evolution_connection_source").val(
		s.mapEvolutionConnectionSource || "default",
	);
	$("#rpg_map_evolution_connection_profile").val(
		s.mapEvolutionConnectionProfileId || "",
	);
	$("#rpg_map_evolution_completion_preset").val(
		s.mapEvolutionCompletionPresetId || "",
	);
	$("#rpg_map_evolution_ollama_url").val(
		s.mapEvolutionOllamaUrl || "http://localhost:11434",
	);
	...
	$("#rpg_map_evolution_openai_model_manual").val(
		s.mapEvolutionOpenaiModel || "",
	);
	$("#rpg_map_evolution_profile_group").toggle(
		s.mapEvolutionConnectionSource === "profile",
	);
	$("#rpg_map_evolution_ollama_group").toggle(
		s.mapEvolutionConnectionSource === "ollama",
	);
	$("#rpg_map_evolution_openai_group").toggle(
		s.mapEvolutionConnectionSource === "openai",
	);
}
...
function applyAgentConnectionSetupToUi(def, settings) {
	...
	const sourceVal = String(
		settings[def.settingsKeys.connectionSource] || "default",
	);
	const profileVal = String(
		settings[def.settingsKeys.connectionProfileId] || "",
	);
	const presetVal = String(settings[def.settingsKeys.completionPresetId] || "");
	const ollamaUrl = String(
		settings[def.settingsKeys.ollamaUrl] || "http://localhost:11434",
	);
...
function populateConnectionApplyAllSourceSelect() {
	const select = /** @type {HTMLSelectElement|null} */ (
		document.getElementById("rpg_connection_apply_all_source")
	);
	if (!select) return;
	...
	select.value = findAgentConnectionSetup(previous)
		? previous
		: "state_tracker";
}
...
function bindConnectionApplyAllControls() {
	...
	btn.addEventListener("click", () => {
		...
		if (!result) {
			toastr["error"](
				"Could not apply that connection setup.",
				"Connections & Models",
			);
			return;
		}
...
async function confirmAndPurgeWorldHistory() {
	...
	const choice = await Popup.show.confirm(
		"Purge World History for this Chat?",
		body,
		{ okButton: "Purge", cancelButton: "Cancel" },
	);
	if (choice !== 1) return;
	try {
		const result = await purgeWorldHistoryForChat({ includeSkeleton: true });
		if (
			typeof runtimeState.updateWorldProgressionLastFiredDisplayRef ===
			"function"
		) {
		...
		toastr["success"](
			`Purged ${result.worldCleared} report(s) and ${result.skeletonCleared} skeleton entries.`,
			"World Progression",
		);
	} catch (e) {
...
function renderLoreActivationDebugPanel() {
	...
	if (!_loreActivationDebugLast) {
		pre.textContent =
			"(no data yet — use Capture now in Extension Settings > Lorebook Agent, or switch chats / Activate Books.)";
		return;
	}
	try {
...
async function refreshWorldInfoRegistry(opts = {}) {
	...
	const usedApiNameFallback =
		clientCount === 0 &&
		apiProbe.ok &&
		apiProbe.count > 0 &&
		Array.isArray(apiProbe.names) &&
		apiProbe.names.length > 0;
	return {
		clientWorldNameCount: clientCount,
		attempts,
		apiProbe,
...
function resolveAllWorldNames(ctx, reg) {
	if (
		reg.clientWorldNameCount > 0 &&
		typeof ctx.getWorldInfoNames === "function"
	) {
		const n = ctx.getWorldInfoNames();
		return Array.isArray(n) ? [...n] : [];
	}
	if (reg.usedApiNameFallback && Array.isArray(reg.apiProbe.names))
		return [...reg.apiProbe.names];
	if (typeof ctx.getWorldInfoNames === "function") {
		const n = ctx.getWorldInfoNames();
		return Array.isArray(n) ? [...n] : [];
	}
...
function computeWorldsToDeactivate(allNames, currentPrefix, bookNames, s) {
	...
	const managedOff = [...allKnownManagedBooks].filter(
		(n) => !currentSet.has(n),
	);
	...
	const combined = [...managedOff, ...crossChatOff].filter(
		(n) => !currentSet.has(n),
	);
	const toDeactivate = [...new Set(combined)];
	const managedSet = new Set(managedOff);
	const crossChatOnlyCount = crossChatOff.filter(
		(n) => !managedSet.has(n),
	).length;
	return {
		toDeactivate,
...
async function readLoreActivationDebugSnapshot(source) {
	...
	const matchingEffective = effectivePrefix
		? allNames.filter((n) => bookBelongsToPrefix(n, effectivePrefix))
		: [];
	const matchingForStored = storedPrefix
		? allNames.filter((n) => bookBelongsToPrefix(n, storedPrefix))
		: [];
	const matchingDerivedOnly = derivedFromChatOnly
		? allNames.filter((n) => bookBelongsToPrefix(n, derivedFromChatOnly))
		: [];
	const allKnownManagedBooks = new Set(
		Object.values(s.chatStates || {}).flatMap((cs) => cs.campaignBooks || []),
	);
	...
	return {
		...
		bookMatchRule:
			'Book matches if name === prefix OR name === prefix + "_" + single segment (no extra underscores in suffix).',
		apis: {
			executeSlashCommandsWithOptions:
				typeof ctx.executeSlashCommandsWithOptions,
...
async function syncCampaignPrefixAndWorldsForChat(newChatId, source) {
	...
	if (s2.worldProgressionEnabled) {
		if (
			allNames.includes(worldBookName) &&
			!matchingBooks.includes(worldBookName)
		) {
		...
		try {
			...
			if (worldBook?.entries) {
				const sorted = Object.entries(worldBook.entries).sort(
	...
	if (
		reg.usedApiNameFallback &&
		reg.clientWorldNameCount === 0 &&
		matchingBooks.length > 0 &&
		!String(source).includes("registry-followup")
	) {
		setTimeout(() => {
			if (newChatId !== runtimeState.currentChatId) return;
			void syncCampaignPrefixAndWorldsForChat(
				newChatId,
				`${source}(registry-followup)`,
			).catch(() => {});
		}, 450);
	}
	void scheduleAgentManifestRefresh();
...
async function forceDiskCheckpoint() {
	...
	const chatId =
		runtimeState.currentChatId || SillyTavern.getContext()?.chatId || null;
...
function flushPendingMapThemeColors(immediateSave = false) {
	...
	for (const key of Object.keys(edits)) {
		document
			.querySelectorAll(`#rpg_map_theme_colors [data-map-theme-key="${key}"]`)
			.forEach((input) => {
...
function syncMapThemeImageUi(settings) {
	...
	const urlInput = /** @type {HTMLInputElement|null} */ (
		document.getElementById("rpg_map_theme_bg_url")
	);
	const overlayInput = /** @type {HTMLInputElement|null} */ (
		document.getElementById("rpg_map_theme_bg_overlay")
	);
	const overlayValue = document.getElementById("rpg_map_theme_bg_overlay_val");
	const strength = theme.backgroundImageStrength;
	if (preview) {
		const alpha = Math.round(strength * 2.55)
			.toString(16)
			.padStart(2, "0");
	...
	if (urlInput)
		urlInput.value = /^https?:\/\//i.test(theme.backgroundImage)
...
function syncMapThemeUi(settings = getSettings()) {
	...
	settings.savedMapThemePresets = normalizeSavedMapThemePresets(
		settings.savedMapThemePresets,
	);
...
function bindMapThemeControls() {
	...
	$colors.on(
		"input.rpgMapTheme change.rpgMapTheme",
		"input[data-map-theme-key]",
		function (event) {
	...
	$("#rpg_map_theme_preset")
		.off(".rpgMapTheme")
		.on("change.rpgMapTheme", function () {
			$("#rpg_map_theme_delete").prop(
				"disabled",
				!String(this.value || "").startsWith("user:"),
			);
		});
	$("#rpg_map_theme_load")
		.off(".rpgMapTheme")
		.on("click.rpgMapTheme", function () {
			const settings = getSettings();
			const id = String($("#rpg_map_theme_preset").val() || "");
			const theme = resolveMapThemePreset(id, settings.savedMapThemePresets);
			if (!theme) {
				toastr["warning"](
					"Choose a saved or factory theme first.",
					"Map Themes",
				);
				return;
			}
			...
			const name = id.startsWith("user:")
				? id.slice(5)
				: FACTORY_MAP_THEME_PRESETS.find((preset) => preset.id === id)?.name;
			toastr["success"](`Loaded ${name || "map theme"}.`, "Map Themes");
		});
	$("#rpg_map_theme_save")
		.off(".rpgMapTheme")
		.on("click.rpgMapTheme", async function () {
			const settings = getSettings();
			const { Popup, POPUP_RESULT } = SillyTavern.getContext();
			const requested = Popup?.show?.input
				? await Popup.show.input(
						"Save Map Theme",
						"Name this map theme preset:",
						"My Map Theme",
					)
			...
			const existingName = Object.keys(
				settings.savedMapThemePresets || {},
			...
			if (existingName) {
				const replace = Popup?.show?.confirm
					? await Popup.show.confirm(
							"Replace Map Theme?",
							`Replace the saved map theme "${escapeHtml(existingName)}"?`,
						)
					: confirm(`Replace the saved map theme "${existingName}"?`);
				if (
					Popup?.show?.confirm
						? replace !== (POPUP_RESULT?.AFFIRMATIVE ?? 1)
						: !replace
				)
					return;
			}
			if (existingName && existingName !== name)
				delete settings.savedMapThemePresets[existingName];
			settings.savedMapThemePresets[name] = normalizeMapTheme(
				settings.mapTheme,
			);
	...
	$("#rpg_map_theme_delete")
		...
		.on("click.rpgMapTheme", async function () {
			...
			const approved = Popup?.show?.confirm
				? await Popup.show.confirm(
						"Delete Map Theme?",
						`Delete the saved map theme "${escapeHtml(name)}"?`,
					)
				: confirm(`Delete the saved map theme "${name}"?`);
			if (
				Popup?.show?.confirm
					? approved !== (POPUP_RESULT?.AFFIRMATIVE ?? 1)
					: !approved
			)
				return;
			delete settings.savedMapThemePresets[name];
			if (settings.activeMapThemePresetId === id)
				settings.activeMapThemePresetId = "";
			syncMapThemeUi(settings);
			scheduleMapThemeSave(true);
	...
	const persistBackgroundImage = async (source) => {
		...
		if (stored.startsWith("data:image/")) {
			...
			toastr["warning"](
				"Use an http(s) image URL or upload a file.",
				"Map Themes",
			);
			return false;
		}
		settings.mapTheme = normalizeMapTheme({
			...settings.mapTheme,
			backgroundImage: stored,
		});
	...
	$("#rpg_map_theme_bg_upload")
		.off(".rpgMapTheme")
		.on("click.rpgMapTheme", function () {
			document.getElementById("rpg_map_theme_bg_file")?.click();
		});
	$("#rpg_map_theme_bg_file")
		.off(".rpgMapTheme")
		.on("change.rpgMapTheme", async function () {
	...
	$("#rpg_map_theme_bg_url")
		.off(".rpgMapTheme")
		.on("change.rpgMapTheme", async function () {
			await persistBackgroundImage(this.value);
		});
	$("#rpg_map_theme_bg_clear")
		.off(".rpgMapTheme")
		.on("click.rpgMapTheme", async function () {
			await persistBackgroundImage("");
		});
	$("#rpg_map_theme_bg_overlay")
		.off(".rpgMapTheme")
		.on("input.rpgMapTheme change.rpgMapTheme", function (event) {
			const settings = getSettings();
			const strength = Math.max(
				0,
				Math.min(100, parseInt(this.value, 10) || 0),
			);
			settings.mapTheme = normalizeMapTheme({
				...settings.mapTheme,
				backgroundImageStrength: strength,
			});
			markMapThemeCustom(settings);
			applyMapThemeToRoot(settings.mapTheme);
			syncMapThemeImageUi(settings);
 ...
 */
function applyLocationImageAutoMode(
	settings,
	{ realTimeMode, lorebookLocations } = {},
) {
...
export function syncNpcPortraitDependentUi(settings) {
	...
	if (!enabled) {
		...
		$("#rpg_tracker_portrait_auto_npcs").prop(
			"checked",
			!!settings.portraitAutoGenerateNpcs,
		);
	}
}
...
export function syncLocationImageDependentUi(settings) {
	...
	if (
		settings.portraitAutoGenerateSceneView &&
		settings.portraitAutoGenerateLocations
	) {
		settings.portraitAutoGenerateLocations = false;
	}
	...
	syncCheckbox(
		"rpg_tracker_portrait_auto_locations",
		lorebookAutoOn,
		!imagesEnabled || realTimeOn,
	);
	...
	syncCheckbox(
		"rpg_portrait_location_include_present_npcs",
		realTimeOn || !!settings.portraitLocationIncludePresentNpcs,
		!imagesEnabled || realTimeOn,
	);
	...
	if (everyNWrap)
		everyNWrap.style.display =
			realTimeOn && triggerMode === "every_n_outputs" ? "flex" : "none";

	const standardCore = document.getElementById("rpg_loc_images_standard_core");
	const realtimeGroup = document.getElementById(
		"rpg_loc_images_realtime_group",
	);
	...
	if (realtimeBadge)
		realtimeBadge.style.display = realTimeOn ? "inline-flex" : "none";
	if (realtimeNote) {
		realtimeNote.style.display = realTimeOn ? "block" : "none";
		if (realTimeOn) {
			const notes = {
				location_enter:
					"Generate once when entering a place that has no scene image yet. Open Visuals/Map in the Lorebook Agent to view scene art.",
				location_change:
					"Regenerate whenever the location changes (including revisits). Open Visuals/Map in the Lorebook Agent to view scene art.",
	...
	if (realtimeGroup)
		realtimeGroup.classList.toggle("rt-loc-realtime-active", realTimeOn);
 ...
 */
function syncPortraitLocationPromptForNpcToggle(
	settings,
	includePresentNpcs,
	opts = {},
) {
	const fromSettings = settings.portraitLocationSystemPrompt || "";
	const fromTextarea = String(
		$("#rpg_portrait_location_system_prompt").val() || "",
	);
	const styleId =
		findShippedPortraitLocationPresetId(fromSettings) ||
	...
	settings.portraitLocationSystemPrompt =
		getDefaultPortraitLocationSystemPrompt(includePresentNpcs, styleId);
...
export function syncTimeFormatSettingsUi(s) {
	const timeDdMmyyCb = /** @type {HTMLInputElement|null} */ (
		document.getElementById("rpg_time_ddmmyy_toggle")
	);
	if (timeDdMmyyCb) timeDdMmyyCb.checked = !!s.useDdMmYyFormat;
	const time24hCb = /** @type {HTMLInputElement|null} */ (
		document.getElementById("rpg_time_24h_toggle")
	);
	if (time24hCb) time24hCb.checked = !!s.use24hTime;
	syncOnboardingUI();
	if (
		typeof runtimeState.updateWorldProgressionLastFiredDisplayRef === "function"
	) {
		runtimeState.updateWorldProgressionLastFiredDisplayRef();
	}
}
...
function applyChatNpcRelMaxSettings(saved) {
	const s = getSettings();
	s.npcRelationshipMax =
		saved?.npcRelationshipMax ?? getNpcRelationshipMaxDefault(s);
...
function setNpcRelationshipMaxDefault(val) {
	const s = getSettings();
	s.npcRelationshipMaxDefault = getNpcRelationshipMax({
		npcRelationshipMax: val,
	});
	saveSettings();
...
export function setUseDdMmYyFormat(isDate) {
	...
	$("#rpg_tracker_router_prompt").val(
		s.routerBasicMode
			? s.routerBasicSystemPromptTemplate
			: s.routerSystemPromptTemplate,
	);
	$("#rpg_tracker_router_modular_prompt").val(s.routerModularPromptTemplate);
	$("#rpg_tracker_router_agent_context").val(
		s.routerAgentSharedContextTemplate,
	);
	refreshOrderList();
...
export function setUse24hTime(is24h) {
	...
	if (parsedMins != null)
		s.initialTime = formatTimeOfDay(parsedMins, s.use24hTime);
	rebuildAllModuleInstructions(s);
	adjustAllStoredTemplatesForTimeFormat(s);
	$("#rpg_tracker_router_prompt").val(
		s.routerBasicMode
...
function setInitialDateValue(val, sourceInput = null) {
	...
	persistChatTimeFormatIfLinked();
	document
		.querySelectorAll("#rt-cr-start-date, #rt-onboarding-start-date")
		.forEach((input) => {
			if (input !== sourceInput)
				/** @type {HTMLInputElement} */ (input).value = val;
		});
}
...
function setInitialTimeValue(val, sourceInput = null) {
	...
	persistChatTimeFormatIfLinked();
	document
		.querySelectorAll("#rt-cr-start-time, #rt-onboarding-start-time")
		.forEach((input) => {
			if (input !== sourceInput)
				/** @type {HTMLInputElement} */ (input).value = val;
		});
...
function syncOnboardingUI() {
	...
	// RNG Mode Sync
	const rngHybrid = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt_onboarding_rng_hybrid")
	);
	const rngLegacy = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt_onboarding_rng_legacy")
	);
	const rngNone = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt_onboarding_rng_none")
	);
	...
	const narrativePacing = [
		"normal",
		"shorter_outputs",
		"high_agency",
		"downtime",
	].includes(s.narrativePacing)
		? s.narrativePacing
		: "normal";
	onboarding
		.querySelectorAll('input[name="rt_onboarding_narrative_pacing"]')
		.forEach((input) => {
			input.checked = input.value === narrativePacing;
		});
	document
		.querySelectorAll('input[name="rpg_narrative_pacing"]')
		.forEach((input) => {
			input.checked = input.value === narrativePacing;
		});

	// Quests Enabled Sync
	const questsEnabled = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt_onboarding_quests_enabled")
	);
	if (questsEnabled) {
		const isEnabled = s.syspromptModules?.quests !== false;
		questsEnabled.checked = isEnabled;
		const optionsDiv = /** @type {HTMLElement|null} */ (
			onboarding.querySelector("#rt_onboarding_quest_options")
		);
		if (optionsDiv) optionsDiv.style.display = isEnabled ? "flex" : "none";
	}

	// Deadlines Sync
	const deadlines = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt_onboarding_quests_deadlines")
	);
	if (deadlines) deadlines.checked = !!s.syspromptModules?.questsDeadlines;
	const frustrationWrapOnb = /** @type {HTMLElement|null} */ (
		onboarding.querySelector("#rt_onboarding_quests_frustration_wrap")
	);
	if (frustrationWrapOnb)
		frustrationWrapOnb.style.display = deadlines?.checked ? "" : "none";

	// Frustration levels Sync
	const frustration = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt_onboarding_quests_frustration")
	);
	if (frustration)
		frustration.checked = !!s.syspromptModules?.questsFrustration;

	const showArchiveOnb = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt_onboarding_quests_show_archive")
	);
	if (showArchiveOnb)
		showArchiveOnb.checked = s.syspromptModules?.questsShowArchive !== false;
	...
	syncMapArchitectOpenerNestedVisibility(
		s.syspromptModules?.[LOCATION_MAPPING_SECTION_TAG] ?? true,
	);

	// Time & Date sync — Character Creator + "Other ways to begin" drawer
	syncSegToggle(
		onboarding.querySelector("#rt-cr-date-seg"),
		s.useDdMmYyFormat ? "date" : "day",
	);
	syncSegToggle(
		onboarding.querySelector("#rt-cr-clock-seg"),
		s.use24hTime ? "24" : "12",
	);
	syncSegToggle(
		onboarding.querySelector("#rt-onboarding-date-seg"),
		s.useDdMmYyFormat ? "date" : "day",
	);
	syncSegToggle(
		onboarding.querySelector("#rt-onboarding-clock-seg"),
		s.use24hTime ? "24" : "12",
	);
	const creatorStartDate = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt-cr-start-date")
	);
	const drawerStartDate = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt-onboarding-start-date")
	);
	const startDateVal =
		s.initialDate && s.initialDate !== "Day 1" ? s.initialDate : "01/01/2026";
	if (creatorStartDate) {
		creatorStartDate.value = startDateVal;
		creatorStartDate.style.display = s.useDdMmYyFormat
			? "inline-block"
			: "none";
	}
	if (drawerStartDate) {
	...
	onboarding
		.querySelectorAll("#rt-cr-start-time, #rt-onboarding-start-time")
		.forEach((input) => {
			if (input instanceof HTMLInputElement && input.value !== startTimeVal)
				input.value = startTimeVal;
		});
	const gearTier = s.onboardingGearTier || "auto";
	onboarding
		.querySelectorAll("#rt-onboarding-gear-tier, #rt-cr-gear-tier")
		.forEach((sel) => {
			if (sel instanceof HTMLSelectElement && sel.value !== gearTier)
				sel.value = gearTier;
		});
	const levelPref =
		s.onboardingLevel === "none" ? "none" : String(s.onboardingLevel || 1);
	onboarding
		.querySelectorAll("#rt-starting-level, #rt-cr-level")
		.forEach((sel) => {
			if (sel instanceof HTMLSelectElement && sel.value !== levelPref)
				sel.value = levelPref;
		});
	const useCombatGuide = s.onboardingUseCombatScalingGuide !== false;
	onboarding
		.querySelectorAll("#rt-onboarding-combat-guide-cb, #rt-cr-combat-guide-cb")
		.forEach((cb) => {
	...
	const abilitiesField = /** @type {HTMLElement|null} */ (
		onboarding.querySelector("#rt-cr-abilities")
	)?.closest(".rt-cr-field");
	if (abilitiesField)
		abilitiesField.style.display = s.modules?.abilities ? "" : "none";

	const playerCardCbSync = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt-onboarding-player-card-cb")
	);
	if (playerCardCbSync) playerCardCbSync.checked = !!s.onboardingCreatePersona;
	const stPersonaCbSync = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt-onboarding-st-persona-cb")
	);
	if (stPersonaCbSync)
		stPersonaCbSync.checked = s.onboardingCreateSillyTavernPersona !== false;
	const personaWordsSync = /** @type {HTMLSelectElement|null} */ (
		onboarding.querySelector("#rt-onboarding-persona-words")
	);
	const personaWordsCustomSync = /** @type {HTMLInputElement|null} */ (
		onboarding.querySelector("#rt-onboarding-persona-words-custom")
	);
	if (personaWordsSync)
		personaWordsSync.value = s.onboardingPersonaWords || "150";
	if (personaWordsCustomSync) {
		personaWordsCustomSync.value = s.onboardingPersonaWordsCustom || "";
		personaWordsCustomSync.style.display =
			personaWordsSync?.value === "other" ? "inline-block" : "none";
	}
}
...
function bookBelongsToPrefix(bookName, prefix) {
	...
	const rest = bookName.startsWith(prefix + "_")
...
async function activateCampaignBooks(opts = {}) {
	...
	if (!prefix) {
		if (typeof ctx.executeSlashCommandsWithOptions === "function") {
			...
			if (allNames.includes("World")) {
				if (s.worldProgressionEnabled) {
					await ctx
						.executeSlashCommandsWithOptions(
							'/world state=on silent=true "World"',
						)
						.catch(() => {});
				} else {
					await ctx
						.executeSlashCommandsWithOptions(
							'/world state=off silent=true "World"',
						)
						.catch(() => {});
				}
			}
		...
		_loreActivationDebugLast = {
			...baseDebug,
			stopped:
				"no routerCampaignPrefix (derive failed earlier or chat id empty)",
			storedPrefix: "",
		};
		renderLoreActivationDebugPanel();
		return 0;
	...
	bookNames = bookNames.filter((n) => {
		const isWorld =
			n.toLowerCase().endsWith("_world") || n.toLowerCase() === "world";
		return !isWorld;
	});
	...
	if (
		allNames.includes(worldBookName) &&
		!toDeactivate.includes(worldBookName)
	) {
...
async function cloneCampaignStack() {
	...
	const ctx = SillyTavern.getContext();
	const liveChatId =
		ctx.getCurrentChatId?.() || ctx.chatId || runtimeState.currentChatId || "";
	...
	if (!currentPrefix) {
		toastr["warning"](
			"No campaign prefix is active. Activate the Lorebook Agent and load a chat first.",
			"Clone Stack",
		);
		return;
	}
	...
	if (!newPrefix) {
		toastr["warning"](
			"New prefix cannot be empty or contain only special characters.",
			"Clone Stack",
		);
		return;
	}
	if (newPrefix.toLowerCase() === currentPrefix.toLowerCase()) {
		toastr["warning"](
			"New prefix is the same as the current prefix. Please choose a different name.",
			"Clone Stack",
		);
		return;
	}
	...
	if (result.matchingCount === 0) {
		toastr["warning"](
			`No lorebooks found for prefix "${currentPrefix}". Nothing to clone.`,
			"Clone Stack",
		);
		return;
	}
...
function resetUnseenChatState(s) {
	...
	if (typeof globalThis._rpgApplyAdventureCompanionSnapshot === "function") {
		globalThis._rpgApplyAdventureCompanionSnapshot(null, {
			resetIfMissing: true,
		});
	}
	...
	refreshOrderList();
	if (
		s.chatSetupLinkEnabled &&
		typeof globalThis._rpgSyncSettingsUi === "function"
	) {
	...
	if (deltaPanel)
		deltaPanel.innerHTML = '<span class="delta-empty">No changes yet.</span>';
	updateUIMemo("");
	refreshRenderedView();
	if (typeof runtimeState.renderRouterUI === "function")
		runtimeState.renderRouterUI();
...
async function refreshExtensionPrompt() {
	...
	const s = getSettings();
	if (
		!isLorebookAgentRuntimeActive(s) ||
		(!s.activeRouterKeys?.length && !s.activeWorldKeys?.length)
	) {
	...
	try {
		...
		for (const k of s.activeRouterKeys) {
			...
			const isWorld =
				bookName.toLowerCase().endsWith("_world") ||
				bookName.toLowerCase() === "world";
			if (isWorld) continue;
		...
		if (s.worldProgressionEnabled && s.activeWorldKeys?.length) {
			...
			for (const k of s.activeWorldKeys) {
				const [bookName] = k.split("::");
				if (!worldBooks[bookName])
					worldBooks[bookName] = await ctx.loadWorldInfo(bookName);
			}
...
function installRouterInterceptor() {
	const ctx = SillyTavern.getContext();
	const { addPromptManagerInterceptor, addChatInterceptor, addInterceptor } =
		ctx;
	...
	console.debug(
		"[RPG Tracker] Lore injection handled exclusively by rpgTrackerInterceptor (narrative-hooks.js). setExtensionPrompt cleared.",
	);
}
...
function syncRouterPrefixDisplays(raw) {
	...
	const settingsEl = document.getElementById(
		"rpg_tracker_router_prefix_display",
	);
...
function onChatChanged(newChatId) {
	...
	const migratedPortraitScope = migrateLegacyPortraitMapsToChat(
		s,
		oldChatId || resolvedId,
	);
	...
	if (
		isDeferredBootAttachment &&
		s.chatLinkEnabled &&
		shouldPreserveLiveChatStateOnBoot(s, resolvedId)
	) {
		saveChatState(resolvedId, { skipDiskWrite: true });
		console.warn(
			"[RPG Tracker] Preserved live tracker state during deferred boot chat attachment:",
			resolvedId,
		);
	}
	...
	if (
		typeof globalThis._rpgFlushAdventureCompanionForChat === "function" &&
		oldChatId
	) {
	...
	if (s.chatLinkEnabled && oldChatId)
		saveChatState(oldChatId, { skipDiskWrite: true });
	...
	if (chatBooks?.length) {
		...
		if (
			typeof SillyTavern.getContext().executeSlashCommandsWithOptions ===
			"function"
		) {
			(async () => {
				const ctx = SillyTavern.getContext();
				...
				await ctx
					.executeSlashCommandsWithOptions(
						`/world state=off silent=true "${oldWorldBookName}"`,
					)
					.catch(() => {});
				...
				for (const name of chatBooks) {
					await ctx
						.executeSlashCommandsWithOptions(
							`/world state=on silent=true "${name}"`,
						)
						.catch(() => {});
				}
				...
				if (s.worldProgressionEnabled) {
					await ctx
						.executeSlashCommandsWithOptions(
							`/world state=on silent=true "${newWorldBookName}"`,
						)
						.catch(() => {});
				} else {
					await ctx
						.executeSlashCommandsWithOptions(
							`/world state=off silent=true "${newWorldBookName}"`,
						)
						.catch(() => {});
				}
		...
		_prefixDeriveTimer = setTimeout(async () => {
			...
			await syncCampaignPrefixAndWorldsForChat(
				resolvedId,
				"CHAT_CHANGED(debounced)",
			);
	...
	if (!found && !s.chatStates?.[resolvedId]) {
		...
		if (isBranchSeedInProgress(resolvedId)) {
			...
			if (!retried && !s.chatStates?.[resolvedId]) {
				console.warn(
					"[RPG Tracker] Branch seed guard active but partition missing for",
					resolvedId,
				);
			}
		} else {
		...
		!found &&
		typeof globalThis._rpgLoadAdventureCompanionForChat === "function"
	) {
...
function updateChatLinkUI() {
	...
	const setupCb = document.getElementById(
		"rpg_tracker_chat_setup_link_enabled",
	);
	if (setupCb instanceof HTMLInputElement)
		setupCb.checked = !!s.chatSetupLinkEnabled;
}
...
async function applyChatLinkToggle(turningOn) {
	...
	if (turningOn && runtimeState.currentChatId) {
		...
		const setupChanged =
			!!s.chatSetupLinkEnabled &&
			!!saved?.setup &&
			!chatSetupsMatch(s, saved.setup);

		const hasConflict =
			(savedContent && liveContent && liveContent !== savedContent) ||
		...
		if (hasConflict && saved) {
			...
			if (choice === POPUP_RESULT.AFFIRMATIVE) {
				...
				toastr["success"](
					"Chat Link ON — restored saved state.",
					"RPG Tracker",
				);
				...
				saveChatState(runtimeState.currentChatId);
				toastr["success"](
					"Chat Link ON — current state saved to chat.",
					"RPG Tracker",
				);
			} else {
				return false;
			...
			toastr["success"](
				"Chat Link ON — state bound to this chat.",
				"RPG Tracker",
			);
		}
	} else if (turningOn) {
...
function updatePanelStatus() {
	...
	const enableBtn = /** @type {HTMLElement|null} */ (
		document.getElementById("rpg-tracker-enable-btn")
	);
	...
	if (enableBtn) {
		...
		enableBtn.title = settings.enabled
			? "Disable Multihog Framework"
			: "Enable Multihog Framework";
	}
	// Keep settings sidebar checkbox in sync
	const sidebarEnableCheck = /** @type {HTMLInputElement|null} */ (
		document.getElementById("rpg_tracker_enabled")
	);
	...
	if (typeof runtimeState.updateAgentPanelDisabledRef === "function") {
		...
		if (ap) {
			if (isLorebookAgentRuntimeActive(settings))
				ap.classList.remove("is-agent-disabled");
	...
	if (!settings.enabled) {
		...
		if (header)
			/** @type {HTMLElement} */ (header).style.pointerEvents = "auto";
 ...
 */
async function runStateModelPass(
	narrativeOutput,
	isFullContext = false,
	overrideLookback = null,
) {
	const settings = getSettings();
	...
	if (!generateRaw) {
		...
		broadcastStateTrackerStep(
			"error",
			"State Tracker unavailable: text generation is not connected.",
		);
		return;
	}
	...
	try {
		...
		broadcastStateTrackerStep(
			"start",
			isFullContext
				? "Initializing State Tracker full audit..."
				: "Initializing State Tracker pass...",
		);
		...
		if (isFullContext) {
			systemPrompt = systemPrompt
				.replace(
					/Only output sections that actually changed/gi,
					"Perform a full audit of the narrative history and output the COMPLETE state for all enabled modules",
				)
				.replace(
					/Omit unchanged sections entirely/gi,
					"Do NOT omit any section; output a complete, verified state memo",
				);
		}
		...
		if (isFullContext) {
			const maxContextLimit =
				SillyTavern.getContext().contextSize ||
				settings.fullAuditMaxTokens ||
				32000;
			...
			for (const m of chat) {
				...
				if (
					currentTokens + lineTokens > chunkTokenLimit &&
			...
			const useFixedLookback =
				overrideLookback !== null || settings.lookbackSinceLastUser === false;
			let startIdx;
			if (useFixedLookback) {
				const N =
					overrideLookback !== null
						? overrideLookback
						: settings.lookbackMessages !== undefined
		...
		if (
			...
			settings.memoHistory.length > 0
		) {
			const historyToInclude = settings.memoHistory
				.slice(0, historyCount)
				.reverse();
			const historyString = historyToInclude
				.map((memo, i) => {
		...
		function commitChunkResult(
			...
			if (settings.historyIndex !== undefined && settings.historyIndex !== -1) {
				if (settings.debugMode)
					console.log(
						`[RPG Tracker] Splicing history at index ${settings.historyIndex} due to new update.`,
					);
		...
		for (let i = 0; i < chunks.length; i++) {
			...
			const memoBeforeThisChunk = settings.currentMemo
				.replace(/<\/?memo>/gi, "")
				.trim();

			if (isFullContext && chunks.length > 1) {
				toastr.info(
					`Running Full Audit: Chunk ${i + 1} of ${chunks.length}...`,
					"RPG Tracker",
					{ timeOut: 5000 },
				);
				...
				broadcastStateTrackerStep(
					"thought",
					`Processing full-audit chunk ${i + 1} of ${chunks.length}...`,
				);
				...
				broadcastStateTrackerStep(
					"thought",
					isFullContext
						? "Analyzing narrative history..."
						: "Analyzing recent narrative...",
				);
			}
			...
			const result = await sendStateRequest(
				...
				userPrompt,
				signal,
				{ stream: true, debugSource: "Tracker" },
			);
			...
			if (result && typeof result === "string") {
				if (settings.debugMode)
					console.log(`[RPG Tracker] Raw Result (Chunk ${i + 1}):`, result);
				...
				const memoBlocks = [
					...cleanedOutput.matchAll(/<memo>([\s\S]*?)<\/memo>/gi),
				];
				...
				if (settings.debugMode) {
					console.log(
						`[RPG Tracker] Memo ${merged !== memoBeforeThisChunk ? "updated" : "unchanged"} after chunk ${i + 1}.`,
					);
				}
				...
				if (relationshipCommands.length) {
					...
					const relResult = await applyStateTrackerRelationshipCommands(
				...
				if (settings.debugMode)
					console.log(
						`[RPG Tracker] Chunk ${i + 1}/${chunks.length} committed.`,
					);
			}
		}
		...
		if (abandonedForChatSwitch) {
			broadcastStateTrackerStep(
				"error",
				"Stopped because the active chat changed.",
			);
			...
			broadcastStateTrackerStep(
				"finish",
				isFullContext
					? "State Tracker full audit complete."
					: "State Tracker pass complete.",
			);
		}

		if (settings.debugMode)
			console.log("[RPG Tracker] State Model pass complete.");
		return lastDelta;
	} catch (error) {
		if (error.name === "AbortError") {
			if (settings.debugMode)
				console.log("[RPG Tracker] State Model pass aborted by user.");
			broadcastStateTrackerStep("error", "Stopped by user.");
			return;
		}
...
function resetTrackerUi(opts = {}) {
	...
	try {
		if (typeof closeAdventureCompanion === "function")
			closeAdventureCompanion();
	} catch (_) {}
	...
	for (const key of [...Object.keys(localStorage)]) {
		if (
			key === "rpg_tracker_geometry" ||
			key.startsWith("rpg_tracker_geometry_")
		) {
	...
	document
		.querySelectorAll("body > .rpg-tracker-detached-panel")
		.forEach((el) => el.remove());

	createPanel();
	if (typeof updatePanelStatus === "function") updatePanelStatus();
	if (typeof applyPanelBackgroundToDom === "function")
		applyPanelBackgroundToDom();

	const msg =
		"Multihog UI reset: panels rebuilt, detached/layout state cleared.";
...
globalThis._rpgGetCurrentPrefix = () =>
	getEffectiveRouterCampaignPrefix(SillyTavern.getContext().chatId || "");
...
function handleLevelUp() {
	...
	toastr["success"](
		"Level Up Detected! System prompt injected.",
		"RPG Tracker",
	);
	...
	if (sendSystemMessage) {
		sendSystemMessage(
			"generic",
			"SYSTEM: Level Up Detected! The character has gained a level. Acknowledge this immediately and prompt the user to make their level-up choices or grant them their logical boons.",
		);
	}
}
...
export async function sendDirectPrompt(message, options = {}) {
	if (runtimeState.stateModelRunning) {
		toastr["info"](
			"State Model is already running. Please wait.",
			"RPG Tracker",
		);
		return {
			success: false,
			status: "busy",
			changed: false,
			message: "State Tracker is already running.",
		};
	}
	...
	if (!generateRaw) {
		toastr["warning"](
			"Text generation is not available. Connect an API in SillyTavern settings.",
			"RPG Tracker",
		);
		return {
			success: false,
			status: "unavailable",
			changed: false,
			message: "State Tracker connection is unavailable.",
		};
	}
	...
	try {
		...
		broadcastStateTrackerStep(
			"start",
			"Processing direct State Tracker instruction...",
		);
		...
		if (
			...
			return {
				...
				message: signal.aborted
					? "State Tracker command was cancelled."
					: "Active chat changed; State Tracker commit was skipped.",
			};
		}
		const worldLoreSection = worldLore ? worldLore + "\n\n" : "";
		...
		const sanitizedCurrentFull = stripMemoHtml(
			settings.currentMemo.replace(/<\/?memo>/gi, "").trim(),
		);
		const sanitizedCurrentForPrompt = stripMemoHtml(
			memoForTrackerContext(sanitizedCurrentFull),
		);

		const { chat } = SillyTavern.getContext();
		const N =
			settings.directPromptContext !== undefined
				? settings.directPromptContext
				: 5;
		let chatLog = "";
		...
		broadcastStateTrackerStep(
			"thought",
			"Requesting memo update from State Tracker...",
		);
		const result = await sendStateRequest(
			options.connectionSettings || settings,
			systemPrompt,
			userPrompt,
			signal,
			{ stream: true, debugSource: "Tracker" },
		...
		if (result && typeof result === "string") {
			...
			if (merged !== sanitizedCurrentFull) {
				...
				// Linear Stone History Logic
				if (
					settings.historyIndex !== undefined &&
				...
				broadcastStateTrackerStep(
					"result",
					"Memo updated from direct instruction.",
				);
				broadcastStateTrackerStep(
					"finish",
					"Direct State Tracker instruction complete.",
				);
				...
				return {
					...
					status: "unchanged",
					changed: false,
					message: "State Tracker made no changes.",
				};
			}
		} else {
			broadcastStateTrackerStep("error", "State Tracker returned no output.");
			toastr["warning"](
				"State Model returned no output. Check your API connection and State Model settings.",
				"RPG Tracker",
			);
			return {
				success: false,
				status: "no_output",
				changed: false,
				message: "State Tracker returned no output.",
			};
		}
	} catch (err) {
		if (err.name === "AbortError") {
			if (settings.debugMode)
				console.log("[RPG Tracker] Direct prompt aborted by user.");
			broadcastStateTrackerStep("error", "Stopped by user.");
			return {
				success: false,
				status: "cancelled",
				changed: false,
				message: "State Tracker command was cancelled.",
			};
		}
		...
		return {
			...
			status: "failed",
			changed: false,
			message: err?.message || "State Tracker command failed.",
		};
	} finally {
		runtimeState.stateModelRunning = false;
		runtimeState.stateController = null;
...
function loadProfile(name) {
	...
	s.blockOrder = p.blockOrder
		? JSON.parse(JSON.stringify(p.blockOrder))
		: s.blockOrder;
	s.stockPrompts = loadStockPromptsFromProfile(p.stockPrompts);
	s.modulePageSizes = p.modulePageSizes
		? JSON.parse(JSON.stringify(p.modulePageSizes))
		: {};
	s.customFields = p.customFields
		? JSON.parse(JSON.stringify(p.customFields))
		: [];
	...
	s.worldProgressionLocationsPerReport =
		p.worldProgressionLocationsPerReport ?? 3;
	s.worldProgressionLocationRandomize =
		p.worldProgressionLocationRandomize !== false;
	s.worldProgressionLocationLastAdvanced = JSON.parse(
		JSON.stringify(p.worldProgressionLocationLastAdvanced || {}),
	);
	s.worldProgressionSkeletonFactions = p.worldProgressionSkeletonFactions ?? 4;
	s.worldProgressionSkeletonLocations =
		p.worldProgressionSkeletonLocations ?? 4;
	s.worldProgressionSkeletonConflicts =
		p.worldProgressionSkeletonConflicts ?? 3;
	s.worldProgressionSkeletonUseLorebooks =
		p.worldProgressionSkeletonUseLorebooks ?? false;
	s.worldProgressionSkeletonLorebookFilter = JSON.parse(
		JSON.stringify(p.worldProgressionSkeletonLorebookFilter || []),
	);
	s.worldProgressionSkeletonLorebookOnly =
		p.worldProgressionSkeletonLorebookOnly ?? false;
	s.worldProgressionLastFiredAtMinutes =
		p.worldProgressionLastFiredAtMinutes ?? -1;
	s.worldProgressionLastFiredPeriodLabel =
		p.worldProgressionLastFiredPeriodLabel || "";
	...
	s.portraitStoryLookback = Math.max(
		0,
		Math.min(100, Number(p.portraitStoryLookback) || 5),
	);
	...
	s.portraitRealtimeTriggerMode = [
		"location_enter",
		"location_change",
		"every_n_outputs",
	].includes(p.portraitRealtimeTriggerMode)
		? p.portraitRealtimeTriggerMode
		: "location_change";
	s.portraitRealtimeEveryNOutputs = Math.max(
		1,
		Number(p.portraitRealtimeEveryNOutputs) || 1,
	);
	...
	s.mapArchitectSystemPrompt =
		p.mapArchitectSystemPrompt || DEFAULT_MAP_ARCHITECT_SYSTEM_PROMPT;
	...
	s.mapRuntimeConnectionSource =
		p.mapRuntimeConnectionSource ?? p.mapArchitectConnectionSource ?? "default";
	s.mapRuntimeConnectionProfileId =
		p.mapRuntimeConnectionProfileId || p.mapArchitectConnectionProfileId || "";
	s.mapRuntimeCompletionPresetId =
		p.mapRuntimeCompletionPresetId || p.mapArchitectCompletionPresetId || "";
	s.mapRuntimeOllamaUrl =
		p.mapRuntimeOllamaUrl ||
		p.mapArchitectOllamaUrl ||
		"http://localhost:11434";
	s.mapRuntimeOllamaModel =
		p.mapRuntimeOllamaModel || p.mapArchitectOllamaModel || "";
	s.mapRuntimeOpenaiUrl =
		p.mapRuntimeOpenaiUrl || p.mapArchitectOpenaiUrl || "";
	s.mapRuntimeOpenaiKey =
		p.mapRuntimeOpenaiKey || p.mapArchitectOpenaiKey || "";
	s.mapRuntimeOpenaiModel =
		p.mapRuntimeOpenaiModel || p.mapArchitectOpenaiModel || "";
	s.mapRuntimeConnectionSeeded = true;
	s.mapEvolutionConnectionSource =
		p.mapEvolutionConnectionSource ??
		p.mapRuntimeConnectionSource ??
		p.mapArchitectConnectionSource ??
		"default";
	s.mapEvolutionConnectionProfileId =
		p.mapEvolutionConnectionProfileId ||
		p.mapRuntimeConnectionProfileId ||
		p.mapArchitectConnectionProfileId ||
		"";
	s.mapEvolutionCompletionPresetId =
		p.mapEvolutionCompletionPresetId ||
		p.mapRuntimeCompletionPresetId ||
		p.mapArchitectCompletionPresetId ||
		"";
	s.mapEvolutionOllamaUrl =
		p.mapEvolutionOllamaUrl ||
		p.mapRuntimeOllamaUrl ||
		p.mapArchitectOllamaUrl ||
		"http://localhost:11434";
	s.mapEvolutionOllamaModel =
		p.mapEvolutionOllamaModel ||
		p.mapRuntimeOllamaModel ||
		p.mapArchitectOllamaModel ||
		"";
	s.mapEvolutionOpenaiUrl =
		p.mapEvolutionOpenaiUrl ||
		p.mapRuntimeOpenaiUrl ||
		p.mapArchitectOpenaiUrl ||
		"";
	s.mapEvolutionOpenaiKey =
		p.mapEvolutionOpenaiKey ||
		p.mapRuntimeOpenaiKey ||
		p.mapArchitectOpenaiKey ||
		"";
	s.mapEvolutionOpenaiModel =
		p.mapEvolutionOpenaiModel ||
		p.mapRuntimeOpenaiModel ||
		p.mapArchitectOpenaiModel ||
		"";
	...
	s.mapUpdaterSystemPrompt =
		p.mapUpdaterSystemPrompt || DEFAULT_MAP_UPDATER_SYSTEM_PROMPT;
	...
	s.mapEvolutionIntervalHours = Math.max(
		1,
		Number(p.mapEvolutionIntervalHours) || 8,
	);
	...
	s.mapEvolutionOnSiteIntervalMinutes = Math.max(
		0,
		Math.min(59, Math.floor(Number(p.mapEvolutionOnSiteIntervalMinutes) || 0)),
	);
	s.mapEvolutionOnSitePreset =
		p.mapEvolutionOnSitePreset === "standard" ? "standard" : "dynamic";
	s.mapEvolutionIntervalHoursBySite = JSON.parse(
		JSON.stringify(p.mapEvolutionIntervalHoursBySite || {}),
	);
	...
	s.mapEvolutionNarratorCommitTokens =
		normalizeMapEvolutionNarratorCommitTokens(
			p.mapEvolutionNarratorCommitTokens,
		);
	s.mapEvolutionCompressSystemPrompt =
		p.mapEvolutionCompressSystemPrompt ||
	...
	s.mapEvolutionSelectedRoots = JSON.parse(
		JSON.stringify(p.mapEvolutionSelectedRoots || []),
	);
	s.mapEvolutionSystemPrompt =
		p.mapEvolutionSystemPrompt || DEFAULT_MAP_EVOLUTION_SYSTEM_PROMPT;
	s.mapEvolutionLastFiredBySite = JSON.parse(
		JSON.stringify(p.mapEvolutionLastFiredBySite || {}),
	);
	s.mapEvolutionBacklogBySite = JSON.parse(
		JSON.stringify(p.mapEvolutionBacklogBySite || {}),
	);
	s.mapEvolutionThreadsBySite = JSON.parse(
		JSON.stringify(p.mapEvolutionThreadsBySite || {}),
	);
	...
	s.mapEvolutionWorldReportApplications = JSON.parse(
		JSON.stringify(p.mapEvolutionWorldReportApplications || {}),
	);
	...
	s.gameSystemWizardConnectionSource =
		p.gameSystemWizardConnectionSource ?? "default";
	s.gameSystemWizardConnectionProfileId =
		p.gameSystemWizardConnectionProfileId || "";
	s.gameSystemWizardCompletionPresetId =
		p.gameSystemWizardCompletionPresetId || "";
	s.gameSystemWizardOllamaUrl =
		p.gameSystemWizardOllamaUrl || "http://localhost:11434";
	...
	s.characterCreationConnectionSource =
		p.characterCreationConnectionSource ?? "default";
	s.characterCreationConnectionProfileId =
		p.characterCreationConnectionProfileId || "";
	s.characterCreationCompletionPresetId =
		p.characterCreationCompletionPresetId || "";
	s.characterCreationOllamaUrl =
		p.characterCreationOllamaUrl || "http://localhost:11434";
	...
	$("#rpg_world_progression_skeleton_factions").val(
		s.worldProgressionSkeletonFactions ?? 4,
	);
	$("#rpg_world_progression_skeleton_locations").val(
		s.worldProgressionSkeletonLocations ?? 4,
	);
	$("#rpg_world_progression_skeleton_conflicts").val(
		s.worldProgressionSkeletonConflicts ?? 3,
	);
	$("#rpg_world_progression_skeleton_use_lorebooks").prop(
		"checked",
		!!s.worldProgressionSkeletonUseLorebooks,
	);
	$("#rpg_world_progression_skeleton_lorebook_filter_group").toggle(
		!!s.worldProgressionSkeletonUseLorebooks,
	);
	$("#rpg_world_progression_skeleton_lorebook_only").prop(
		"checked",
		!!s.worldProgressionSkeletonLorebookOnly,
	);
	$("#rpg_world_progression_skeleton_lorebook_only").prop(
	...
	const profileLorebookOnlyActive =
		!!s.worldProgressionSkeletonUseLorebooks &&
		!!s.worldProgressionSkeletonLorebookOnly;
	$("#rpg_world_progression_skeleton_counts")
		.css({
			opacity: profileLorebookOnlyActive ? "0.4" : "1",
			pointerEvents: profileLorebookOnlyActive ? "none" : "auto",
		})
		.find("input")
		.prop("disabled", profileLorebookOnlyActive);
	if (
		s.worldProgressionSkeletonUseLorebooks &&
		typeof globalThis._rpgRefreshSkeletonLorebookList === "function"
	) {
		void globalThis._rpgRefreshSkeletonLorebookList();
	}
	$("#rpg_world_progression_exclusion_list").val(
		s.worldProgressionExclusionList,
	);
	$("#rpg_world_progression_locations_per_report").val(
		s.worldProgressionLocationsPerReport ?? 3,
	);
	$("#rpg_world_progression_location_randomize").prop(
		"checked",
		s.worldProgressionLocationRandomize !== false,
	);

	// Sync portrait connection settings UI
	$("#rpg_portrait_generator_source").val(
		s.portraitGeneratorSource || "native",
	);
	$("#rpg_tracker_pollinations_group").toggle(
		(s.portraitGeneratorSource || "native") === "pollinations",
	);
	$("#rpg_tracker_portrait_skip_prompt").prop(
		"checked",
		!!s.portraitSkipPromptDialog,
	);
	$("#rpg_tracker_hide_image_gen_toasts").prop(
		"checked",
		!!s.hideImageGenToasts,
	);
	$("#rpg_tracker_portrait_use_story_lookback").prop(
		"checked",
		!!s.portraitUseStoryLookback,
	);
	...
	$("#rpg_tracker_portrait_auto_party").prop(
		"checked",
		!!s.portraitAutoGenerateParty,
	);
	$("#rpg_tracker_portrait_auto_player").prop(
		"checked",
		!!s.portraitAutoGeneratePlayer,
	);
	$("#rpg_tracker_portrait_auto_enemies").prop(
		"checked",
		!!s.portraitAutoGenerateEnemies,
	);
	$("#rpg_tracker_portrait_auto_npcs").prop(
		"checked",
		!!s.portraitAutoGenerateNpcs,
	);
	$("#rpg_tracker_portrait_auto_locations").prop(
		"checked",
		!!s.portraitAutoGenerateLocations,
	);
	$("#rpg_tracker_portrait_auto_scene_view").prop(
		"checked",
		!!s.portraitAutoGenerateSceneView,
	);
	$("#rpg_tracker_location_images").prop("checked", !!s.locationImages);
	syncNpcPortraitDependentUi(s);
	syncLocationImageDependentUi(s);
	$("#rpg_portrait_connection_source").val(
		s.portraitConnectionSource || "default",
	);
	$("#rpg_portrait_connection_profile").val(
		s.portraitConnectionProfileId || "",
	);
	$("#rpg_portrait_completion_preset").val(s.portraitCompletionPresetId || "");
	$("#rpg_portrait_ollama_url").val(
		s.portraitOllamaUrl || "http://localhost:11434",
	);
	...
	$("#rpg_map_architect_system_prompt").val(
		s.mapArchitectSystemPrompt || DEFAULT_MAP_ARCHITECT_SYSTEM_PROMPT,
	);
	$("#rpg_map_architect_connection_source").val(
		s.mapArchitectConnectionSource || "default",
	);
	$("#rpg_map_architect_connection_profile").val(
		s.mapArchitectConnectionProfileId || "",
	);
	$("#rpg_map_architect_completion_preset").val(
		s.mapArchitectCompletionPresetId || "",
	);
	$("#rpg_map_architect_ollama_url").val(
		s.mapArchitectOllamaUrl || "http://localhost:11434",
	);
	...
	$("#rpg_map_architect_openai_model_manual").val(
		s.mapArchitectOpenaiModel || "",
	);
	...
	$("#rpg_map_updater_system_prompt").val(
		s.mapUpdaterSystemPrompt || DEFAULT_MAP_UPDATER_SYSTEM_PROMPT,
	);
	$("#rpg_map_evolution_enabled").prop(
		"checked",
		s.mapEvolutionEnabled !== false,
	);
	$("#rpg_map_evolution_interval_hours").val(s.mapEvolutionIntervalHours ?? 8);
	$("#rpg_map_evolution_onsite_interval_hours").val(
		s.mapEvolutionOnSiteIntervalHours ?? 1,
	);
	$("#rpg_map_evolution_onsite_interval_minutes").val(
		s.mapEvolutionOnSiteIntervalMinutes ?? 0,
	);
	$("#rpg_map_evolution_onsite_preset").val(
		s.mapEvolutionOnSitePreset === "standard" ? "standard" : "dynamic",
	);
	$("#rpg_map_evolution_lookback").val(s.mapEvolutionLookback ?? 20);
	$("#rpg_map_evolution_max_tokens").val(s.mapEvolutionMaxTokens ?? 25000);
	$("#rpg_map_evolution_compress_enabled").prop(
		"checked",
		s.mapEvolutionCompressEnabled !== false,
	);
	$("#rpg_map_evolution_compress_threshold").val(
		s.mapEvolutionCompressThreshold ?? 10000,
	);
	$("#rpg_map_evolution_narrator_commit_tokens").val(
		s.mapEvolutionNarratorCommitTokens ?? 2000,
	);
	$("#rpg_map_evolution_world_report_lookback").val(
		s.mapEvolutionWorldReportLookback ?? 5,
	);
	applyMapEvolutionTickSettingsToUi(s);
	$("#rpg_map_evolution_system_prompt").val(
		s.mapEvolutionSystemPrompt || DEFAULT_MAP_EVOLUTION_SYSTEM_PROMPT,
	);
	$("#rpg_map_evolution_compress_prompt").val(
		s.mapEvolutionCompressSystemPrompt ||
			DEFAULT_MAP_EVOLUTION_COMPRESS_SYSTEM_PROMPT,
	);
	syncMapThemeUi(s);
	...
	$("#rpg_gs_wizard_connection_source").val(
		s.gameSystemWizardConnectionSource || "default",
	);
	$("#rpg_gs_wizard_connection_profile").val(
		s.gameSystemWizardConnectionProfileId || "",
	);
	$("#rpg_gs_wizard_completion_preset").val(
		s.gameSystemWizardCompletionPresetId || "",
	);
	$("#rpg_gs_wizard_ollama_url").val(
		s.gameSystemWizardOllamaUrl || "http://localhost:11434",
	);
	...
	$("#rpg_gs_wizard_openai_model_manual").val(
		s.gameSystemWizardOpenaiModel || "",
	);

	// Toggle container visibilities
	$("#rpg_portrait_profile_group").toggle(
		s.portraitConnectionSource === "profile",
	);
	$("#rpg_portrait_ollama_group").toggle(
		s.portraitConnectionSource === "ollama",
	);
	$("#rpg_portrait_openai_group").toggle(
		s.portraitConnectionSource === "openai",
	);
	$("#rpg_map_architect_profile_group").toggle(
		s.mapArchitectConnectionSource === "profile",
	);
	$("#rpg_map_architect_ollama_group").toggle(
		s.mapArchitectConnectionSource === "ollama",
	);
	$("#rpg_map_architect_openai_group").toggle(
		s.mapArchitectConnectionSource === "openai",
	);
	...
	$("#rpg_gs_wizard_profile_group").toggle(
		s.gameSystemWizardConnectionSource === "profile",
	);
	$("#rpg_gs_wizard_ollama_group").toggle(
		s.gameSystemWizardConnectionSource === "ollama",
	);
	$("#rpg_gs_wizard_openai_group").toggle(
		s.gameSystemWizardConnectionSource === "openai",
	);
	...
	if (dp)
		dp.innerHTML =
			s.lastDelta || '<span class="delta-empty">No changes yet.</span>';
	syncMemoView();
}
...
function refreshProfileDropdown() {
	...
	sel.innerHTML =
		...
		names
			.map(
				(n) =>
					`<option value="${escapeHtml(n)}"${n === s.activeProfile ? " selected" : ""}>${escapeHtml(n)}</option>`,
			)
			.join("");
}

/** Shared Popup options for long help/docs dialogs (scrollable on mobile). */
const RT_HELP_POPUP_OPTS = {
	okButton: "Got it",
	cancelButton: false,
...
async function showRngExplanation() {
	...
	const ol = (items) =>
		`<ol style="margin: 6px 0 0 0; padding-left: 20px; text-align: left; list-style-position: outside;">${items.map((t) => `<li style="margin-bottom: 4px;">${t}</li>`).join("")}</ol>`;
	const popupBody = `
            <div style="font-size: 0.9em; line-height: 1.5; max-width: 520px; text-align: left;">
                ...
                ${card(
									"🧭",
									"CYOA Mode + combat fix the queue",
...
async function showComponentsExplanation() {
	...
	const popupBody = `
            <div style="font-size: 0.9em; line-height: 1.5; max-width: 480px; text-align: left;">
                ...
                ${card(
									"💤",
									"Resting Restrictions (=>9h between rests)",
                ...
                ${card(
									"🧭",
									"CYOA Mode (action choices every turn)",
                ...
                ${card(
									"💞",
									"Relationship System (Friendship & Affection)",
...
function applyPortraitPromptPresetBundle(settings, bundle, meta = {}) {
	...
	$("#rpg_portrait_character_system_prompt").val(
		settings.portraitCharacterSystemPrompt,
	);
	if (bundle.locationSystemPrompt !== undefined) {
		setPortraitLocationPromptTextarea(settings.portraitLocationSystemPrompt);
	}
	if (bundle.includePresentNpcs !== undefined) {
		$("#rpg_portrait_location_include_present_npcs").prop(
			"checked",
			!!settings.portraitLocationIncludePresentNpcs,
		);
	}
	if (bundle.wordTarget !== undefined) {
		$("#rpg_portrait_prompt_word_target").val(
			settings.portraitPromptWordTarget,
		);
	}
	syncLocationImageDependentUi(settings);
}
...
function refreshPortraitPromptPresetsList() {
	...
	const container = document.getElementById(
		"rpg_portrait_prompt_presets_container",
	);
	...
	const appendSectionLabel = (label) => {
		...
		header.style.cssText =
			"font-size:0.78em;font-weight:bold;opacity:0.7;margin:6px 0 2px;";
	...
	for (const preset of FACTORY_PORTRAIT_PROMPT_PRESETS) {
		appendRow({
			...
			onLoad: () => {
				...
				const bundle = resolveFactoryPortraitPromptBundle(
					preset.id,
					includeNpcs,
				);
				applyPortraitPromptPresetBundle(
					settings,
					{
				...
				toastr["success"](
					`Loaded factory style: ${preset.name}`,
					"Portrait Prompt Library",
				);
			},
		});
	}
	...
	appendSectionLabel(
		entries.length ? "Your Saved Setups" : "Your Saved Setups (none yet)",
	);
	entries.forEach(([name, preset]) => {
		appendRow({
			...
			onLoad: () => {
				applyPortraitPromptPresetBundle(settings, preset, {
					activeId: `user:${name}`,
				});
				saveSettings();
				refreshPortraitPromptPresetsList();
				toastr["success"](
					`Loaded portrait prompt setup: ${name}`,
					"Portrait Prompt Library",
				);
			},
			onDelete: () => {
				if (
					confirm(
						`Are you sure you want to delete the portrait prompt setup "${name}"?`,
					)
				) {
...
async function showPortraitSettingsMenu(
	entityName,
	onRefresh,
	npcContent = null,
	options = {},
	...
	const currentSrc = String(
		options.currentSrc || lookupCustomPortraitSrc(s, entityName) || "",
	);
	...
	if (!ctx.callGenericPopup) {
		toastr["warning"]("Popup API not available.", "RPG Tracker");
		return;
	}
	const popupOpts = {
		okButton: "Apply",
	...
	if (currentSrc) {
		popupOpts.customButtons.push({
			text: "✂️ Crop Existing",
			result: 5,
			classes: ["menu_button"],
		});
		popupOpts.customButtons.push({
	...
	const persistPortrait =
		typeof options.applyPortrait === "function"
		...
		if (browseBtn && fileInput) {
			...
			fileInput.addEventListener("change", async () => {
				...
				try {
					...
					capturedUrl = "";
					if (urlInput)
						urlInput.value = "(file selected — click Apply to crop ✔)";
				} catch (err) {
					console.error(err);
		...
		if (currentSrc) {
			...
			if (wrap && img && badge) {
				...
				function initView() {
					...
					const parentW = wrap.parentElement
						? wrap.parentElement.offsetWidth - 20
				...
				setTimeout(() => {
					const popup = wrap.closest(
						'.popup, .dialogue_popup, [class*="popup"]',
					);
					if (popup) {
						const closeBtn = popup.querySelector(
							'.popup-close, .menu_button[data-result="0"], button[data-result]',
						);
						if (closeBtn)
							closeBtn.addEventListener("click", origPopupCleanup, {
								once: true,
							});
					}
	...
	if (result === 2) {
		...
		try {
			if (s.portraitSkipPromptDialog) {
				imageGenToast(
					"info",
					`Generating portrait for ${entityName} in background…`,
					"RPG Tracker",
				);
				const aiPrompt =
					npcContent !== null
				...
				if (!aiPrompt) {
					toastr["warning"](
						"Could not generate prompt — no context found.",
						"RPG Tracker",
					);
					return;
				}
				imageGenToast(
					"info",
					`Generating image for ${entityName}…`,
					"RPG Tracker",
				);
				...
				imageGenToast(
					"success",
					`Portrait auto-generated and applied for ${entityName}!`,
					"RPG Tracker",
				);
			} else {
			...
			toastr["error"](
				"AI portrait generation failed: " + (err.message || err),
				"RPG Tracker",
			);
		}
	} else if (result) {
		if (capturedRawUrl) {
			...
			(capturedUrl.startsWith("data:image/") ||
				/^https?:\/\//i.test(capturedUrl))
		) {
			await localApply(capturedUrl);
		} else if (capturedUrl) {
			toastr["warning"](
				"Please enter a valid https:// URL or use the Browse button.",
				"RPG Tracker",
			);
		}
	}
...
async function showLocationImageSettingsMenu(
	locationPath,
	onRefresh,
	locContent = "",
) {
	...
	if (result === 2) {
		...
		try {
			if (s.portraitSkipPromptDialog) {
				imageGenToast(
					"info",
					`Generating location image for ${normPath} in background…`,
					"RPG Tracker",
				);
				const aiPrompt = await generateLocationImagePrompt(
					normPath,
					locContent || "",
				);
				if (!aiPrompt) {
				...
				imageGenToast(
					"success",
					`Location image generated for ${normPath}!`,
					"RPG Tracker",
				);
			} else {
				imageGenToast(
					"info",
					"Generating location image prompt…",
					"RPG Tracker",
				);
				const aiPrompt = await generateLocationImagePrompt(
					normPath,
			...
			toastr["error"](
				"AI location image generation failed: " + (err.message || err),
				"RPG Tracker",
			);
		}
	} else if (result) {
...
function syncOnboardingPersonaPrefsFromDom(el) {
	if (!el) return;
	const playerCardCb = /** @type {HTMLInputElement|null} */ (
		el.querySelector("#rt-onboarding-player-card-cb")
	);
	const stPersonaCb = /** @type {HTMLInputElement|null} */ (
		el.querySelector("#rt-onboarding-st-persona-cb")
	);
	const wordsSelect = /** @type {HTMLSelectElement|null} */ (
		el.querySelector("#rt-onboarding-persona-words")
	);
	const wordsCustom = /** @type {HTMLInputElement|null} */ (
		el.querySelector("#rt-onboarding-persona-words-custom")
	);
	const s = getSettings();
...
async function maybeCreateOnboardingPersona(extraHints = "", options = {}) {
	...
	const charName =
		preferredName || extractCharNameFromMemo(s.currentMemo) || "My Character";
	if (createStPersona) {
		try {
			...
			console.error(
				"[RPG Tracker] Could not create name-only ST persona:",
				error,
			);
			toastr["warning"](
				`Character created, but the ST persona for "${charName}" could not be created.`,
				"RPG Tracker",
			);
		}
	}
	...
	if (bio) {
		...
		toastr["warning"](
			"Character created, but Player Card generation failed.",
			"RPG Tracker",
		);
	}
}
...
function getDisplayQuests(memoText) {
	...
	const activeFromMemo = memoQuests.filter(
		(q) => !isArchivedQuestStatus(q.status),
	);
	...
	const archivedFromSettings = (s.quests || []).filter(
		(q) => isArchivedQuestStatus(q.status) && !memoIds.has(q.id),
	);
	// Include archived rows still in memo until the next strip pass
	const archivedFromMemo = memoQuests.filter((q) =>
		isArchivedQuestStatus(q.status),
	);
...
export function refreshRenderedView() {
	...
	const displayMemo = substituteDisplayMacros(
		runtimeState.historyViewIndex === -1
	...
	if (el) {
		const bottomXp = /** @type {HTMLElement|null} */ (
			document.getElementById("rt-bottom-xp-bar")
		);
		const xpAnimationHost = s.xpBarAtBottom === true ? bottomXp : el;
		const capturedXp = captureXpGainAnimationState(
			xpAnimationHost,
			xpAnimationContext,
		);
		const capturedBars = captureBarChangeAnimationState(el, xpAnimationContext);
		const questsEnabled =
			s.syspromptModules?.quests !== false && !!(memo && memo.trim());
		let html;

		if (s.panelLayoutMode === "tabs") {
			const questsCtx = questsEnabled
				? { quests: getDisplayQuests(displayMemo), currentTime }
				: null;
		...
		if (bottomXp) {
			const bottomXpHtml =
				s.xpBarAtBottom === true ? renderBottomXpBar(displayMemo) : "";
		...
		document
			.querySelectorAll("#rt-footer-location, #rt-agent-footer-location")
			.forEach((el) => {
	...
	detached.forEach((tag) => {
		...
		if (panel) {
			...
			if (body) {
				...
				const capturedBars = captureBarChangeAnimationState(
					body,
					xpAnimationContext,
				);
				if (tag === "QUESTS") {
					body.innerHTML = renderQuestLog(
						getDisplayQuests(displayMemo),
						currentTime,
						collapsed,
						detached,
...
async function applyDungeonMapForHistoryView() {
	...
	if (runtimeState.historyViewIndex === -1) {
		...
		if (liveMap) {
			try {
				await restoreActiveDungeonMapHistory(liveMap);
			} catch (error) {
				console.warn(
					"[RPG Tracker] Could not restore live dungeon map occupancy:",
					error,
				);
			}
		}
	...
	if (!runtimeState.liveDungeonMapBackup) {
		try {
			runtimeState.liveDungeonMapBackup =
				await captureActiveDungeonMapHistory();
		} catch (error) {
			console.warn(
				"[RPG Tracker] Could not snapshot live dungeon map occupancy:",
				error,
			);
		}
	}
	...
	if (overlay) {
		try {
			await restoreActiveDungeonMapHistory(overlay);
		} catch (error) {
			console.warn(
				"[RPG Tracker] Could not roll dungeon map occupancy back to this snapshot:",
				error,
			);
		}
	}
...
function applyDayNightCycleUI(settings, memoText) {
	...
	if (daynightSlot) {
		if (cycleActive) {
			daynightSlot.innerHTML = currentTimeStr
				? renderDayNightBadge(currentTimeStr)
				: "";
		} else {
...
function refreshDayNightCycleFromCurrentMemo() {
	...
	const ta = /** @type {HTMLTextAreaElement|null} */ (
		document.getElementById("rpg-tracker-memo")
	);
...
export function syncMemoView() {
	...
	const btnBack = /** @type {HTMLButtonElement|null} */ (
		document.getElementById("rpg-tracker-nav-back")
	);
	const btnFwd = /** @type {HTMLButtonElement|null} */ (
		document.getElementById("rpg-tracker-nav-fwd")
	);
	...
	if (runtimeState.historyViewIndex === -1) {
		...
		runtimeState.dungeonMapHistoryOverlay = getDungeonMapHistoryEntry(
			s,
			runtimeState.historyViewIndex,
		);
	}
...
function updateUIMemo(text) {
	...
	const counter = document.getElementById("rpg-tracker-count");
	if (counter)
		counter.textContent = `~${Math.round(text.length / 2.62)} tokens`;
}

function updateAgentStatusIndicator(running) {
	const stopBtn = /** @type {HTMLElement} */ (
		document.getElementById("rt-agent-stop-btn")
	);
	const playBtn = /** @type {HTMLElement} */ (
		document.getElementById("rt-agent-router-manual-run")
	);
...
export function updateStatusIndicator(state) {
	const indicator = document.getElementById("rpg-tracker-status");
	const stopBtn = /** @type {HTMLElement} */ (
		document.getElementById("rpg-tracker-stop-btn")
	);
	if (!indicator) return;
...
const RENDER_HINTS = {
	CHARACTER: {
		...
		description:
			'Each entity is one row with an HP bar. First line: "Name (Race/Class): cur/max HP". Sub-lines: Combat (BAB), Gear, Attr, Saves, Skills, Traits, Abilities, HD, Status.',
		example:
			"Korgath Iron-Hide (Dwarven Warrior): 32/32 HP\nCombat: BAB: +2 | Ranged (1 attack): +3 | Melee (1 attack): +5\nGear: Volcanic Mace (+1 / 2d6+3), AC: 13 (Furs)\nAttr: STR 16 (+3), DEX 12 (+1), CON 16 (+3), INT 8 (-1), WIS 16 (+3), CHA 6 (-2)\nSaves: Fort +6 | Ref +1 | Will +1\nSkills: Athletics +5, Intimidation +4\nTraits: Darkvision (60 ft)\nAbilities: Second Wind (1/1), Action Surge (1/1)\nHD: d10 (2/2)\nStatus: Healthy",
	},
	COMBAT: {
		label: "Entity Rows — HP Bars (Enemies)",
		description:
			"Entity rows with COMBAT ROUND header. Martial: weapon Att/def. Caster: Spell Atk + Spell DC + backup weapon, then Cantrips/Level N Spells lines (same rendering as PARTY).",
		example:
	...
	SPELLS: {
		...
		description:
			'One line per spell level. Cantrips: comma-separated names. Slots: "Level N (available/max): Spell1, Spell2".',
		example:
...
export async function fetchBaseSyspromptRaw(settingsOverride = null) {
	...
	const fileName = s.diceFunctionTool
		? "sysprompt.txt"
		: "sysprompt_legacy.txt";
	let content;
	try {
		const response = await fetch(
			`/scripts/extensions/third-party/${FOLDER_NAME}/${fileName}`,
		);
		if (response.ok) {
		...
		console.warn(
			`[Multihog Framework] fetchBaseSyspromptRaw: could not fetch ${fileName}, using fallback:`,
			err,
		);
...
function updateMainSyspromptBackupStatusUi(settings = getSettings()) {
	...
	const controlsEl = document.getElementById(
		"rpg_main_sysprompt_backup_controls",
	);
	const enabledCb = /** @type {HTMLInputElement|null} */ (
		document.getElementById("rpg_main_sysprompt_backup_enabled")
	);
	...
	if (!backupOn) {
		statusEl.textContent =
			"Backup is disabled — enable above to save or restore.";
		return;
	}
	...
	if (!backupLen) {
		statusEl.textContent =
			"No backup saved yet. It is created automatically before the framework overwrites Main, and kept in this browser even if the tracker or extension is turned off.";
		return;
	}
	...
	if (s.enabled && isMainSyspromptBackupEnabled(s)) {
		...
		if (result.shouldDefer) {
			...
			if (!getEffectiveBackupText(s).trim()) {
				console.warn(
					"[RPG Tracker] Refusing to overwrite Quick Prompt Main: backup source is not ready and no durable backup exists.",
				);
				return;
			}
		}
		if (!result.ok && !getEffectiveBackupText(s).trim()) {
			console.warn(
				"[RPG Tracker] Refusing to overwrite Quick Prompt Main until a user backup can be saved.",
			);
			return;
		}
	}
...
export function buildSysprompt(rawText) {
	...
	const baseSectionMap = new Map(
		baseSections.map((sec) => [sec.tag, sec.content]),
	);
 ...
 * @returns {boolean}
 */
function tryBindConnectionProfileDropdown(
	selector,
	initialProfileId,
	onProfileIdChange,
) {
	if (!$(selector).length) {
		console.warn(
			`[RPG Tracker] Connection profile dropdown not found: ${selector}`,
		);
		return false;
	}
	const ctx = SillyTavern.getContext();
	...
	try {
		svc.handleDropdown(selector, initialProfileId || "", (profile) =>
			onProfileIdChange(profile?.id || ""),
		);
		return true;
	} catch (e) {
		console.warn(
			`[RPG Tracker] Could not bind connection profile dropdown ${selector}:`,
			e,
		);
		return false;
	}
...
async function bindFeatureConnectionSettings(options) {
	...
	source
		.val(settings[key("ConnectionSource")] || "default")
		.on("change", function () {
	...
	ollamaUrl
		.val(settings[key("OllamaUrl")] || "http://localhost:11434")
		.on("input", function () {
	...
	if (savedOllamaModel) {
		ollamaModel.append(
			$("<option></option>").val(savedOllamaModel).text(savedOllamaModel),
		);
	}
	...
	$(`#${uiPrefix}_ollama_refresh`).on("click", async function () {
		...
		try {
			...
			models.forEach((model) => {
				ollamaModel.append(
					$("<option></option>").val(model.name).text(model.name),
				);
			});
	...
	openaiModel.on("change", function () {
		...
		settings[key("OpenaiModel")] =
			value || String(openaiManual.val() || "").trim();
		saveSettings();
	});
	...
	$(`#${uiPrefix}_openai_refresh`).on("click", async function () {
		...
		try {
			...
			toastr["warning"](
				"Cannot auto-detect models. Type the model name manually.",
			);
		}
	});
	...
	if (
		!tryBindConnectionProfileDropdown(
			profileSelector,
			settings[profileKey] || "",
			(id) => {
		...
		try {
			...
			console.warn(
				`[RPG Tracker] Could not load ${keyPrefix} connection profiles:`,
				error,
			);
		}
		profileSelect
			.empty()
			.append('<option value="">-- No Profile Selected --</option>');
		profiles.forEach((profile) =>
			profileSelect.append($("<option></option>").val(profile).text(profile)),
		);
	...
	presetSelect
		.empty()
		.append('<option value="">-- Use Current Settings --</option>');
	...
	const hasSavedPreset = presetSelect
		.find("option")
		.toArray()
		.some((option) => option.value === savedPreset);
	if (savedPreset && !hasSavedPreset) {
		presetSelect.append(
			$("<option></option>").val(savedPreset).text(savedPreset),
		);
	}
...
async function runPortraitMigrationIfNeeded() {
	...
	try {
		...
		if (remaining === 0) {
			...
			console.warn(
				`[RPG Tracker] Portrait migration finished but ${remaining} embedded portrait(s) remain.`,
			);
			if (stats.failed > 0) {
				toastr["warning"](
		...
		toastr["error"](
			"Portrait migration failed — see browser console.",
			"RPG Tracker",
		);
	} finally {
...
const CONNECTION_SETTINGS_UI = [
	{
		key: "state_tracker",
		control: "#rpg_tracker_connection_source",
		slot: "#rpg_connection_slot_state_tracker",
		label: "State Tracker",
		recommendation:
			"I recommend a cheap mid-tier model such as GPT-5.6 Luna, Gemini Flash/Flash-Lite series, or Deepseek V4 Flash latest.",
	},
	{
		key: "combat_override",
		control: "#rpg_combat_api_override",
		slot: "#rpg_connection_slot_combat_override",
		label: "Combat API Override",
	},
	{
		key: "lorebook_agent",
		control: "#rpg_tracker_router_source",
		slot: "#rpg_connection_slot_lorebook_agent",
		label: "Lorebook Agent",
		recommendation: "Same models work fine here as with the State Tracker.",
	},
	{
		key: "adventure_companion",
		control: "#rpg_adventure_companion_connection_source",
		slot: "#rpg_connection_slot_adventure_companion",
		label: "Adventure Companion",
	},
	{
		key: "game_system_wizard",
		control: "#rpg_gs_wizard_connection_source",
		slot: "#rpg_connection_slot_game_system_wizard",
		label: "Game System Wizard",
		recommendation:
			"I recommend using a somewhat better model here such as Sonnet 5 or above for more robust and complex systems. Your mileage varies a lot here. Experiment.",
	},
	{
		key: "map_architect",
		control: "#rpg_map_architect_connection_source",
		slot: "#rpg_connection_slot_map_architect",
		label: "Map Architect",
		recommendation:
			"A capable reasoning model is recommended for coherent topology, hidden information, and entity placement. Map Architect builds the foundation map; give it a stronger model than occupancy and evolution.",
	},
	{
		key: "map_runtime",
		control: "#rpg_map_runtime_connection_source",
		slot: "#rpg_connection_slot_map_runtime",
		label: "Map Updater",
		recommendation:
			"Occupancy can use a cheaper model than Map Architect. JSON discipline still helps.",
	},
	{
		key: "map_evolution",
		control: "#rpg_map_evolution_connection_source",
		slot: "#rpg_connection_slot_map_evolution",
		label: "Map Evolution",
	},
	{
		key: "world_progression",
		control: "#rpg_world_connection_source",
		slot: "#rpg_connection_slot_world_progression",
		label: "World Progression",
	},
	{
		key: "portraits",
		control: "#rpg_portrait_connection_source",
		slot: "#rpg_connection_slot_portraits",
		label: "Portrait Generation",
		recommendation: "A lightweight model should do fine.",
	},
];

function normalizeCentralConnectionDrawer(
	drawer,
	key,
	label,
	recommendation = "",
	...
	drawerHeader?.classList.add(
		"rt-centered-drawer-header",
		"rt-central-connection-header",
	);
	drawerHeader
		?.querySelectorAll(":scope > i:not(.inline-drawer-icon)")
	...
	if (chevron) {
		chevron.className =
			"inline-drawer-icon fa-solid fa-circle-chevron-down rt-central-connection-chevron";
	...
	if (content instanceof HTMLElement) {
		...
		if (
			recommendation &&
			!content.querySelector(":scope > .rt-connection-recommendation")
		) {
...
function setSettingsDrawerOpen(drawer) {
	...
	drawer
		.querySelector(":scope > .inline-drawer-toggle .inline-drawer-icon")
		?.classList.add("down");
}
...
function openConnectionsModelsSettings(targetKey = "") {
	...
	const connectionsDrawer = document.getElementById(
		"rpg_connections_models_drawer",
	);
	setSettingsDrawerOpen(connectionsDrawer);

	const targetDrawer =
		targetKey === "character_creation"
			? document.getElementById("rpg_character_creation_connection_drawer")
			: document.querySelector(
					`#rt-settings-overlay .rt-central-connection-drawer[data-connection-key="${targetKey}"]`,
				);
	setSettingsDrawerOpen(targetDrawer);
	setTimeout(
		() =>
			(targetDrawer || connectionsDrawer)?.scrollIntoView?.({
				behavior: "smooth",
				block: "center",
			}),
		80,
...
function organizeConnectionSettingsUI() {
	const settingsRoot =
		getSettingsOverlayRoot() || document.querySelector(".rpg-tracker-settings");
	...
	for (const definition of CONNECTION_SETTINGS_UI) {
		...
		if (
			...
			!settingsRoot.querySelector(
				`.rt-connection-shortcut[data-connection-key="${definition.key}"]`,
			)
		) {
...
(async function init() {
	...
	document.getElementById("rpg-tracker-panel")?.remove();
	document
		.querySelectorAll(".rpg-tracker-settings")
		.forEach((el) => el.remove());
	document
		.querySelectorAll(".rpg-tracker-settings-stub")
		.forEach((el) => el.remove());
	...
	configureRuntimeActions({
		...
		parseMemoBlocks,
		refreshAgentManifest: (...args) =>
			runtimeState.refreshAgentManifest(...args),
		...
		setPillDeselectHandler: (handler) => {
			_pillDeselectHandler = handler;
		},
	});
	...
	try {
		...
		const stubHtml = await renderExtensionTemplateAsync(
			`third-party/${FOLDER_NAME}`,
			"settings-stub",
			cacheBust,
		);
		...
		$(document).on(
			"click.rpgTrackerSettingsDrawers",
			settingsDrawerToggleSelector,
			function (e) {
				// Never steal clicks from controls that live in a header row.
				if (
					$(e.target).closest(
						"input, select, textarea, button, a, label.checkbox_label",
					).length
				) {
					return;
				}
		...
		$(document).on(
			"click.rpgTrackerSettingsDrawers",
			"#rt-settings-overlay .rt-connection-shortcut-button",
			function (e) {
				e.preventDefault();
				e.stopPropagation();
				openConnectionsModelsSettings(
					String($(this).data("connectionTarget") || ""),
				);
			},
		);
		$(document).on(
		...
		bindCharacterCreationConnectionSettings(
			getSettingsOverlayRoot() ||
				document.querySelector(".rpg-tracker-settings"),
		);
		bindAdventureCompanionSettingsDrawer();
		...
		syncMapArchitectOpenerNestedVisibility(
			settings.syspromptModules?.[LOCATION_MAPPING_SECTION_TAG] ?? true,
		);
		$(
			'input[name="rpg_map_architect_opener"], input[name="rpg_map_architect_opener_components"]',
		).on("change", function () {
		...
		$("#rpg_map_architect_lookback")
			.val(settings.mapArchitectLookback ?? 12)
			.on("change", function () {
				settings.mapArchitectLookback = Math.max(
					0,
					Math.min(100, parseInt(String($(this).val()), 10) || 0),
				);
				$(this).val(settings.mapArchitectLookback);
				saveSettings();
			});
		$("#rpg_map_architect_max_tokens")
			.val(settings.mapArchitectMaxTokens ?? 25000)
			.on("change", function () {
				settings.mapArchitectMaxTokens = Math.max(
					1000,
					Math.min(32000, parseInt(String($(this).val()), 10) || 25000),
				);
				$(this).val(settings.mapArchitectMaxTokens);
				saveSettings();
			});
		$("#rpg_map_architect_system_prompt")
			.val(
				settings.mapArchitectSystemPrompt ||
		...
		$("#rpg_map_updater_enabled")
			.prop("checked", settings.mapUpdaterEnabled !== false)
			.on("change", function () {
				settings.mapUpdaterEnabled = !!$(this).prop("checked");
				$("#rt-agent-map-updater-run-every")
					.closest("div")
					.toggle(settings.mapUpdaterEnabled !== false);
				saveSettings();
			});
		$("#rpg_map_updater_run_every")
			.val(settings.mapUpdaterRunEvery ?? 1)
			.on("change", function () {
				settings.mapUpdaterRunEvery = Math.max(
					1,
					Math.min(50, parseInt(String($(this).val()), 10) || 1),
				);
				$(this).val(settings.mapUpdaterRunEvery);
				$("#rt-agent-map-updater-run-every").val(settings.mapUpdaterRunEvery);
				saveSettings();
			});
		$("#rpg_map_updater_max_tokens")
			.val(settings.mapUpdaterMaxTokens ?? 25000)
			.on("change", function () {
				settings.mapUpdaterMaxTokens = Math.max(
					1000,
					Math.min(32000, parseInt(String($(this).val()), 10) || 25000),
				);
		...
		$("#rpg_map_updater_system_prompt")
			.val(settings.mapUpdaterSystemPrompt || DEFAULT_MAP_UPDATER_SYSTEM_PROMPT)
			.on("input", function () {
		...
		$("#rpg_map_evolution_enabled")
			.prop("checked", settings.mapEvolutionEnabled !== false)
			.on("change", function () {
				settings.mapEvolutionEnabled = !!$(this).prop("checked");
				saveSettings();
				if (
					typeof runtimeState.updateAgentMapEvolutionStatusRef === "function"
				) {
					runtimeState.updateAgentMapEvolutionStatusRef();
				}
			});
		$("#rpg_map_evolution_interval_hours")
			.val(settings.mapEvolutionIntervalHours ?? 8)
			.on("change", function () {
				settings.mapEvolutionIntervalHours = Math.max(
					1,
					Math.min(168, parseInt(String($(this).val()), 10) || 8),
				);
		...
		$("#rpg_map_evolution_onsite_interval_hours")
			.val(settings.mapEvolutionOnSiteIntervalHours ?? 1)
			.on("change", function () {
				const parsed = parseInt(String($(this).val()), 10);
				settings.mapEvolutionOnSiteIntervalHours = Math.max(
					0,
					Math.min(168, Number.isFinite(parsed) ? parsed : 1),
				);
				$(this).val(settings.mapEvolutionOnSiteIntervalHours);
				saveSettings();
				updateMapEvolutionScheduleDisplay();
				$("#rt-agent-map-evo-onsite-interval").val(
					settings.mapEvolutionOnSiteIntervalHours,
				);
				void refreshMapEvolutionSelectedList();
			});
		$("#rpg_map_evolution_onsite_interval_minutes")
			.val(settings.mapEvolutionOnSiteIntervalMinutes ?? 0)
			.on("change", function () {
				const parsed = parseInt(String($(this).val()), 10);
				settings.mapEvolutionOnSiteIntervalMinutes = Math.max(
					0,
					Math.min(59, Number.isFinite(parsed) ? parsed : 0),
				);
				$(this).val(settings.mapEvolutionOnSiteIntervalMinutes);
				saveSettings();
				updateMapEvolutionScheduleDisplay();
				$("#rt-agent-map-evo-onsite-minutes").val(
					settings.mapEvolutionOnSiteIntervalMinutes,
				);
				void refreshMapEvolutionSelectedList();
			});
		$("#rpg_map_evolution_onsite_preset")
			.val(
				settings.mapEvolutionOnSitePreset === "standard"
					? "standard"
					: "dynamic",
			)
			.on("change", function () {
				settings.mapEvolutionOnSitePreset =
					String($(this).val() || "") === "standard" ? "standard" : "dynamic";
				$(this).val(settings.mapEvolutionOnSitePreset);
				saveSettings();
			});
		$("#rpg_map_evolution_lookback")
			.val(settings.mapEvolutionLookback ?? 20)
			.on("change", function () {
				settings.mapEvolutionLookback = Math.max(
					0,
					Math.min(100, parseInt(String($(this).val()), 10) || 0),
				);
		...
		$("#rpg_map_evolution_max_tokens")
			.val(settings.mapEvolutionMaxTokens ?? 25000)
			.on("change", function () {
				settings.mapEvolutionMaxTokens = Math.max(
					1000,
					Math.min(32000, parseInt(String($(this).val()), 10) || 25000),
				);
		...
		$("#rpg_map_evolution_compress_enabled")
			.prop("checked", settings.mapEvolutionCompressEnabled !== false)
			.on("change", function () {
				settings.mapEvolutionCompressEnabled = !!$(this).prop("checked");
				saveSettings();
			});
		$("#rpg_map_evolution_compress_threshold")
			.val(settings.mapEvolutionCompressThreshold ?? 10000)
			.on("change", function () {
				const n = Math.floor(parseInt(String($(this).val()), 10));
				settings.mapEvolutionCompressThreshold = Number.isFinite(n)
					? Math.max(500, Math.min(100000, n))
					: 10000;
				$(this).val(settings.mapEvolutionCompressThreshold);
				saveSettings();
			});
		$("#rpg_map_evolution_narrator_commit_tokens")
			.val(settings.mapEvolutionNarratorCommitTokens ?? 2000)
			.on("change", function () {
				settings.mapEvolutionNarratorCommitTokens =
					normalizeMapEvolutionNarratorCommitTokens($(this).val());
				$(this).val(settings.mapEvolutionNarratorCommitTokens);
				saveSettings();
			});
		$("#rpg_map_evolution_world_report_lookback")
			.val(settings.mapEvolutionWorldReportLookback ?? 5)
			.on("change", function () {
				settings.mapEvolutionWorldReportLookback = Math.max(
					1,
					Math.min(20, parseInt(String($(this).val()), 10) || 5),
				);
		...
		$("#rpg_map_evolution_tick_count")
			.val(settings.mapEvolutionTickCount ?? 2)
			.on("change", function () {
				const parsed = parseInt(String($(this).val()), 10);
				settings.mapEvolutionTickCount = Math.max(
					0,
					Math.min(50, Number.isFinite(parsed) ? parsed : 2),
				);
				$(this).val(settings.mapEvolutionTickCount);
				saveSettings();
				$("#rt-agent-map-evo-tick-count").val(settings.mapEvolutionTickCount);
			});
		$("#rpg_map_evolution_tick_randomize")
			.prop("checked", settings.mapEvolutionTickRandomize !== false)
			.on("change", function () {
				settings.mapEvolutionTickRandomize = !!$(this).prop("checked");
				saveSettings();
				$("#rt-agent-map-evo-tick-randomize").prop(
					"checked",
					settings.mapEvolutionTickRandomize,
				);
			});
		...
		$("#rpg_map_evolution_selected_all").on("click", function () {
			$('#rpg_map_evolution_selected_list input[type="checkbox"]').prop(
				"checked",
				true,
			);
		...
		$("#rpg_map_evolution_selected_current").on("click", async function () {
			...
			$('#rpg_map_evolution_selected_list input[type="checkbox"]').each(
				function () {
					$(this).prop(
						"checked",
						String($(this).attr("data-site-root") || "") === current.siteRoot,
					);
				},
			);
			persistMapEvolutionSelectedRootsFromUi();
		...
		$("#rpg_map_evolution_btn_override_next").on("click", async function () {
			...
			try {
				...
				console.warn(
					"[RPG Tracker] Failed to list mapped sites for Evolution override:",
					error,
				);
			}
			const currentRoot =
				listed.find((site) => site.current)?.siteRoot ||
				s.mapEvolutionLastSiteRoot ||
				"";
			...
			const fallbackHours = Math.max(
				1,
				Number(s.mapEvolutionIntervalHours) || 8,
			);
			const currentNextMins =
				schedule.nextMins >= 0
					? schedule.nextMins
					: currentMemoMinutes() >= 0
						? currentMemoMinutes() + fallbackHours * 60
						: fallbackHours * 60;

			function fmtHint(totalMins) {
				if (totalMins < 0)
					return s.useDdMmYyFormat
						? "01/01/2026, 08:00 AM"
						: s.use24hTime
							? "Day 1, 00:00"
							: "Day 1, 12:00 AM";
			...
			let roots = [
				...Object.keys(s.mapEvolutionLastFiredBySite || {}),
				...listed.map((site) => site.siteRoot),
			];
			if (!roots.length) {
			...
			for (const root of roots) {
				...
				stamps = stampEvolutionLastFired(
					stamps,
					[root],
					formatInWorldTime(parsedNextMins - hours * 60),
				);
			}
			if (!Object.keys(stamps).length) {
				toastr.warning(
					"Every mapped site is set to never auto-tick.",
					"Map Evolution",
				);
				return;
			}
			...
			updateMapEvolutionScheduleDisplay();
			toastr["success"](
				`Next interval tick set to ${fmtHint(parsedNextMins)}.`,
				"Map Evolution",
			);
		});
		$("#rpg_map_evolution_reset_timeline").on("click", function () {
			...
			updateMapEvolutionScheduleDisplay();
			toastr["info"](
				"Map Evolution timeline reset. Next interval starts from the current time.",
				"Map Evolution",
			);
		});
		$("#rpg_map_evolution_testing_ground").on("click", async function () {
			const { openMapEvolutionTestingGround } =
				await import("./src/ui/panel/panel-map-evolution-debug.js");
			await openMapEvolutionTestingGround();
		});
		$("#rpg_map_evolution_system_prompt")
			.val(
				settings.mapEvolutionSystemPrompt ||
					DEFAULT_MAP_EVOLUTION_SYSTEM_PROMPT,
			)
			.on("input", function () {
		...
		$("#rpg_map_evolution_compress_prompt")
			.val(
				settings.mapEvolutionCompressSystemPrompt ||
					DEFAULT_MAP_EVOLUTION_COMPRESS_SYSTEM_PROMPT,
			)
			.on("input", function () {
		...
		function syncPromptDefaultsUpgradeButton() {
			const btn = document.getElementById(
				"rpg_tracker_btn_upgrade_changed_prompts",
			);
			if (!btn) return;
		 ...
		 */
		function buildPromptDefaultsDiffSectionHtml(
			oldSnap,
			newSnap,
			liveSettings,
			mainSyspromptText = "",
			...
			const badgeStyles = {
				customized:
					"background:rgba(234,179,8,0.2);color:#fbbf24;border:1px solid rgba(234,179,8,0.35);",
				"matches new":
			...
			for (const cat of PROMPT_DEFAULTS_CATEGORIES) {
				...
				const impact = getPromptCategoryImpactBadge(
					oldSnap,
					newSnap,
					liveSettings,
					cat,
		...
		// --- Version Upgrade Prompt Reset Dialog ---
		{
			...
			try {
				...
				console.warn(
					"[RPG Tracker] Could not fetch manifest.json for version check",
					e,
				);
			}
			...
			const persistPromptDefaultsAck = (target) => {
				...
				stampCriticalSettingsSynced(
					target,
					writeCriticalSettingsBackup(target),
				);
			};
			...
			if (!settings.lastResetVersion) {
				...
				_runPromptDefaultsStartupAction = () =>
				...
				if (settings.autoResetPromptsOnUpdate) {
					...
					_runPromptDefaultsStartupAction = async () => {
						...
						const customSyspromptCb = document.getElementById(
							"rpg_tracker_custom_sysprompt",
						);
						if (customSyspromptCb) {
							customSyspromptCb.checked = false;
							const narratorConfigBlock = document.getElementById(
								"rpg_narrator_config_block",
							);
						...
						sTempTracker.stockPrompts = JSON.parse(
						...
						if (sTempTracker.fullReviewStateMode) {
							if ($corePromptEl.length)
								$corePromptEl.val(FULL_REVIEW_STATE_SYSTEM_PROMPT);
							if ($suffixPromptEl.length)
								$suffixPromptEl.val(FULL_REVIEW_USER_PROMPT_SUFFIX);
						} else {
							if ($corePromptEl.length)
						...
						$("#rpg_tracker_npc_major_words").val(
							sTempTracker.npcMajorWords ?? 225,
						);
						$("#rpg_tracker_npc_minor_words").val(
							sTempTracker.npcMinorWords ?? 135,
						);
						$("#rpg_tracker_npc_rel_max_default").val(
							getNpcRelationshipMaxDefault(sTempTracker),
						);
						$("#rpg_tracker_npc_portraits").prop(
							"checked",
							sTempTracker.npcPortraits !== false,
						);
						syncNpcPortraitDependentUi(sTempTracker);
						$("#rpg_tracker_npc_rel_bars").prop(
							"checked",
							!!sTempTracker.npcRelationshipBars,
						);
						$("#rpg_tracker_animate_all_custom_bars").prop(
							"checked",
							!!sTempTracker.animateAllCustomBarChanges,
						);
						$("#rpg_sysprompt_mod_npc_rel_bars").prop(
							"checked",
							!!sTempTracker.npcRelationshipBars,
						);
						$("#rpg_tracker_npc_card_import").prop(
							"checked",
							!!sTempTracker.experimentalNpcImport,
						);
						$("#rpg_tracker_ignore_npc_limits").prop(
							"checked",
							!!sTempTracker.ignoreNpcImportLimits,
						);
						...
						fresh.npcCoreSections = JSON.parse(
							JSON.stringify(DEFAULT_NPC_SECTIONS),
						);
						fresh.pcCoreSections = JSON.parse(
							JSON.stringify(DEFAULT_PC_SECTIONS),
						);
						...
						for (const [id, def] of Object.entries(DEFAULT_MODULES)) {
							if (fresh.routerModules && fresh.routerModules[id]) {
								if (id === "npc") {
									fresh.routerModules[id].instruction = buildNpcInstruction(
										fresh.npcMajorWords,
										fresh.npcMinorWords,
										false,
										fresh,
						...
						if (extensionSettings[MODULE_NAME]) {
							...
							delete extensionSettings[MODULE_NAME]
								.worldProgressionSkeletonSystemPrompt;
						}
						const $wpPromptEl = $("#rpg_world_progression_system_prompt");
						if ($wpPromptEl.length) {
							$wpPromptEl
								.val(sTemp.worldProgressionSystemPrompt)
								.trigger("input");
						}
						const $wpSkelPromptEl = $(
							"#rpg_world_progression_skeleton_system_prompt",
						);
						if ($wpSkelPromptEl.length) {
							$wpSkelPromptEl
								.val(sTemp.worldProgressionSkeletonSystemPrompt)
								.trigger("input");
						}

						await acknowledgePromptDefaults(fresh);
						toastr["info"](
							`Prompts auto-updated to latest defaults (v${currentVersion}).`,
							"RPG Tracker",
						);
						console.log(
							`[RPG Tracker] Automatically reset all prompts/sections to defaults for version ${currentVersion}.`,
						);
					};
				} else {
					const { Popup } = SillyTavern.getContext();
					if (Popup && Popup.show && Popup.show.confirm) {
						...
						_runPromptDefaultsDialog = async () => {
							...
							const changedCats = getChangedPromptDefaultCategories(
								storedSnapshot,
								currentSnapshot,
							);
							const hasSnapshot = !!storedSnapshot;
							const changedLabels = [...changedCats].map(
								(c) => PROMPT_DEFAULTS_CATEGORY_LABELS[c] || c,
							);
							...
							const allChangedChecked = PROMPT_DEFAULTS_CATEGORIES.every((c) =>
								changedCats.has(c),
							);
							...
							setTimeout(() => {
								...
								const alwaysCb = document.getElementById(
									"rt-reset-always-auto",
								);
								const cbs = [sysCb, trackerCb, loreCb, worldCb, sectionsCb];

								if (sysCb)
									sysCb.addEventListener("change", () => {
										sysReset = sysCb.checked;
									});
								if (trackerCb)
									trackerCb.addEventListener("change", () => {
										trackerReset = trackerCb.checked;
									});
								if (loreCb)
									loreCb.addEventListener("change", () => {
										loreReset = loreCb.checked;
									});
								if (worldCb)
									worldCb.addEventListener("change", () => {
										worldReset = worldCb.checked;
									});
								if (sectionsCb)
									sectionsCb.addEventListener("change", () => {
										sectionsReset = sectionsCb.checked;
									});
								if (alwaysCb)
									alwaysCb.addEventListener("change", () => {
										alwaysAuto = alwaysCb.checked;
									});
							...
							if (alwaysAuto) {
								...
								const stCb = document.getElementById(
									"rpg_tracker_auto_reset_prompts",
								);
							...
							if (confirmResult === POPUP_RESULT.CUSTOM1) {
								...
								if (!saved) {
									toastr["info"](
										"Cartridge save cancelled — prompts left unchanged. You'll be asked again next load.",
										"RPG Tracker",
									);
							...
							const shouldUpgrade =
								confirmResult === POPUP_RESULT.AFFIRMATIVE ||
							...
							if (shouldUpgrade) {
								...
								if (sysReset) {
									...
									if (customSyspromptCb) {
										...
										if (narratorConfigBlock)
											narratorConfigBlock.style.display = "";
									}
									...
									if (!fresh.cyoaConfig) fresh.cyoaConfig = {};
									refreshCyoaConfigToShipped(fresh.cyoaConfig, {
										resetSlots: true,
									});
									await autoApplySysprompt(true);
									resetCount++;
									console.log(
										"[RPG Tracker] Main system prompt reset to defaults.",
									);
								}
								...
								if (trackerReset) {
									...
									resetCount++;
									console.log(
										"[RPG Tracker] State tracker prompts reset to defaults.",
									);
								}
								...
								if (sectionsReset) {
									...
									if (!loreReset && fresh.routerModules?.npc) {
										fresh.routerModules.npc.instruction = buildNpcInstruction(
											fresh.npcMajorWords,
											fresh.npcMinorWords,
											false,
											fresh,
									...
									resetCount++;
									console.log(
										"[RPG Tracker] NPC/PC core sections reset to defaults.",
									);
								}
								...
								if (loreReset) {
									...
									for (const [id, def] of Object.entries(DEFAULT_MODULES)) {
										if (fresh.routerModules && fresh.routerModules[id]) {
											if (id === "npc") {
												fresh.routerModules[id].instruction =
													buildNpcInstruction(
														fresh.npcMajorWords,
														fresh.npcMinorWords,
														false,
									...
									resetCount++;
									console.log(
										"[RPG Tracker] Lorebook Agent prompts reset to defaults.",
									);
								}
								...
								if (worldReset) {
									...
									if ($mapPromptEl.length) {
										$mapPromptEl
											.val(sTemp.mapArchitectSystemPrompt)
											.trigger("input");
									}
									const $mapUpdaterPromptEl = $(
										"#rpg_map_updater_system_prompt",
									);
									if ($mapUpdaterPromptEl.length) {
										$mapUpdaterPromptEl
											.val(sTemp.mapUpdaterSystemPrompt)
											.trigger("input");
									}
									const $mapEvolutionPromptEl = $(
										"#rpg_map_evolution_system_prompt",
									);
									if ($mapEvolutionPromptEl.length) {
										$mapEvolutionPromptEl
											.val(sTemp.mapEvolutionSystemPrompt)
											.trigger("input");
									}
									const $mapEvolutionCompressPromptEl = $(
										"#rpg_map_evolution_compress_prompt",
									);
									if ($mapEvolutionCompressPromptEl.length) {
										$mapEvolutionCompressPromptEl
											.val(sTemp.mapEvolutionCompressSystemPrompt)
											.trigger("input");
									}
									const $wpPromptEl = $("#rpg_world_progression_system_prompt");
									if ($wpPromptEl.length) {
									...
									resetCount++;
									console.log(
										"[RPG Tracker] World and Map Architect prompts reset to defaults.",
									);
								}
								...
								if (resetCount > 0) {
									toastr["success"](
										`Successfully reset ${resetCount} prompt category/categories to defaults.`,
										"RPG Tracker",
									);
								} else {
									toastr["info"](
										"No prompts were selected for reset.",
										"RPG Tracker",
									);
								}
							} else {
								...
								syncPromptDefaultsUpgradeButton();
								toastr["info"](
									"Kept custom — no prompts were changed.",
									"RPG Tracker",
								);
							}
							syncPromptDefaultsUpgradeButton();
		...
		if (!settings.stockPrompts)
			settings.stockPrompts = { ...DEFAULT_STOCK_PROMPTS };
		{
			let changed = false;
			...
			if (settings.stockPrompts.quests?.includes('"updates"')) {
				...
				const migratedPrompt = settings.stockPrompts.quests_legacy?.includes(
					"OBJ_ACTIVE",
				)
		...
		$("#rpg_tracker_debug")
			.prop("checked", settings.debugMode)
			.on("change", function () {
				settings.debugMode = !!$(this).prop("checked");
				saveSettings();
			});

		$("#rpg_main_sysprompt_backup_enabled")
			.prop("checked", isMainSyspromptBackupEnabled(settings))
			.on("change", function () {
				const fresh = getSettings();
		...
		$("#rpg_main_sysprompt_backup_stash").on("click", function () {
			...
			if (!isMainSyspromptBackupEnabled(fresh)) {
				return toastr["warning"](
					"Main prompt backup is disabled. Enable it above first.",
					"RPG Tracker",
				);
			}
			...
			if (
				...
				toastr["success"](
					"Current Quick Prompt Main saved to backup.",
					"RPG Tracker",
				);
			} else if (result.reason === "empty") {
				toastr["warning"](
					"Current Main is empty — refusing to overwrite a saved backup with nothing.",
					"RPG Tracker",
				);
			} else if (result.reason === "already-framework") {
				toastr["warning"](
					"Current Main looks like the framework prompt, so the existing backup was left unchanged.",
					"RPG Tracker",
				);
			} else {
		...
		$("#rpg_tracker_daynight_cycle")
			.prop("checked", !!settings.dayNightCycleEnabled)
			.on("change", function () {
				settings.dayNightCycleEnabled = !!$(this).prop("checked");
				saveSettings();
				const ta = document.getElementById("rpg-tracker-memo");
				refreshDayNightCycleFromMemo(
					ta ? ta.value : settings.currentMemo || "",
				);
				applyPanelBackgroundToDom();
				refreshRenderedView();
			});

		$("#rpg_tracker_xp_bar_bottom")
			.prop("checked", !!settings.xpBarAtBottom)
			.on("change", function () {
		...
		const wirePanelBgControls = (idPrefix, keys, dayToast, nightToast) => {
			const syncUi = () => {
				...
				const dayPreview = /** @type {HTMLElement|null} */ (
					document.getElementById(`${idPrefix}_preview`)
				);
				const nightPreview = /** @type {HTMLElement|null} */ (
					document.getElementById(`${idPrefix}_night_preview`)
				);
				const overlayInp = /** @type {HTMLInputElement|null} */ (
					document.getElementById(`${idPrefix}_overlay`)
				);
				const overlayVal = document.getElementById(`${idPrefix}_overlay_val`);
				const urlInp = /** @type {HTMLInputElement|null} */ (
					document.getElementById(`${idPrefix}_url`)
				);
				if (dayPreview)
					dayPreview.style.backgroundImage = cfg.daySrc
						? `url(${JSON.stringify(cfg.daySrc)})`
						: "none";
				if (nightPreview)
					nightPreview.style.backgroundImage = cfg.nightSrc
						? `url(${JSON.stringify(cfg.nightSrc)})`
						: "none";
				if (overlayInp)
					overlayInp.value = String(Math.round(cfg.strength * 100));
				if (overlayVal)
					overlayVal.textContent = `${Math.round(cfg.strength * 100)}%`;
			...
			const fileInp = /** @type {HTMLInputElement|null} */ (
				document.getElementById(`${idPrefix}_file`)
			);
			...
			const nightUploadBtn = document.getElementById(
				`${idPrefix}_night_upload`,
			);
			const nightFileInp = /** @type {HTMLInputElement|null} */ (
				document.getElementById(`${idPrefix}_night_file`)
			);
			...
			document
				.getElementById(`${idPrefix}_clear`)
			...
			document
				.getElementById(`${idPrefix}_night_clear`)
				?.addEventListener("click", () => {
			...
			overlayInp?.addEventListener("input", () => {
				const val = Math.max(
					0,
					Math.min(100, parseInt(overlayInp.value, 10) || 0),
				);
		...
		$("#rpg_tracker_auto_reset_prompts")
			.prop("checked", !!settings.autoResetPromptsOnUpdate)
			.on("change", function () {
				settings.autoResetPromptsOnUpdate = !!$(this).prop("checked");
				saveSettings();
			});

		$("#rpg_tracker_enable_portraits")
			.prop("checked", settings.enablePortraits !== false)
		...
		$("#rpg_portrait_generator_source")
			.val(settings.portraitGeneratorSource || "native")
			.on("change", function () {
				settings.portraitGeneratorSource = String($(this).val());
				saveSettings();
				$("#rpg_tracker_pollinations_group").toggle(
					settings.portraitGeneratorSource === "pollinations",
				);
				$("#rpg_tracker_horde_group").toggle(
					settings.portraitGeneratorSource === "horde",
				);
			});
		$("#rpg_tracker_pollinations_group").toggle(
			(settings.portraitGeneratorSource || "native") === "pollinations",
		);
		$("#rpg_tracker_horde_group").toggle(
			(settings.portraitGeneratorSource || "native") === "horde",
		);

		$("#rpg_tracker_portrait_skip_prompt")
			.prop("checked", !!settings.portraitSkipPromptDialog)
			.on("change", function () {
				settings.portraitSkipPromptDialog = !!$(this).prop("checked");
				saveSettings();
			});

		$("#rpg_tracker_hide_image_gen_toasts")
			.prop("checked", !!settings.hideImageGenToasts)
			.on("change", function () {
				settings.hideImageGenToasts = !!$(this).prop("checked");
				saveSettings();
			});

		const portraitStoryLookbackRow = $(
			"#rpg_tracker_portrait_story_lookback_row",
		);
		...
		$("#rpg_tracker_portrait_use_story_lookback")
			.prop("checked", !!settings.portraitUseStoryLookback)
			.on("change", function () {
		...
		applyPortraitStoryLookbackUI(!!settings.portraitUseStoryLookback);
		$("#rpg_tracker_portrait_story_lookback")
			.val(settings.portraitStoryLookback ?? 5)
			.on("input", function () {
				settings.portraitStoryLookback = Math.max(
					0,
					Math.min(100, parseInt(String($(this).val() || ""), 10) || 0),
				);
		...
		$("#rpg_tracker_portrait_auto_player")
			.prop("checked", !!settings.portraitAutoGeneratePlayer)
			.on("change", function () {
		...
		$("#rpg_tracker_portrait_auto_party")
			.prop("checked", !!settings.portraitAutoGenerateParty)
			.on("change", function () {
		...
		$("#rpg_tracker_portrait_auto_enemies")
			.prop("checked", !!settings.portraitAutoGenerateEnemies)
			.on("change", function () {
		...
		$("#rpg_tracker_portrait_auto_npcs")
			.prop("checked", !!settings.portraitAutoGenerateNpcs)
			.on("change", function () {
		...
		$("#rpg_tracker_portrait_auto_locations")
			.prop("checked", !!settings.portraitAutoGenerateLocations)
			.on("change", function () {
				if (settings.portraitAutoGenerateSceneView) return;
				applyLocationImageAutoMode(settings, {
					lorebookLocations: !!$(this).prop("checked"),
				});
				saveSettings();
				if (settings.portraitAutoGenerateLocations) {
					forceCheckAutoGenerations(refreshAll);
		...
		$("#rpg_tracker_portrait_auto_scene_view")
			.prop("checked", !!settings.portraitAutoGenerateSceneView)
			.on("change", async function () {
		...
		$("#rpg_tracker_portrait_realtime_trigger")
			.val(settings.portraitRealtimeTriggerMode || "location_change")
			.on("change", function () {
				const mode = String($(this).val() || "location_change");
				settings.portraitRealtimeTriggerMode = [
					"location_enter",
					"location_change",
					"every_n_outputs",
				].includes(mode)
		...
		$("#rpg_tracker_portrait_realtime_every_n")
			.val(Math.max(1, Number(settings.portraitRealtimeEveryNOutputs) || 1))
			.on("change input", function () {
				settings.portraitRealtimeEveryNOutputs = Math.max(
					1,
					Math.floor(Number($(this).val()) || 1),
				);
		...
		$("#rpg_tracker_pollinations_key")
			.val(settings.pollinationsApiKey || "")
			.on("change", function () {
				settings.pollinationsApiKey = String($(this).val()).trim();
				saveSettings();
			});

		$("#rpg_tracker_horde_key")
			.val(settings.hordeApiKey || "")
			.on("change", function () {
				settings.hordeApiKey = String($(this).val()).trim();
				saveSettings();
			});
		$("#rpg_tracker_horde_model")
			.val(settings.hordeModel || "")
			.on("change", function () {
		...
		$("#rpg_tracker_horde_sampler")
			.val(settings.hordeSampler || "er_sde")
			.on("change", function () {
				settings.hordeSampler = String($(this).val());
				saveSettings();
			});
		$("#rpg_tracker_horde_scheduler")
			.val(settings.hordeScheduler ?? "simple")
			.on("change", function () {
				settings.hordeScheduler = String($(this).val());
				saveSettings();
			});
		$("#rpg_tracker_horde_steps")
			.val(settings.hordeSteps ?? 8)
			.on("change", function () {
				settings.hordeSteps = Math.max(
					1,
					Math.min(150, Math.round(Number($(this).val())) || 8),
				);
				$(this).val(settings.hordeSteps);
				saveSettings();
			});
		$("#rpg_tracker_horde_cfg")
			.val(settings.hordeCfgScale ?? 1)
			.on("change", function () {
				settings.hordeCfgScale = Math.max(
					0,
					Math.min(30, Number($(this).val())) || 1,
				);
				$(this).val(settings.hordeCfgScale);
				saveSettings();
			});
		$("#rpg_tracker_horde_width")
			.val(settings.hordeWidth ?? 1024)
			.on("change", function () {
				settings.hordeWidth = Math.max(
					64,
					Math.min(3072, Math.round(Number($(this).val()) / 64) * 64 || 1024),
				);
				$(this).val(settings.hordeWidth);
				saveSettings();
			});
		$("#rpg_tracker_horde_height")
			.val(settings.hordeHeight ?? 1024)
			.on("change", function () {
				settings.hordeHeight = Math.max(
					64,
					Math.min(3072, Math.round(Number($(this).val()) / 64) * 64 || 1024),
				);
		...
		$("#rpg_tracker_inventory_worth_mode")
			.val(settings.inventoryWorthMode || "hover")
			.on("change", function () {
		...
		$("#rpg_tracker_show_total_value")
			.prop("checked", settings.showTotalInventoryValue !== false)
			.on("change", function () {
		...
		$("#rpg_tracker_combat_profile_auto_switch")
			.prop("checked", !!settings.combatProfileAutoSwitch)
			.on("change", async function () {
		...
		if (
			!tryBindConnectionProfileDropdown(
				"#rpg_combat_connection_profile",
				settings.combatConnectionProfileId,
				(id) => {
			...
			getConnectionProfiles().then((profiles) => {
				combatProfileSelect
					...
					.append('<option value="">-- No Profile Selected --</option>');
				profiles.forEach((p) =>
					combatProfileSelect.append($("<option></option>").val(p).text(p)),
				);
		...
		if (pm && typeof pm.getAllPresets === "function") {
			...
			combatPresetSelect
				.empty()
				.append('<option value="">-- Use Profile Preset --</option>');
			presets.forEach((p) =>
				combatPresetSelect.append($("<option></option>").val(p).text(p)),
			);
			...
			if (settings.combatCompletionPresetId) {
				combatPresetSelect.append(
					$("<option></option>")
						.val(settings.combatCompletionPresetId)
						.text(settings.combatCompletionPresetId),
				);
				combatPresetSelect.val(settings.combatCompletionPresetId);
			}
		}
		...
		$("#rpg_tracker_legacy_dice")
			.prop("checked", settings.legacyDiceNaming)
			.on("change", function () {
		...
		$("#rpg_tracker_dice_d100_mode")
			.prop("checked", !!settings.diceD100Mode)
			.on("change", function () {
				...
				scheduleAutoApply();
				toastr["info"](
					settings.diceD100Mode
						? "🎲 d100 Mode enabled."
						: "🎲 d100 Mode disabled — reverted to d20.",
					"RPG Tracker",
				);
			});

		$("#rpg_rng_tool_d20")
			.prop("checked", !!settings.rngToolD20)
			.on("change", function () {
		...
		$("#rpg_rng_tool_d100")
			.prop("checked", !!settings.rngToolD100)
			.on("change", function () {
		...
		$("#rpg_rng_queue_d20")
			.prop("checked", !!settings.rngQueueD20)
			.on("change", function () {
		...
		$("#rpg_rng_queue_d100")
			.prop("checked", !!settings.rngQueueD100)
			.on("change", function () {
		...
		$("#rpg_tracker_dice_function_tool")
			.prop("checked", settings.diceFunctionTool)
			.on("change", function () {
		...
		$("#rpg_tracker_chat_link_enabled")
			.prop("checked", !!settings.chatLinkEnabled)
			.on("change", async function () {
		...
		$("#rpg_tracker_chat_setup_link_enabled")
			.prop("checked", !!settings.chatSetupLinkEnabled)
			.on("change", function () {
				const enabled = !!$(this).prop("checked");
				...
				if (enabled && settings.chatLinkEnabled && runtimeState.currentChatId) {
					saveChatState(runtimeState.currentChatId);
					toastr["success"](
						"Per-item scopes active. Chat-bound setup saved to this chat; Global items remain shared.",
						"RPG Tracker",
					);
				} else if (enabled) {
					toastr["info"](
						"Setup lock is ready. Turn on Chat-Linked Mode to bind the current setup.",
						"RPG Tracker",
					);
				} else {
					toastr["info"](
						"Setup scope bypass on — the current setup will carry between chats without changing saved item scopes.",
						"RPG Tracker",
					);
				}
				saveSettings();
		...
		$("#rpg_tracker_clear_chat_states").on("click", function () {
			...
			if (count === 0)
				return toastr["info"]("No saved chat states to clear.", "RPG Tracker");
			if (
				confirm(
					`Clear ALL ${count} saved chat state(s)?\n\nThis removes the auto-saved tracker data for every chat. Your current live state is unaffected.\n\nProceed?`,
				)
			) {
				s.chatStates = {};
				saveSettings();
		...
		$("#rpg_tracker_create_game_master_card").on("click", async function () {
			...
			const name = resolveNarratorCardName(
				$("#rpg_tracker_game_master_name").val(),
			);
			btn.disabled = true;
			try {
		...
		$("#rpg_tracker_purge_all_portraits").on("click", async function () {
			...
			const fileRefs = [...collectAllPortraitRefs(s)].filter(
				isManagedPortraitPath,
			).length;
			const totalMaps =
				Object.keys(s.customPortraits || {}).length +
				Object.keys(s.customLocationImages || {}).length +
				Object.values(s.chatStates || {}).reduce(
					(n, cs) =>
						n +
						Object.keys(cs.customPortraits || {}).length +
						Object.keys(cs.customLocationImages || {}).length,
					0,
				);
			...
			const msg = [
				...
				embedded > 0
					? `• ${embedded} still embedded in settings (will be removed)`
					: null,
				fileRefs > 0
					? `• ${fileRefs} file(s) under user/images/${PORTRAIT_STORAGE_FOLDER}/`
					: null,
				"",
			...
			try {
				...
				refreshRenderedView();
				toastr["success"](
					"All portraits purged. Restart SillyTavern if the UI still feels sluggish.",
					"RPG Tracker",
				);
			} catch (err) {
		...
		const shouldStripOldCyoaChoices = () => {
			...
			return (
				isEffectiveSectionEnabled("CYOA_mode", fresh) &&
				fresh.cyoaConfig?.stripOldChoicesFromPrompt !== false
			);
		};
		if (event_types.CHAT_COMPLETION_PROMPT_READY) {
			eventSource.on(event_types.CHAT_COMPLETION_PROMPT_READY, (eventData) => {
				if (!shouldStripOldCyoaChoices() || !Array.isArray(eventData?.chat))
					return;
				replacePromptArray(
					eventData.chat,
					stripSupersededChoicesFromChatPrompt(eventData.chat),
				);
			});
		}
		if (event_types.GENERATE_BEFORE_COMBINE_PROMPTS) {
			eventSource.on(
				...
				(eventData) => {
					if (
						!shouldStripOldCyoaChoices() ||
						!Array.isArray(eventData?.finalMesSend)
					)
						return;
					replacePromptArray(
		...
		if (event_types.MESSAGE_SENT)
			eventSource.on(event_types.MESSAGE_SENT, onMapUpdaterUserMessage);
		...
		const catchUpMapArchitectFence = () => {
			setTimeout(() => {
				void onMapArchitectAssistantMessage(undefined, "normal");
			}, 0);
		};
		if (event_types.CHAT_CHANGED)
			eventSource.on(event_types.CHAT_CHANGED, catchUpMapArchitectFence);
		if (event_types.CHAT_LOADED)
			eventSource.on(event_types.CHAT_LOADED, catchUpMapArchitectFence);
		if (event_types.MESSAGE_EDITED)
			eventSource.on(event_types.MESSAGE_EDITED, handleRelationshipSwipeChange);
		if (event_types.MESSAGE_SWIPED)
			eventSource.on(event_types.MESSAGE_SWIPED, handleRelationshipSwipeChange);
		...
		const pendingSettingsBackup = LEGACY_LOCAL_RECOVERY_ENABLED
			? getPendingModuleSchemaBackup()
			: null;
		const healedFromBackup =
			pendingSettingsBackup &&
			(await confirmLocalSettingsRecovery(pendingSettingsBackup))
		...
		if (
			...
			console.log(
				"[RPG Tracker] Healed module schema before chat-state load.",
				{
					strippedTombstones,
					healedFromBackup,
					strippedGlobalUi,
		...
		if (healedFromBackup) {
			toastr["info"](
				"Restored the browser-local tracker configuration you selected.",
				"RPG Tracker",
				{ timeOut: 6000 },
			);
		}
		...
		if (bootChatId && settings.chatLinkEnabled) {
			...
			const preserveLiveBootState = shouldPreserveLiveChatStateOnBoot(
				settings,
				bootChatId,
			);
			const restoredBootChat = preserveLiveBootState
				? false
				: loadChatState(bootChatId);
			if (
				preserveLiveBootState ||
				(!restoredBootChat && !settings.chatStates?.[bootChatId])
			) {
				saveChatState(bootChatId, { skipDiskWrite: true });
				console.warn(
					"[RPG Tracker] Preserved live tracker state for an unsafe boot partition:",
					bootChatId,
				);
			}
		...
		if (LEGACY_LOCAL_RECOVERY_ENABLED && bootChatId) {
			...
			setTimeout(() => {
				if (!memoRecovery?.isBootCheckDone()) {
					console.warn(
						"[RPG Tracker] Memo recovery gate opened after timeout (no chat yet)",
					);
		...
		if (event_types.APP_READY)
			eventSource.once(event_types.APP_READY, markCoreSettingsReady);
		_attemptStartupPersistenceRelease();
		...
		const snapshotPendingStateForNavigation = (reason = "navigation") => {
			...
			try {
				...
				if (s.debugMode)
					console.log(
						`[RPG Tracker] Local navigation snapshot captured (${reason}); settings.json write skipped.`,
					);
			} catch (err) {
				...
				setTimeout(() => {
					_navigationSnapshotInFlight = false;
				}, 0);
			}
		};
		...
		window.addEventListener("pagehide", () =>
			snapshotPendingStateForNavigation("pagehide"),
		);
		document.addEventListener("visibilitychange", () => {
			if (document.visibilityState === "hidden")
				snapshotPendingStateForNavigation("visibilityhidden");
		});
			...
			if (
				...
				(async () => {
					...
					if (settings.worldProgressionEnabled) {
						await ctx
							.executeSlashCommandsWithOptions(
								`/world state=on silent=true "${worldBookName}"`,
							)
							.catch(() => {});
					} else {
						await ctx
							.executeSlashCommandsWithOptions(
								`/world state=off silent=true "${worldBookName}"`,
							)
							.catch(() => {});
					}
				})();
				...
				_bootstrapSyncPromise = syncCampaignPrefixAndWorldsForChat(
					bootChatId,
					"BOOTSTRAP",
				).finally(() => {
		...
		if (isLorebookAgentRuntimeActive(s)) {
			scheduleDeferred(() => {
				disableManagedEntries().catch((e) =>
					console.warn(
						"[RPG Tracker] disableManagedEntries on init failed:",
						e,
					),
				);
			});
		...
		import("./quests.js")
			...
			.catch((e) =>
				console.error("[RPG Tracker] quests.js failed to load:", e),
			);
		...
		// Ollama
		$("#rpg_tracker_ollama_url")
			.val(settings.ollamaUrl)
			.on("input", function () {
		...
		async function refreshOllamaModelsList() {
			...
			try {
				...
				ollamaModelSelect
					...
					.append('<option value="">-- Select Model --</option>');
				models.forEach((m) => {
					ollamaModelSelect.append(
						$("<option></option>").val(m.name).text(m.name),
					);
				});
		...
		// OpenAI
		$("#rpg_tracker_openai_url")
			.val(settings.openaiUrl)
			.on("input", function () {
				settings.openaiUrl = $(this).val();
				saveSettings();
			});
		$("#rpg_tracker_openai_key")
			.val(settings.openaiKey)
			.on("input", function () {
		...
		openaiModelSelect.on("change", function () {
			...
			if (val) {
				...
				settings.openaiModel =
					String(openaiModelManual.val() || "").trim() || "";
			}
			saveSettings();
		});
		...
		async function refreshOpenAIModelsList() {
			...
			try {
				...
				models.forEach((m) => {
					...
					if (id)
						openaiModelSelect.append($("<option></option>").val(id).text(id));
				});
				// Restore saved selection
				const saved = settings.openaiModel;
				if (
					saved &&
					openaiModelSelect.find(`option[value="${saved}"]`).length
				) {
		...
		portraitSourceSelect
			.val(settings.portraitConnectionSource || "default")
			.on("change", function () {
		...
		// Ollama URL / Model
		$("#rpg_portrait_ollama_url")
			.val(settings.portraitOllamaUrl || "http://localhost:11434")
			.on("input", function () {
				settings.portraitOllamaUrl = $(this).val();
				saveSettings();
			});
		const portraitOllamaModelSelect = $("#rpg_portrait_ollama_model");
		portraitOllamaModelSelect
			.val(settings.portraitOllamaModel)
			.on("change", function () {
		...
		$("#rpg_portrait_ollama_refresh").on("click", async function () {
			...
			try {
				...
				portraitOllamaModelSelect
					...
					.append('<option value="">-- Select Model --</option>');
				models.forEach((m) => {
					portraitOllamaModelSelect.append(
						$("<option></option>").val(m.name).text(m.name),
					);
				});
		...
		$("#rpg_portrait_openai_url")
			.val(settings.portraitOpenaiUrl)
			.on("input", function () {
				settings.portraitOpenaiUrl = $(this).val();
				saveSettings();
			});
		$("#rpg_portrait_openai_key")
			.val(settings.portraitOpenaiKey)
			.on("input", function () {
		...
		portraitOpenaiModelSelect.on("change", function () {
			...
			if (val) {
				...
				settings.portraitOpenaiModel =
					String(portraitOpenaiModelManual.val() || "").trim() || "";
			}
			saveSettings();
		});
		portraitOpenaiModelManual.on("input", function () {
			...
			settings.portraitOpenaiModel =
				manual || String(portraitOpenaiModelSelect.val() || "") || "";
			saveSettings();
		});
		$("#rpg_portrait_openai_refresh").on("click", async function () {
			...
			try {
				...
				const models = await fetchOpenAIModels(url, key);
				portraitOpenaiModelSelect
					.empty()
					.append('<option value="">-- Select Model --</option>');
				models.forEach((m) => {
					const id = typeof m === "string" ? m : m.id || m.name;
					if (id)
						portraitOpenaiModelSelect.append(
		...
		if (
			!tryBindConnectionProfileDropdown(
				"#rpg_portrait_connection_profile",
				settings.portraitConnectionProfileId,
				(id) => {
			...
			getConnectionProfiles().then((profiles) => {
				portraitProfileSelect
					.empty()
					.append('<option value="">-- No Profile Selected --</option>');
				profiles.forEach((p) =>
					portraitProfileSelect.append($("<option></option>").val(p).text(p)),
				);
		...
		if (pm && typeof pm.getAllPresets === "function") {
			const presets = pm.getAllPresets();
			portraitPresetSelect
				.empty()
				.append('<option value="">-- Use Current Settings --</option>');
			presets.forEach((p) =>
				portraitPresetSelect.append($("<option></option>").val(p).text(p)),
			);
		...
		$("#rpg_portrait_prompt_word_target")
			.val(settings.portraitPromptWordTarget ?? 200)
			.on("input", function () {
				const raw = parseInt(String($(this).val() || ""), 10);
				settings.portraitPromptWordTarget = isNaN(raw)
					? 200
					: Math.max(1, Math.min(2000, raw));
				saveSettings();
			});

		$("#rpg_portrait_npc_system_prompt")
			.val(settings.portraitNpcSystemPrompt)
			.on("input", function () {
		...
		$("#rpg_portrait_character_system_prompt")
			.val(settings.portraitCharacterSystemPrompt)
			.on("input", function () {
		...
		$("#rpg_portrait_location_system_prompt")
			.val(settings.portraitLocationSystemPrompt || "")
			.on("input", function () {
		...
		$("#rpg_portrait_location_include_present_npcs")
			.prop("checked", !!settings.portraitLocationIncludePresentNpcs)
			.on("change", function () {
				const s = getSettings();
		...
		syncPortraitLocationPromptForNpcToggle(
			settings,
			!!settings.portraitLocationIncludePresentNpcs,
		);
		...
		$("#rpg_portrait_npc_btn_reset_prompt").on("click", function () {
			...
			refreshPortraitPromptPresetsList();
			toastr["success"](
				"NPC/PC Portrait Prompt reset to default.",
				"RPG Tracker",
			);
		});

		$("#rpg_portrait_character_btn_reset_prompt").on("click", function () {
			if (!confirm("Reset Character/Party/Combat Portrait Prompt to default?"))
				return;
			...
			refreshPortraitPromptPresetsList();
			toastr["success"](
				"Character/Party/Combat Portrait Prompt reset to default.",
				"RPG Tracker",
			);
		});
		...
		$("#rpg_portrait_location_btn_reset_prompt").on("click", function () {
			...
			const s = getSettings();
			const freshDefault = getDefaultPortraitLocationSystemPrompt(
				!!s.portraitLocationIncludePresentNpcs,
			);
			...
			refreshPortraitPromptPresetsList();
			toastr["success"](
				"Location Scene Prompt reset to default.",
				"RPG Tracker",
			);
		});
		...
		$("#rpg_portrait_prompt_preset_save_btn").on("click", function () {
			const name = prompt(
				"Enter a name for this portrait prompt setup:",
				"My Portrait Prompts",
			);
			if (!name || !name.trim()) return;
			const trimmedName = name.trim();
			if (
				getFactoryPortraitPromptPresetNameSet().has(trimmedName.toLowerCase())
			) {
				toastr["warning"](
					`"${trimmedName}" is a factory art style name. Choose a different name for your saved setup.`,
					"Portrait Prompt Library",
				);
				return;
			}
			if (
				settings.savedPortraitPromptPresets &&
				settings.savedPortraitPromptPresets[trimmedName]
			) {
				if (
					!confirm(`A setup named "${trimmedName}" already exists. Overwrite?`)
				)
					return;
			}
			if (!settings.savedPortraitPromptPresets)
				settings.savedPortraitPromptPresets = {};
			settings.savedPortraitPromptPresets[trimmedName] = {
		...
		worldSourceSelect
			.val(settings.worldConnectionSource || "default")
			.on("change", function () {
		...
		// Ollama URL / Model
		$("#rpg_world_ollama_url")
			.val(settings.worldOllamaUrl || "http://localhost:11434")
			.on("input", function () {
				settings.worldOllamaUrl = $(this).val();
				saveSettings();
			});
		const worldOllamaModelSelect = $("#rpg_world_ollama_model");
		worldOllamaModelSelect
			.val(settings.worldOllamaModel)
			.on("change", function () {
		...
		// OpenAI URL / Key / Model
		$("#rpg_world_openai_url")
			.val(settings.worldOpenaiUrl)
			.on("input", function () {
				settings.worldOpenaiUrl = $(this).val();
				saveSettings();
			});
		$("#rpg_world_openai_key")
			.val(settings.worldOpenaiKey)
			.on("input", function () {
		...
		worldOpenaiModelSelect.on("change", function () {
			...
			if (val) {
				...
				settings.worldOpenaiModel =
					String(worldOpenaiModelManual.val() || "").trim() || "";
			}
			saveSettings();
		});
		worldOpenaiModelManual.on("input", function () {
			...
			settings.worldOpenaiModel =
				manual || String(worldOpenaiModelSelect.val() || "") || "";
			saveSettings();
		});
		$("#rpg_world_openai_refresh").on("click", async function () {
			...
			try {
				toastr["info"]("Fetching models...");
				const models = await fetchOpenAIModels(url, key);
				worldOpenaiModelSelect
					.empty()
					.append('<option value="">-- Select Model --</option>');
				models.forEach((m) => {
					const id = typeof m === "string" ? m : m.id || m.name;
					if (id)
						worldOpenaiModelSelect.append(
							$("<option></option>").val(id).text(id),
						);
				});
		...
		if (
			!tryBindConnectionProfileDropdown(
				"#rpg_world_connection_profile",
				settings.worldConnectionProfileId,
				(id) => {
			...
			getConnectionProfiles().then((profiles) => {
				worldProfileSelect
					...
					.append('<option value="">-- No Profile Selected --</option>');
				profiles.forEach((p) =>
					worldProfileSelect.append($("<option></option>").val(p).text(p)),
				);
		...
		if (pm && typeof pm.getAllPresets === "function") {
			...
			worldPresetSelect
				...
				.append('<option value="">-- Use Current Settings --</option>');
			presets.forEach((p) =>
				worldPresetSelect.append($("<option></option>").val(p).text(p)),
			);
		...
		gsWizardSourceSelect
			.val(settings.gameSystemWizardConnectionSource || "default")
			.on("change", function () {
		...
		$("#rpg_gs_wizard_ollama_url")
			.val(settings.gameSystemWizardOllamaUrl || "http://localhost:11434")
			.on("input", function () {
				settings.gameSystemWizardOllamaUrl = $(this).val();
				saveSettings();
			});
		const gsWizardOllamaModelSelect = $("#rpg_gs_wizard_ollama_model");
		gsWizardOllamaModelSelect
			.val(settings.gameSystemWizardOllamaModel)
			.on("change", function () {
		...
		$("#rpg_gs_wizard_ollama_refresh").on("click", async function () {
			...
			try {
				...
				gsWizardOllamaModelSelect
					...
					.append('<option value="">-- Select Model --</option>');
				models.forEach((m) => {
					gsWizardOllamaModelSelect.append(
						$("<option></option>").val(m.name).text(m.name),
					);
				});
		...
		$("#rpg_gs_wizard_openai_url")
			.val(settings.gameSystemWizardOpenaiUrl)
			.on("input", function () {
				settings.gameSystemWizardOpenaiUrl = $(this).val();
				saveSettings();
			});
		$("#rpg_gs_wizard_openai_key")
			.val(settings.gameSystemWizardOpenaiKey)
			.on("input", function () {
		...
		gsWizardOpenaiModelSelect.on("change", function () {
			...
			if (val) {
				...
				settings.gameSystemWizardOpenaiModel =
					String(gsWizardOpenaiModelManual.val() || "").trim() || "";
			}
			saveSettings();
		});
		gsWizardOpenaiModelManual.on("input", function () {
			...
			settings.gameSystemWizardOpenaiModel =
				manual || String(gsWizardOpenaiModelSelect.val() || "") || "";
			saveSettings();
		});
		$("#rpg_gs_wizard_openai_refresh").on("click", async function () {
			...
			try {
				toastr["info"]("Fetching models...");
				const models = await fetchOpenAIModels(url, key);
				gsWizardOpenaiModelSelect
					.empty()
					.append('<option value="">-- Select Model --</option>');
				models.forEach((m) => {
					const id = typeof m === "string" ? m : m.id || m.name;
					if (id)
						gsWizardOpenaiModelSelect.append(
							$("<option></option>").val(id).text(id),
						);
				});
		...
		if (
			!tryBindConnectionProfileDropdown(
				"#rpg_gs_wizard_connection_profile",
				settings.gameSystemWizardConnectionProfileId,
				(id) => {
			...
			getConnectionProfiles().then((profiles) => {
				gsWizardProfileSelect
					.empty()
					.append('<option value="">-- No Profile Selected --</option>');
				profiles.forEach((p) =>
					gsWizardProfileSelect.append($("<option></option>").val(p).text(p)),
				);
				gsWizardProfileSelect.val(
					settings.gameSystemWizardConnectionProfileId || "",
				);
			});
		...
		if (pm && typeof pm.getAllPresets === "function") {
			const presets = pm.getAllPresets();
			gsWizardPresetSelect
				.empty()
				.append('<option value="">-- Use Current Settings --</option>');
			presets.forEach((p) =>
				gsWizardPresetSelect.append($("<option></option>").val(p).text(p)),
			);
			gsWizardPresetSelect.val(
				settings.gameSystemWizardCompletionPresetId || "",
			);
		}
		...
		if (lookbackInput.length) {
			lookbackInput
				...
				.on("input", function () {
					settings.lookbackMessages =
						parseInt(/** @type {string} */ ($(this).val())) || 2;
					saveSettings();
				});
		}
		...
		if (historyCountInput.length) {
			historyCountInput
				.val(
					settings.trackerHistoryCount !== undefined
						? settings.trackerHistoryCount
						: 1,
				)
				.on("input", function () {
					settings.trackerHistoryCount =
						parseInt(/** @type {string} */ ($(this).val())) || 1;
					saveSettings();
				});
		}
		const fullAuditMaxTokensInput = $("#rpg_tracker_full_audit_max_tokens");
		if (fullAuditMaxTokensInput.length) {
			fullAuditMaxTokensInput
				.val(
					settings.fullAuditMaxTokens !== undefined
						? settings.fullAuditMaxTokens
						: 32000,
				)
				.on("input", function () {
					settings.fullAuditMaxTokens =
						parseInt(/** @type {string} */ ($(this).val())) || 32000;
					saveSettings();
				});
		}
		...
		if (stateRunEveryInput.length) {
			stateRunEveryInput
				.val(
					settings.stateTrackerRunEvery !== undefined
						? settings.stateTrackerRunEvery
						: 1,
				)
				.on("input", function () {
					settings.stateTrackerRunEvery = Math.max(
						1,
						parseInt(/** @type {string} */ ($(this).val())) || 1,
					);
					saveSettings();
		...
		if (stateSwipeRollbackCb.length) {
			stateSwipeRollbackCb
				.prop("checked", settings.stateTrackerSwipeRollback !== false)
				.on("change", function () {
		...
		async function refreshLorebookList() {
			...
			try {
				...
				if (!worldNames.length && stCtx.updateWorldInfoList) {
					if (settings.debugMode)
						console.log(
							"[RPG Tracker] world_names empty — forcing backend refresh…",
						);
				...
				if (!worldNames.length) {
					if (settings.debugMode)
						console.log(
							"[RPG Tracker] world_names still empty — falling back to direct API fetch…",
						);
					try {
						...
						console.warn(
							"[RPG Tracker] Direct world_names fetch failed:",
							fetchErr,
						);
					}
				}
			...
			sortedBooks.forEach((bookName) => {
				...
				$item.find("input").on("change", function () {
					...
					if (!Array.isArray(settings.lorebookFilter))
						settings.lorebookFilter = [];
					if ($(this).prop("checked")) {
						if (!settings.lorebookFilter.includes(book)) {
							settings.lorebookFilter.push(book);
						}
					} else {
						settings.lorebookFilter = settings.lorebookFilter.filter(
							(b) => b !== book,
						);
					}
					saveSettings();
		...
		$("#rpg_tracker_ctx_worldinfo")
			.prop("checked", settings.ctxWorldInfo ?? false)
			.on("change", async function () {
		...
		const layoutModeSeg = document.getElementById(
			"rpg_tracker_layout_mode_seg",
		);
		if (layoutModeSeg) {
		...
		const wizardBlock = document.getElementById(
			"rpg_tracker_theme_wizard_block",
		);
		const showHideWizard = (theme) => {
			if (wizardBlock)
				wizardBlock.style.display =
					theme === "rt-theme-custom" ? "block" : "none";
		};
		showHideWizard(settings.trackerTheme || "rt-theme-native");

		// Theme Wizard buttons
		document
			.getElementById("rpg_tracker_theme_generate")
			?.addEventListener("click", () => {
				openThemeWizard(false);
			});
		document
			.getElementById("rpg_tracker_theme_iterate")
			?.addEventListener("click", () => {
				if (!settings.customTheme) {
					toastr["info"](
						"No custom theme to iterate on. Generating a new one instead.",
						"Theme Wizard",
					);
					openThemeWizard(false);
				} else {
		...
		document
			.getElementById("rpg_tracker_theme_save")
			?.addEventListener("click", () => {
				if (!settings.customTheme) {
					toastr["warning"](
						"No custom theme to save. Generate one first!",
						"Theme Wizard",
					);
					return;
				}
				...
				if (name && name.trim()) {
					...
					if (settings.savedThemes && settings.savedThemes[trimmedName]) {
						if (
							!confirm(
								`A theme named "${trimmedName}" already exists. Overwrite?`,
							)
						)
							return;
					}
					if (!settings.savedThemes) settings.savedThemes = {};
					settings.savedThemes[trimmedName] = JSON.parse(
		...
		document
			.getElementById("rpg_tracker_theme_wizard_undo")
			?.addEventListener("click", () => {
		...
		if (agentFontSizeVal.length)
			agentFontSizeVal.text((settings.agentFontSize || 13) + "px");
		...
		if (
			!tryBindConnectionProfileDropdown(
				"#rpg_tracker_connection_profile",
		...
		if (pm && typeof pm.getAllPresets === "function") {
			...
			if (settings.completionPresetId) {
				presetSelect.append(
					$("<option></option>")
						.val(settings.completionPresetId)
						.text(settings.completionPresetId),
				);
				presetSelect.val(settings.completionPresetId);
			}
		}
		...
		$("#rpg_tracker_manage_display_groups").on("click", () =>
			openDisplayGroupsManager(),
		);
		...
		$("#rpg_tracker_add_custom_field").on("click", function () {
			...
			let counter = 1;
			const isTagTaken = (tag) =>
				BLOCK_ORDER.includes(tag) ||
				settings.customFields.some((f) => f.tag.toUpperCase() === tag);
		...
		$("#rpg_tracker_add_custom_field_ai").on("click", async function () {
			...
			const inputResult = await Popup.show.confirm(
				"Describe Your Custom Field",
				inputContent,
			...
			if (!description) {
				toastr["warning"](
					"Please describe what you want to track.",
					"AI Field Creator",
				);
				return;
			}

			const existingTags = BLOCK_ORDER.concat(
				(settings.customFields || []).map((f) => f.tag.toUpperCase()),
			);

			let existingFieldsContext = "";
			BLOCK_ORDER.forEach((tag) => {
				if (tag === "QUESTS" && settings.syspromptModules?.quests === false)
					return;
				if (!settings.modules || settings.modules[tag] !== false) {
					const modLower =
						tag === "TIME" ? resolveTimePromptKey(settings) : tag.toLowerCase();
					const promptContent =
						settings.stockPrompts && settings.stockPrompts[modLower]
...
- Return ONLY the JSON. No explanation, no markdown fences.`;
			...
			toastr["info"]("Generating custom field with AI...", "AI Field Creator", {
				timeOut: 3000,
			});
			try {
				const result = await sendStateRequest(
					settings,
					"You are a JSON configuration generator. Return ONLY valid JSON.",
					aiPrompt,
				);
				...
				if (existingTags.includes(normalTag)) {
					parsed.tag =
						normalTag + "_" + Date.now().toString(36).slice(-3).toUpperCase();
				} else {
				...
				if (!approved) {
					toastr["info"](
						"Custom field creation cancelled.",
						"AI Field Creator",
					);
					return;
				}
				...
				refreshOrderList();
				toastr["success"](
					`Custom field "${parsed.label}" created!`,
					"AI Field Creator",
				);
			} catch (err) {
				console.error("[RPG Tracker] AI Field Creator error:", err);
				toastr["error"](
					`Failed to create field: ${err.message}`,
					"AI Field Creator",
				);
			}
		});
		...
		$("#rpg_tracker_import_modules").on("click", async () => {
			...
			setTimeout(() => {
				...
				const textarea = /** @type {HTMLTextAreaElement} */ (
					document.getElementById("rt_import_blob")
				);
			...
			const result = await Popup.show.confirm(
				...
				content,
				{ okButton: "Import", cancelButton: "Cancel" },
			);
		...
		$("#rpg_tracker_delete_all_custom_modules").on("click", function () {
			const s = getSettings();
			if (!s.customFields || s.customFields.length === 0)
				return toastr["info"]("No custom modules to delete.", "RPG Tracker");

			if (
				confirm(
					`Delete ALL (${s.customFields.length}) custom modules?\n\nThis will also remove their data from the current tracker state. Stock modules (COMBAT, CHARACTER, etc.) will not be touched.\n\nProceed?`,
				)
			) {
				const customTags = new Set(
					s.customFields.map((f) => f.tag.toUpperCase()),
				);
				...
				// Clean block order
				if (s.blockOrder) {
					s.blockOrder = s.blockOrder.filter(
						(tag) => !customTags.has(tag.toUpperCase()),
					);
				}
		...
		$("#rt_btn_tag_library").on("click", async function () {
			...
			const themeClass = panel
				? Array.from(panel.classList).find((c) => c.startsWith("rt-theme-")) ||
					"rt-theme-native"
				: "rt-theme-native";
			...
			for (let i = 0; i < RENDERING_TAGS_LIBRARY.length; i++) {
				...
				const rendered =
					tryRenderMarker(item, "TAGLIB", "", i) || `<i>(Failed to render)</i>`;
			...
			await Popup.show.confirm("🎨 Rendering Tags Library", html, {
				okButton: "Close",
				cancelButton: false,
			});
		});
		...
		const applyFullReviewUI = (enabled) => {
			...
			corePromptTextarea
				...
				.css("opacity", enabled ? "0.4" : "1");
			suffixPromptTextarea
				.prop("disabled", enabled)
				.css("opacity", enabled ? "0.4" : "1");
			resetPromptBtn
				.prop("disabled", enabled)
		...
		$("#rpg_tracker_btn_reset_prompt").on("click", function () {
			if (
				!confirm(
					"Reset the State Model prompt and user prompt suffix to the built-in defaults?",
				)
			)
				return;
			...
			saveSettings();
			toastr["success"](
				"Core prompt and user prompt suffix reset to defaults.",
				"RPG Tracker",
			);
		});

		$("#rpg_tracker_btn_update_sysprompt_general").on(
			"click",
			async function () {
				const fileName = getSettings().diceFunctionTool
					? "sysprompt.txt"
					: "sysprompt_legacy.txt";
				let content;
				try {
					...
					console.warn(
						`[Multihog Framework] Could not fetch ${fileName}, using hardcoded fallback:`,
						err,
					);
					content = RT_PROMPTS[fileName];
				}
				...
				if (!content) {
					toastr["error"](
						`Could not load ${fileName}. Main prompt was NOT updated.`,
						"RPG Tracker",
					);
					return;
				}
				...
				const mainTextarea = /** @type {HTMLTextAreaElement} */ (
				...
				if (mainTextarea) {
					...
					toastr["success"](
						`Main sysprompt updated (${getSettings().diceFunctionTool ? "Normal" : "Legacy"} mode)! ✅`,
						"RPG Tracker",
					);
				} else {
					await navigator.clipboard.writeText(content).catch(() => {});
					toastr["info"](
						"Quick-edit textarea not found. Sysprompt copied to clipboard — paste it manually into your Main prompt.",
						"RPG Tracker",
					);
				}
			},
		...
		$("#rpg_tracker_btn_reset_all_prompts").on("click", function () {
			if (
				!confirm(
					"This will reset the Module Prompts, Active Modules, and Module Order to their factory defaults. Custom modules will be moved to the bottom of the list. Your Core Prompt will not be affected. Proceed?",
				)
			)
				return;
			const { extensionSettings } = SillyTavern.getContext();
			...
			saveSettings();
			toastr["success"](
				"Stock modules, order, and prompts reset to factory defaults.",
				"RPG Tracker",
			);
		});
		...
		$(
			"#rpg_tracker_btn_edit_pc_sections, #rpg_tracker_btn_edit_pc_sections_agent",
		).on("click", function () {
			openPcSectionEditor();
		});
		...
		$("#rpg_tracker_btn_game_system_wizard").on("click", () =>
			openGameSystemWizard(),
		);
		$("#rpg_tracker_btn_manage_game_systems").on("click", () =>
			openManageGameSystems(),
		);
		$("#rpg_tracker_btn_control_room").on("click", () =>
			openSystemPromptControlRoom(),
		);
		...
		$("#rpg_tracker_btn_manage_cartridges").on("click", () =>
			openManageGameCartridges(),
		);
		...
		$("#rpg_tracker_btn_reset_and_apply_sysprompt").on(
			...
			async function () {
				...
				$("#rpg_tracker_core_prompt").val(freshSettings.systemPromptTemplate);
				$("#rpg_tracker_user_prompt_suffix").val(
					freshSettings.userPromptSuffix,
				);
				...
				try {
					...
					if (response.ok) {
						content = await response.text();
						console.log(
							`[Multihog Framework] Loaded ${fileName} from live file for auto-apply.`,
						);
					} else {
						throw new Error(`Server returned ${response.status}`);
					}
				...
				if (!content) {
					toastr["error"](
						"Could not load sysprompt.txt. Reset completed but Main prompt was NOT updated.",
						"RPG Tracker",
					);
					return;
				}
				...
				if (mainTextarea) {
					...
					toastr["success"](
						"All prompts reset & Main sysprompt applied! \u2705",
						"RPG Tracker",
					);
				} else {
					...
					try {
						...
						toastr["warning"](
							'All prompts reset. Quick Prompt "Main" textarea not found. Sysprompt copied to clipboard — paste it manually and enable function calls in the completion preset!',
							"RPG Tracker",
						);
					} catch (e) {
						toastr["warning"](
							'All prompts reset. Quick Prompt "Main" textarea not found and clipboard copy failed. Use the SYSPROMPT button to copy manually.',
							"RPG Tracker",
						);
					} finally {
		...
		function readCyoaStyleSettings(cfg) {
			...
			const mechAccentHex = parseCyoaHexColor(
				cfg?.mechAccentColor || cfg?.mechColor,
				"#ffb43c",
			);
			const textColor =
				cfg?.buttonTextColor && /^#[0-9a-f]{6}$/i.test(cfg.buttonTextColor)
					? cfg.buttonTextColor
					: "var(--SmartThemeBodyColor, #e8e8e8)";
			const borderColor =
				cfg?.buttonBorderColor && /^#[0-9a-f]{6}$/i.test(cfg.buttonBorderColor)
			...
			const borderHover =
				cfg?.buttonBorderColor && /^#[0-9a-f]{6}$/i.test(cfg.buttonBorderColor)
					...
					: "rgba(120, 80, 220, 0.7)";
			const choiceAccent =
				cfg?.choiceAccentColor && /^#[0-9a-f]{6}$/i.test(cfg.choiceAccentColor)
		...
		function isCyoaMechanicalBracket(inner) {
			return /\bDC\s*\d+|vs\s*AC|AC\s*\d+|\(\s*[+\-]?\d|GP\b|per\s+(short|long)\s+rest|untrained|timeskip|hour|\d+\s*\/\s*\d+/i.test(
				inner,
			);
		}
		...
		function decorateCyoaMechanicsInButton(btn) {
			if (
				...
				btn.querySelector(".rt-cyoa-mech, .rt-cyoa-tag")
			) {
				return;
			}
			if (
		...
		function isSillyTavernWelcomeMesBlock(el) {
			...
			return (
				type === "welcome_prompt" ||
				type === "welcome" ||
				!!mes.querySelector(".welcomePanel")
			);
		}
		...
		function isSillyTavernChromeButton(btn) {
			if (!(btn instanceof HTMLElement) || btn.tagName !== "BUTTON")
				return false;
			return (
				btn.classList.contains("drawer-opener") ||
		...
		function flattenCyoaChoiceBlocks(root) {
			...
			root.querySelectorAll("choices").forEach((choicesEl) => {
				const buttons = Array.from(choicesEl.querySelectorAll("button")).filter(
					(b) => !isSillyTavernChromeButton(b),
				);
				if (!buttons.length) {
					choicesEl.remove();
					return;
				}
				replaceWithFlatCyoaWrap(
					/** @type {HTMLElement} */ (choicesEl),
					buttons,
				);
			});
			...
			root.querySelectorAll("p, div").forEach((host) => {
				...
				if (
					host.classList?.contains("flex-container") &&
					host.querySelector(
						":scope > button.drawer-opener, :scope > button.menu_button",
					)
				)
					return;
			...
			root.querySelectorAll("div.rt-cyoa-choices").forEach((choicesEl) => {
				...
				if (areAllSillyTavernChromeButtons(buttons)) return;
				const alreadyFlat =
					buttons.every((btn) => btn.parentElement === choicesEl) &&
					Array.from(choicesEl.childNodes).every(
						(n) =>
							n.nodeType === Node.ELEMENT_NODE &&
							/** @type {Element} */ (n).tagName === "BUTTON",
					);
			...
			const flush = () => {
				...
				for (const el of run) {
					if (el.tagName === "BUTTON")
						buttons.push(/** @type {HTMLButtonElement} */ (el));
					else
						buttons.push(
				...
				if (areAllSillyTavernChromeButtons(buttons)) {
					run = [];
					return;
				}
				if (buttons.every((b) => b.closest(".rt-cyoa-choices, choices"))) {
					run = [];
					return;
				}
				const wrap = createCyoaChoicesWrap();
		...
		function cyoaBindChoiceButtons({ allowFlatten = true } = {}) {
			...
			if (isSillyTavernWelcomeScreen()) {
				document
					.querySelectorAll("#chat .mes_text")
					.forEach(repairHijackedSillyTavernChromeButtons);
				return;
			}
		...
		if (event_types.MESSAGE_SWIPED)
			eventSource.on(event_types.MESSAGE_SWIPED, () => scheduleCyoaBind(100));
		if (event_types.MESSAGE_EDITED)
			eventSource.on(event_types.MESSAGE_EDITED, () => scheduleCyoaBind(100));
		// During stream: refresh placeholder visibility only (no flatten)
		if (event_types.MESSAGE_RECEIVED)
			eventSource.on(event_types.MESSAGE_RECEIVED, () => scheduleCyoaBind(50));
		if (event_types.MESSAGE_UPDATED)
			eventSource.on(event_types.MESSAGE_UPDATED, () => scheduleCyoaBind(30));
		if (event_types.MORE_MESSAGES_LOADED)
			eventSource.on(event_types.MORE_MESSAGES_LOADED, () =>
				scheduleCyoaBind(100),
			);
		...
		if (_cyoaChatRoot) {
			new MutationObserver(() => {
				...
				_cyoaMutTimer = setTimeout(() => {
					if (
						document.querySelector(
							"#chat .mes_text p:has(> button + br + button)",
						)
					) {
						scheduleCyoaBind(0);
					}
				}, 100);
			}).observe(_cyoaChatRoot, {
				childList: true,
				subtree: true,
				characterData: true,
			});
		...
		function buildCyoaSlotInput(type, slot = {}) {
			if (type === "custom") {
				const text =
					slot.text || [slot.left, slot.right].filter(Boolean).join(" ");
			...
			const placeholder =
				type === "trait"
					? "Ability name (e.g. Illithid) — optional"
					: type === "prefix"
						? "Label (e.g. [Attack] or [Timeskip])"
						: "";
		...
		function buildCyoaSlotRow(slot, idx) {
			...
			const typeOpts = [
				...
				.map(
					([v, l]) =>
						`<option value="${v}"${slotType === v ? " selected" : ""}>${l}</option>`,
				)
				.join("");

			const hasInput =
				slotType === "trait" || slotType === "prefix" || slotType === "custom";
		...
		function readSlotsFromPopup(container) {
			return Array.from(container.querySelectorAll(".cyoa-slot-row")).map(
				(row) => {
					let type = row.querySelector(".cyoa-slot-type").value;
					if (type === "roll") type = "narrative";
					if (type === "custom") {
						const text =
							row.querySelector(".cyoa-slot-custom-text")?.value?.trim() || "";
						return { type, ...(text ? { text } : {}) };
					}
					const label =
						row.querySelector(".cyoa-slot-label")?.value?.trim() || "";
		...
		function readCyoaStyleFromDialog(dlg) {
			...
			const mechAccentCustom = !!dlg.querySelector("#cyoa-mech-accent-custom")
				?.checked;
			return {
				buttonColor: dlg.querySelector("#cyoa-btn-color")?.value || "#120a28",
				buttonOpacity:
					parseInt(dlg.querySelector("#cyoa-btn-opacity")?.value ?? "90", 10) /
					100,
				buttonTextColor: useThemeText
					? ""
					: dlg.querySelector("#cyoa-text-color")?.value || "",
				buttonBorderColor: borderCustom
					? dlg.querySelector("#cyoa-border-color")?.value || ""
					: "",
				choiceAccentColor: accentCustom
					? dlg.querySelector("#cyoa-accent-color")?.value || ""
					: "",
				mechColor: dlg.querySelector("#cyoa-mech-color")?.value || "#ffc966",
				mechBgOpacity:
					parseInt(
						dlg.querySelector("#cyoa-mech-bg-opacity")?.value ?? "14",
						10,
					) / 100,
				...
				mechAccentColor: mechAccentCustom
					? dlg.querySelector("#cyoa-mech-accent-color")?.value || ""
					: "",
			};
		}
		...
		function refreshCyoaStylePreview(dlg) {
			...
			if (!preview) return;
			preview.innerHTML = buildCyoaStylePreviewHtml(
				readCyoaStyleFromDialog(dlg),
			);
			const hex = dlg.querySelector("#cyoa-btn-color")?.value || "#120a28";
			const pct = parseInt(
				dlg.querySelector("#cyoa-btn-opacity")?.value ?? "90",
				10,
			);
		...
		function refreshCyoaPresetSelect(dlg, selectedName = "") {
			...
			select.innerHTML =
				...
				Object.keys(presets)
					.map(
						(name) =>
							`<option value="${escapeCyoaPresetHtml(name)}" ${name === selectedName ? "selected" : ""}>${escapeCyoaPresetHtml(name)}</option>`,
					)
					.join("");
		}
		...
		function showCyoaPresetExportPopup(presetName, visibleSlots = null) {
			...
			const slots = Array.isArray(visibleSlots)
				? visibleSlots
				: presets[presetName];
			...
			Popup.show.confirm(
				...
				{ okButton: "Done", cancelButton: false },
			);
			setTimeout(() => {
				document
					.getElementById("cyoa-preset-export-copy")
					?.addEventListener("click", async () => {
						try {
							await navigator.clipboard.writeText(json);
							toastr.success(
								`CYOA preset "${presetName}" copied to clipboard!`,
								"CYOA",
							);
						} catch (err) {
							console.error(
								"[RPG Tracker] CYOA preset clipboard copy failed:",
								err,
							);
							toastr.error(
				...
				document
					.getElementById("cyoa-preset-export-download")
					?.addEventListener("click", () => {
		...
		async function importCyoaPresets() {
			...
			setTimeout(() => {
				document
					.getElementById("cyoa-preset-import-json")
			...
			if (
				parsed?.format !== CYOA_PRESET_EXPORT_FORMAT ||
			...
			const config =
				getSettings().cyoaConfig || (getSettings().cyoaConfig = {});
			if (!config.presets || typeof config.presets !== "object")
				config.presets = {};
			const baseName = parsed.name.trim().slice(0, 100) || "Imported Preset";
			let name = baseName;
			let suffix = 2;
			while (Object.prototype.hasOwnProperty.call(config.presets, name))
			...
			config.presets[name] = parsed.slots.map((slot) =>
				slot?.type === "roll" ? { ...slot, type: "narrative" } : { ...slot },
			);
			saveSettings();
		...
		function showCyoaSettingsPopup() {
			...
			const initialPrompt =
				cfg.useCustomPrompt && cfg.customPromptText?.trim()
					? cfg.customPromptText.trim()
					: buildCyoaPrompt(cfg);

			const currentSlotsStr = JSON.stringify(slots);
			const presetMatches = Object.entries(cfg.presets || {}).find(
				([k, v]) => JSON.stringify(v) === currentSlotsStr,
			);
            ...
            <div style="font-family:inherit;max-width:560px;min-width:380px;max-height:80vh;overflow-y:auto;overflow-x:hidden;padding-right:4px;box-sizing:border-box;">
                ...
                <div style="display:flex;gap:5px;margin-bottom:12px;align-items:center;">
                    ...
                    <select id="cyoa-preset-select" class="text_pole" style="flex:1;font-size:11px;height:24px;">
                        <option value="">-- Select Preset --</option>
                        ${Object.keys(cfg.presets || {})
													.map(
														(k) =>
															`<option value="${k}" ${k === activePreset ? "selected" : ""}>${k}</option>`,
													)
													.join("")}
                    </select>
			...
			if (dlg) {
				dlg.addEventListener("change", (e) => {
					...
					if (
						e.target.matches(
							"#cyoa-use-emojis, #cyoa-use-xml, #cyoa-use-buttons",
						)
					) {
						regeneratePromptPreview(dlg);
					}
					if (e.target.id === "cyoa-preset-select") {
						...
						list.innerHTML = pSlots
							.map((sl, i) => buildCyoaSlotRow(sl, i))
							.join("");
						regeneratePromptPreview(dlg);
					}
				});
				...
				dlg.addEventListener("input", (e) => {
					if (
						e.target.classList.contains("cyoa-slot-label") ||
					...
					const styleInputIds = new Set([
						...
						"cyoa-accent-color",
						"cyoa-mech-color",
						"cyoa-mech-bg-opacity",
						"cyoa-dc-color",
						"cyoa-mod-color",
						"cyoa-tag-color",
						"cyoa-mech-accent-color",
					]);
				...
				dlg.addEventListener("click", (e) => {
					...
					if (e.target.id === "cyoa-add-slot") {
						...
						list.insertAdjacentHTML(
							...
							buildCyoaSlotRow(
								{ type: "narrative" },
								list.querySelectorAll(".cyoa-slot-row").length,
							),
						);
						regeneratePromptPreview(dlg);
					}
					if (e.target.id === "cyoa-reset-slots") {
						if (
							!confirm("Reset all choice slots to the default configuration?")
						)
							return;
						const list = dlg.querySelector("#cyoa-slot-list");
						list.innerHTML = DEFAULT_CYOA_SLOTS.map((sl, i) =>
							buildCyoaSlotRow(sl, i),
						).join("");
						regeneratePromptPreview(dlg);
					...
					if (e.target.id === "cyoa-preset-save") {
						...
						if (!name) {
							toastr.warning(
								"Select a preset to save, or use Save As… to create one.",
								"CYOA",
							);
							return;
						}
					...
					if (e.target.id === "cyoa-preset-save-as") {
						...
						if (!freshS.cyoaConfig.presets) freshS.cyoaConfig.presets = {};
						if (
							freshS.cyoaConfig.presets[name] &&
							!confirm(`Overwrite preset "${name}"?`)
						)
							return;
						freshS.cyoaConfig.presets[name] = readSlotsFromPopup(dlg);
						saveSettings();
					...
					if (e.target.id === "cyoa-preset-import") {
						void importCyoaPresets().then((imported) => {
							...
							const list = dlg.querySelector("#cyoa-slot-list");
							if (list)
								list.innerHTML = imported.slots
									.map((slot, index) => buildCyoaSlotRow(slot, index))
									.join("");
							regeneratePromptPreview(dlg);
						});
					}
					if (e.target.id === "cyoa-preset-del") {
						...
						if (freshS.cyoaConfig.presets) {
							...
							sel.innerHTML =
								'<option value="">-- Select Preset --</option>' +
								Object.keys(freshS.cyoaConfig.presets)
			...
			popup.show().then((result) => {
				...
				freshS.cyoaConfig.useEmojis =
					!!dlg.querySelector("#cyoa-use-emojis")?.checked;
				freshS.cyoaConfig.useXmlTag =
					!!dlg.querySelector("#cyoa-use-xml")?.checked;
				freshS.cyoaConfig.useButtonTags =
					!!dlg.querySelector("#cyoa-use-buttons")?.checked;
				freshS.cyoaConfig.stripOldChoicesFromPrompt = !!dlg.querySelector(
					"#cyoa-strip-old-prompt",
				)?.checked;
				const styleCfg = readCyoaStyleFromDialog(dlg);
				Object.assign(freshS.cyoaConfig, styleCfg);
				const promptTa =
					dlg.querySelector("#cyoa-prompt-textarea")?.value?.trim() || "";
		...
		const _syspromptModDefs = [
			...
			{
				key: "dungeon_reality_and_hidden_mapping",
				id: "rpg_sysprompt_mod_dungeon_reality_and_hidden_mapping",
			},
		...
		// ── Narrative pacing ──────────────────────────────────────────────────
		const validNarrativePacing = new Set([
			"normal",
			"shorter_outputs",
			"high_agency",
			"downtime",
		...
		const syncNarrativePacingUi = () => {
			const mode = validNarrativePacing.has(getSettings().narrativePacing)
				...
				: "normal";
			$(`input[name="rpg_narrative_pacing"][value="${mode}"]`).prop(
				"checked",
				true,
			);
		};
		...
		document
			.getElementById("rpg_cyoa_settings_btn")
			?.addEventListener("click", () => showCyoaSettingsPopup());
		document
			.getElementById("rt_onboarding_cyoa_settings_btn")
			?.addEventListener("click", () => showCyoaSettingsPopup());
		...
		async function showRelationshipSettingsPopup() {
			...
			const configuredPrompt =
				typeof settings.npcRelationshipStateTrackerPrompt === "string"
					? settings.npcRelationshipStateTrackerPrompt
					: "";
			...
			const promptField = popup.dlg?.querySelector(
				"#rpg_relationship_state_tracker_prompt",
			);
			popup.dlg
				?.querySelector("#rpg_relationship_state_tracker_prompt_reset")
				?.addEventListener("click", () => {
			...
			if (result !== PR.YES && result !== 1) return;
			const selected = popup.dlg?.querySelector(
				'input[name="rpg_relationship_update_mode"]:checked',
			)?.value;
			settings.npcRelationshipUpdateMode =
				selected === RELATIONSHIP_UPDATE_MODES.STATE_TRACKER
					? RELATIONSHIP_UPDATE_MODES.STATE_TRACKER
		...
		document
			.getElementById("rpg_relationship_settings_btn")
			?.addEventListener("click", () => void showRelationshipSettingsPopup());

		// Deadlines Toggle
		const deadlinesCb = /** @type {HTMLInputElement} */ (
			document.getElementById("rpg_quests_deadlines")
		);
		const frustrationWrap = document.getElementById(
			"rpg_quests_frustration_wrap",
		);
		const syncFrustrationVisibility = () => {
			if (frustrationWrap)
				frustrationWrap.style.display = deadlinesCb?.checked ? "" : "none";
		};
		if (deadlinesCb) {
			...
			deadlinesCb.addEventListener("change", function () {
				...
				if (!this.checked) {
					...
					const fCb = /** @type {HTMLInputElement|null} */ (
						document.getElementById("rpg_quests_frustration")
					);
		...
		// Frustration Toggle
		const frustrationCb = /** @type {HTMLInputElement} */ (
			document.getElementById("rpg_quests_frustration")
		);
		if (frustrationCb) {
			frustrationCb.checked =
				!!getSettings().syspromptModules?.questsFrustration;
		...
		const showArchiveCb = /** @type {HTMLInputElement|null} */ (
			document.getElementById("rpg_quests_show_archive")
		);
		if (showArchiveCb) {
			showArchiveCb.checked =
				getSettings().syspromptModules?.questsShowArchive !== false;
		...
		// Quests Help Trigger
		const rngModeRadios = document.querySelectorAll(
			'input[name="rpg_sysprompt_rng_mode"]',
		);
		if (rngModeRadios.length) {
			...
			$(`input[name="rpg_sysprompt_rng_mode"][value="${currentRngMode}"]`).prop(
				"checked",
				true,
			);
		...
		$("#rpg_tracker_router_enabled")
			.prop("checked", settings.routerEnabled)
			.on("change", function () {
		...
		routerSourceSelect
			.val(settings.routerConnectionSource || "default")
			.on("change", function () {
		...
		$("#rpg_tracker_router_prefix_override")
			.val(settings.routerCampaignPrefixOverride || "")
			.on("input", function () {
				...
				const chatId =
					...
					ctx.chatId ||
					"";
				settings.routerCampaignPrefix =
					getEffectiveRouterCampaignPrefix(chatId);
				saveSettings();
				updateSettingsLorePrefixReadout();
			});
		...
		$("#rpg_tracker_activate_books_btn").on("click", async function () {
			...
			try {
				const count = await activateCampaignBooks({
					debugSource: "manual:settings-activate-books",
				});
				toastr["success"](
					`Activated ${count} campaign lorebook${count === 1 ? "" : "s"}.`,
				);
		...
		$("#rt-agent-router-full-audit, #rt-agent-router-full-audit-panel").on(
			"click",
			async function () {
				const { Popup } = SillyTavern.getContext();
				const confirmHtml = `
				...
				$(
					"#rt-agent-router-full-audit, #rt-agent-router-full-audit-panel",
				).prop("disabled", true);
				try {
					const ctx = SillyTavern.getContext();
					const { chat } = ctx;
					...
					const maxContextLimit =
						ctx.contextSize || settings.fullAuditMaxTokens || 32000;
					const tokenBuffer = 3000;
					const chunkTokenLimit = Math.max(1000, maxContextLimit - tokenBuffer);
					...
					console.log(
						`[RPG Tracker] Agent Full Audit: ${chunks.length} chunk(s) queued.`,
					);

					for (let i = 0; i < chunks.length; i++) {
						toastr.info(
							`Agent Full Audit: Chunk ${i + 1} of ${chunks.length}...`,
							"Lorebook Agent",
							{ timeOut: 8000 },
						);
						console.log(
							`[RPG Tracker] Agent Full Audit: Starting chunk ${i + 1}/${chunks.length} (${chunks[i].length} messages)`,
						);
						...
						if (isRouterRunning()) {
							console.warn(
								`[RPG Tracker] Agent Full Audit: Chunk ${i + 1} skipped — router still busy after 30s.`,
							);
							toastr.warning(
								`Chunk ${i + 1} skipped — agent was still busy.`,
								"Lorebook Agent",
							);
							continue;
						}
						...
						const chunkResult = await runRouterPass(
							null,
							LOREBOOK_FULL_AUDIT_INSTRUCTION,
							null,
							true,
						...
						console.log(
							`[RPG Tracker] Agent Full Audit: Chunk ${i + 1}/${chunks.length} finished. Result: ${chunkResult}`,
						);
					...
					toastr.success(
						`Agent Full Audit complete (${chunks.length} chunk${chunks.length > 1 ? "s" : ""}).`,
						"Lorebook Agent",
					);
				} catch (e) {
		...
		$("#rpg_tracker_lore_debug_capture").on("click", async function () {
			...
			btn.prop("disabled", true);
			try {
				_loreActivationDebugLast = await readLoreActivationDebugSnapshot(
					"manual:capture-settings",
				);
				renderLoreActivationDebugPanel();
				toastr["info"](
					"Lore debug snapshot captured (read-only, no /world commands).",
				);
			} catch (_) {
		...
		$("#rpg_tracker_lore_debug_resync").on("click", async function () {
			...
			try {
				...
				toastr["info"](
					"Re-sync finished; see JSON in Lore activation debug below.",
				);
			} catch (_) {
		...
		// Router Ollama
		$("#rpg_tracker_router_ollama_url")
			.val(settings.routerOllamaUrl)
			.on("input", function () {
				settings.routerOllamaUrl = $(this).val();
				saveSettings();
			});
		const routerOllamaModelSelect = $("#rpg_tracker_router_ollama_model");
		routerOllamaModelSelect
			.val(settings.routerOllamaModel)
			.on("change", function () {
		...
		$("#rpg_tracker_router_ollama_refresh").on("click", async function () {
			...
			try {
				...
				routerOllamaModelSelect
					...
					.append('<option value="">-- Select Model --</option>');
				models.forEach((m) => {
					routerOllamaModelSelect.append(
						$("<option></option>").val(m.name).text(m.name),
					);
				});
		...
		// Router OpenAI
		$("#rpg_tracker_router_openai_url")
			.val(settings.routerOpenaiUrl)
			.on("input", function () {
				settings.routerOpenaiUrl = $(this).val();
				saveSettings();
			});
		$("#rpg_tracker_router_openai_key")
			.val(settings.routerOpenaiKey)
			.on("input", function () {
				settings.routerOpenaiKey = $(this).val();
				saveSettings();
			});
		const routerOpenaiModelSelect = $("#rpg_tracker_router_openai_model");
		const routerOpenaiModelManual = $(
			"#rpg_tracker_router_openai_model_manual",
		);
		...
		routerOpenaiModelSelect.on("change", function () {
			...
			if (val) {
				...
				settings.routerOpenaiModel =
					String(routerOpenaiModelManual.val() || "").trim() || "";
			}
			saveSettings();
		});
		routerOpenaiModelManual.on("input", function () {
			...
			settings.routerOpenaiModel =
				manual || String(routerOpenaiModelSelect.val() || "") || "";
			saveSettings();
		});
		$("#rpg_tracker_router_openai_refresh").on("click", async function () {
			...
			try {
				toastr["info"]("Fetching models...");
				const models = await fetchOpenAIModels(url, key);
				routerOpenaiModelSelect
					.empty()
					.append('<option value="">-- Select Model --</option>');
				models.forEach((m) => {
					const id = typeof m === "string" ? m : m.id || m.name;
					if (id)
						routerOpenaiModelSelect.append(
							$("<option></option>").val(id).text(id),
						);
				});
		...
		if (
			!tryBindConnectionProfileDropdown(
				"#rpg_tracker_router_connection_profile",
				settings.routerConnectionProfileId,
				(id) => {
			...
			getConnectionProfiles().then((profiles) => {
				routerProfileSelect
					...
					.append('<option value="">-- No Profile Selected --</option>');
				profiles.forEach((p) =>
					routerProfileSelect.append($("<option></option>").val(p).text(p)),
				);
		...
		if (pm && typeof pm.getAllPresets === "function") {
			...
			routerPresetSelect
				...
				.append('<option value="">-- Use Current Settings --</option>');
			presets.forEach((p) =>
				routerPresetSelect.append($("<option></option>").val(p).text(p)),
			);
		...
		$("#rpg_tracker_router_basic_mode")
			.prop("checked", settings.routerBasicMode)
			.on("change", function () {
		...
		$("#rpg_tracker_router_native_keyword_activation")
			.prop("checked", settings.routerNativeKeywordActivation)
			.on("change", function () {
				settings.routerNativeKeywordActivation = $(this).prop("checked");
				$("#rt-agent-router-native-kw").prop(
					"checked",
					settings.routerNativeKeywordActivation,
				);
				saveSettings();
			});
		$("#rpg_tracker_router_include_hidden")
			.prop("checked", settings.routerIncludeHidden)
			.on("change", function () {
				settings.routerIncludeHidden = $(this).prop("checked");
				$("#rt-agent-router-include-hidden").prop(
					"checked",
					settings.routerIncludeHidden,
				);
				saveSettings();
			});
		$("#rpg_tracker_router_swipe_rollback")
			.prop("checked", settings.routerSwipeRollback !== false)
			.on("change", function () {
				settings.routerSwipeRollback = $(this).prop("checked");
				$("#rt-agent-router-swipe-rollback").prop(
					"checked",
					settings.routerSwipeRollback,
				);
				saveSettings();
			});
		// Lorebook Agent lookback mode — three-option radio group
		const routerLookbackNumericRow = $(
			"#rpg_tracker_router_lookback_numeric_row",
		);
		const applyDrawerLookbackUI = (mode) => {
			const isFixed = mode === "fixed";
			routerLookbackNumericRow.css({
				opacity: isFixed ? "1" : "0.35",
				"pointer-events": isFixed ? "auto" : "none",
			});
		};

		// Determine current mode from settings
		const currentLookbackMode =
			settings.routerLookbackSinceLastRun !== false
				? "since_last_run"
				: settings.routerLookbackSinceLastUser === true
					? "since_last_user"
					: "fixed";

		// Init radio selection and numeric row state
		$(`#rpg_tracker_router_lookback_since_last_run`).prop(
			"checked",
			currentLookbackMode === "since_last_run",
		);
		$(`#rpg_tracker_router_lookback_since_last_user`).prop(
			"checked",
			currentLookbackMode === "since_last_user",
		);
		$(`#rpg_tracker_router_lookback_fixed`).prop(
			"checked",
			currentLookbackMode === "fixed",
		);
		...
		$('input[name="router_lookback_mode"]').on("change", function () {
			...
			const panelRadio = $(
				`#rt-agent-lookback-mode-${mode === "since_last_run" ? "run" : mode === "since_last_user" ? "user" : "fixed"}`,
			);
			...
			if (panelContainer.length) {
				panelContainer.css({
					opacity: mode === "fixed" ? "1" : "0.35",
					"pointer-events": mode === "fixed" ? "auto" : "none",
				});
			}
			saveSettings();
		});

		$("#rpg_tracker_router_lookback")
			.val(settings.routerLookback)
			.on("input", function () {
				settings.routerLookback = parseInt(String($(this).val() || "")) || 4;
				$("#rt-agent-router-lookback").val(settings.routerLookback);
				saveSettings();
			});
		$("#rpg_tracker_router_run_every")
			.val(settings.routerRunEvery || 3)
			.on("input", function () {
				settings.routerRunEvery = parseInt(String($(this).val() || "")) || 3;
				$("#rt-agent-router-run-every").val(settings.routerRunEvery);
				saveSettings();
			});
		$("#rpg_tracker_router_max_turns")
			.val(settings.routerMaxTurns)
			.on("input", function () {
				settings.routerMaxTurns = parseInt(String($(this).val() || "")) || 5;
				$("#rt-agent-router-max-turns").val(settings.routerMaxTurns);
				saveSettings();
			});
		$("#rpg_tracker_router_max_activations")
			.val(settings.routerMaxActivations)
			.on("input", function () {
				const val = parseInt(String($(this).val() || "")) || 12;
				settings.routerMaxActivations = val;
				$("#rt-agent-router-max-activations").val(
					settings.routerMaxActivations,
				);
				saveSettings();
			});
		$("#rpg_tracker_router_max_keyword_overflow")
			.val(settings.routerMaxKeywordOverflow ?? 6)
			.on("input", function () {
				settings.routerMaxKeywordOverflow =
					parseInt(String($(this).val() || "")) || 0;
				$("#rt-agent-router-kw-overflow-cap").val(
					settings.routerMaxKeywordOverflow,
				);
				saveSettings();
			});
		...
		$(document).off(
			"change.rpgPortraitDisplay",
			"#rpg_tracker_npc_portraits, #rpg_tracker_location_images",
		);
		$(document).on(
			"change.rpgPortraitDisplay",
			"#rpg_tracker_npc_portraits",
			async function () {
				const s = getSettings();
		...
		$("#rpg_tracker_npc_portraits").prop(
			"checked",
			settings.npcPortraits !== false,
		);
		syncNpcPortraitDependentUi(settings);

		$("#rpg_tracker_location_images").prop(
			"checked",
			!!settings.locationImages,
		);
		syncLocationImageDependentUi(settings);

		$("#rpg_tracker_npc_major_words")
			.val(settings.npcMajorWords ?? 225)
			.on("change", function () {
				...
				if (settings.routerModules?.npc) {
					settings.routerModules.npc.instruction = buildNpcInstruction(
						settings.npcMajorWords,
						settings.npcMinorWords,
						false,
						settings,
					);
				}
		...
		$("#rpg_tracker_npc_minor_words")
			.val(settings.npcMinorWords ?? 135)
			.on("change", function () {
				const raw = parseInt(String($(this).val() || ""), 10);
		...
		$("#rpg_tracker_npc_rel_bars")
			.prop("checked", !!settings.npcRelationshipBars)
			.on("change", function () {
				handleRelBarsChange($(this).prop("checked"));
			});

		$("#rpg_sysprompt_mod_npc_rel_bars")
			.prop("checked", !!settings.npcRelationshipBars)
			.on("change", function () {
				handleRelBarsChange($(this).prop("checked"));
			});
		$("#rpg_tracker_npc_rel_toast")
			.prop("checked", settings.npcRelationshipToast !== false)
			.on("change", function () {
				settings.npcRelationshipToast = $(this).prop("checked");
				saveSettings();
			});
		$("#rpg_tracker_npc_rel_max_default")
			.val(getNpcRelationshipMaxDefault(settings))
			.on("change", function () {
				const raw = parseInt(String($(this).val() || ""), 10);
		...
		$("#rpg_tracker_ignore_npc_limits")
			.prop("checked", !!settings.ignoreNpcImportLimits)
			.on("change", function () {
		...
		$(
			`input[name="rpg_npc_add_as_is_mode_main"][value="${_currentAddAsIsMode}"]`,
		).prop("checked", true);
		...
		$('input[name="rpg_npc_add_as_is_mode_main"]').on("change", function () {
			...
			$(`input[name="rt-npc-add-as-is-mode"][value="${newMode}"]`).prop(
				"checked",
				true,
			);
		});
		...
		defPosSelect
			.find(`option[value="${defaultPosition}"][data-role="${roleAttr}"]`)
			.prop("selected", true);

		$("#rpg_tracker_router_default_depth").val(
			settings.routerDefaultDepth ?? 4,
		);
		$("#rpg_tracker_router_default_order").val(
			settings.routerDefaultOrder ?? 100,
		);

		function updateDefaultPositionFieldsVisibility() {
			const posVal = parseInt(String(defPosSelect.val() || "4"));
			jqueryToggleSlide(
				$("#rpg_tracker_router_default_depth_container"),
				posVal === 4,
			);
		}
		...
		defPosSelect.on("change", function () {
			...
			settings.routerDefaultRole =
				roleVal !== undefined && roleVal !== "" ? parseInt(String(roleVal)) : 0;
			saveSettings();
			updateDefaultPositionFieldsVisibility();
		});
		...
		lorePosSelect
			.find(`option[value="${lorePosition}"][data-role="${loreRoleAttr}"]`)
			.prop("selected", true);

		$("#rpg_tracker_lore_injection_depth").val(
			settings.loreInjectionDepth ?? 4,
		);

		function updateLorePositionFieldsVisibility() {
			const posVal = parseInt(String(lorePosSelect.val() || "4"));
			jqueryToggleSlide(
				$("#rpg_tracker_lore_injection_depth_container"),
				posVal === 4,
			);
		}
		...
		function syncRouterPromptUi() {
			...
			if (isBasic) {
				$desc.html(
					"The editable <strong>Basic Mode</strong> base prompt. Its format/module template is editable below; dynamic placeholders are expanded only for each request.",
				);
				...
				$fragmentsAgent.hide();
				$btn.html(
					'<i class="fa-solid fa-arrow-rotate-left"></i> Reset Basic Mode Prompts',
				);
			} else {
				$desc.html(
					"The editable <strong>Agent Mode</strong> base prompt. Shared rules are editable below; action/tool schemas are generated from enabled modules at request time.",
				);
				...
				$fragmentsAgent.show();
				$btn.html(
					'<i class="fa-solid fa-arrow-rotate-left"></i> Reset Agent Mode Prompts',
				);
			}
			$("#rpg_tracker_router_combat_guidance_basic").val(
				settings.routerCombatProfileGuidanceBasicTemplate || "",
			);
			$("#rpg_tracker_router_combat_guidance_agent").val(
				settings.routerCombatProfileGuidanceAgentTemplate || "",
			);
			$("#rpg_tracker_router_rel_section_basic").val(
				settings.routerRelSectionBasicTemplate || "",
			);
			$("#rpg_tracker_router_rel_section_agent").val(
				settings.routerRelSectionAgentTemplate || "",
			);
			$("#rpg_tracker_router_auto_pass_restriction").val(
				settings.routerAutoPassRestrictionTemplate || "",
			);
			$("#rpg_tracker_router_manual_pass_restriction").val(
				settings.routerManualPassRestrictionTemplate || "",
			);
			$("#rpg_tracker_router_existing_npc_nudge").val(
				settings.routerExistingNpcNudgeTemplate || "",
			);
			...
			if (
				typeof /** @type {any} */ (isBasic ? $modular : $agentContext)
					.trigger === "function"
			) {
				/** @type {any} */ (isBasic ? $modular : $agentContext).trigger(
					"autosize.resize",
				);
			}
		}
		...
		$("#rpg_tracker_router_prompt").on("input", function () {
			...
			const limitMatch =
				val.match(/You are limited to \*\*(\d+) active entries\*\*/i) ||
		...
		const routerFragmentBindings = [
			[
				"#rpg_tracker_router_combat_guidance_basic",
				"routerCombatProfileGuidanceBasicTemplate",
			],
			[
				"#rpg_tracker_router_combat_guidance_agent",
				"routerCombatProfileGuidanceAgentTemplate",
			],
			[
				"#rpg_tracker_router_rel_section_basic",
				"routerRelSectionBasicTemplate",
			],
			[
				"#rpg_tracker_router_rel_section_agent",
				"routerRelSectionAgentTemplate",
			],
			[
				"#rpg_tracker_router_auto_pass_restriction",
				"routerAutoPassRestrictionTemplate",
			],
			[
				"#rpg_tracker_router_manual_pass_restriction",
				"routerManualPassRestrictionTemplate",
			],
			[
				"#rpg_tracker_router_existing_npc_nudge",
		...
		$("#rpg_tracker_router_btn_reset_prompt").on("click", function () {
			...
			const promptLabel = isBasic
				? "base, format/module, and runtime-fragment prompts"
				: "base, shared-context, and runtime-fragment prompts";
		...
		if (
			settings.npcWordTargetRescaleNotice &&
			typeof settings.npcWordTargetRescaleNotice === "object"
		) {
		...
		const $wpLocationsPerReport = $(
			"#rpg_world_progression_locations_per_report",
		);
		...
		$wpEnabled
			.prop("checked", !!settings.worldProgressionEnabled)
			.on("change", async function () {
				...
				if (runtimeState.currentChatId) {
					await syncCampaignPrefixAndWorldsForChat(
						runtimeState.currentChatId,
						"toggle-world-progression",
					);
				}
			});
		$wpInterval
			.val(settings.worldProgressionIntervalHours || 24)
			.on("input", function () {
				getSettings().worldProgressionIntervalHours =
					parseInt(String($(this).val() || "")) || 24;
				saveSettings();
				updateWorldProgressionLastFiredDisplay();
			});

		$wpKeepActive
			.val(settings.worldProgressionKeepActive || 1)
			.on("input", function () {
				getSettings().worldProgressionKeepActive =
					parseInt(String($(this).val() || "")) || 1;
				saveSettings();
			});
		$wpHistoryLookback
			.val(settings.worldProgressionHistoryLookback ?? 0)
			.on("input", function () {
				getSettings().worldProgressionHistoryLookback =
					parseInt(String($(this).val() || "")) || 0;
				saveSettings();
			});
		$wpLocationsPerReport
			.val(settings.worldProgressionLocationsPerReport ?? 3)
			.on("change", function () {
				getSettings().worldProgressionLocationsPerReport = Math.max(
					1,
					Math.min(12, parseInt(String($(this).val()), 10) || 3),
				);
				$(this).val(getSettings().worldProgressionLocationsPerReport);
				saveSettings();
				$("#rt-agent-world-locations").val(
					getSettings().worldProgressionLocationsPerReport,
				);
			});
		$wpLocationRandomize
			.prop("checked", settings.worldProgressionLocationRandomize !== false)
			.on("change", function () {
				getSettings().worldProgressionLocationRandomize =
					!!$(this).prop("checked");
				saveSettings();
			});

		const $wpConsolidateEnabled = $(
			"#rpg_world_progression_consolidate_enabled",
		);
		const $wpConsolidateInterval = $(
			"#rpg_world_progression_consolidate_interval",
		);
		const $wpConsolidateIntervalContainer = $(
			"#rpg_world_progression_consolidate_interval_container",
		);
		...
		$wpConsolidateEnabled
			.prop("checked", !!settings.worldProgressionConsolidateEnabled)
			.on("change", function () {
				getSettings().worldProgressionConsolidateEnabled =
					!!$(this).prop("checked");
				saveSettings();
				updateConsolidateVisibility();
			});
		$wpConsolidateInterval
			.val(settings.worldProgressionConsolidateInterval ?? 7)
			.on("input", function () {
				getSettings().worldProgressionConsolidateInterval =
					parseInt(String($(this).val() || "")) || 7;
				saveSettings();
			});
		updateConsolidateVisibility();

		$wpLookback
			.val(settings.worldProgressionLookback ?? 0)
			.on("input", function () {
				getSettings().worldProgressionLookback =
					parseInt(String($(this).val() || "")) || 0;
				saveSettings();
			});
		const $wpExclusionList = $("#rpg_world_progression_exclusion_list");
		$wpExclusionList
			.val(settings.worldProgressionExclusionList || "")
			.on("input", function () {
				getSettings().worldProgressionExclusionList = String(
					$(this).val() || "",
				);
				saveSettings();
			});
		$wpSystemPrompt
			.val(settings.worldProgressionSystemPrompt || "")
			.on("input", function () {
				getSettings().worldProgressionSystemPrompt = String(
					$(this).val() || "",
				);
				saveSettings();
			});
		$wpResetPrompt.on("click", function () {
			...
			saveSettings();
			toastr["success"](
				"World Progression prompt reset to default.",
				"World Progression",
			);
		});
		...
		const $wpInjectionDepthContainer = $(
			"#rpg_world_progression_injection_depth_container",
		);
		...
		$wpInjectionPosition
			.find(`option[value="${wpPositionVal}"][data-role="${wpRoleAttrVal}"]`)
			.prop("selected", true);
		...
		$wpInjectionPosition.on("change", function () {
			...
			settings.worldProgressionInjectionRole =
				roleVal !== undefined && roleVal !== "" ? parseInt(String(roleVal)) : 0;
			saveSettings();
			updateWpPositionFieldsVisibility();
		});
		...
		$wpInjectionDepth.on("input", function () {
			settings.worldProgressionInjectionDepth =
				parseInt(String($(this).val() || "")) || 0;
			saveSettings();
		});
		...
		$("#rpg_world_progression_btn_override_next").on("click", function () {
			...
			const currentNextMins =
				currentLastMins >= 0
					? currentLastMins + intervalMinutes
					: intervalMinutes;
			...
			updateWorldProgressionLastFiredDisplay();
			toastr["success"](
				`Next report set to ${fmtHint(parsedNextMins)}.`,
				"World Progression",
			);
		});

		$wpGenerateNow.on("click", async function () {
			const { parseInWorldMinutes: piw, runWorldProgressionPass: rwp } =
			...
			if (currentMinutes < 0) {
				toastr["warning"](
					"Cannot parse in-world time from State Memo. Make sure the State Tracker has run at least once.",
					"World Progression",
				);
				return;
			}
			...
			try {
				...
				updateWorldProgressionLastFiredDisplay();
				toastr["success"](
					"World Progression report generated.",
					"World Progression",
				);
			} catch (e) {
				toastr["error"](
					`World Progression error: ${e.message}`,
					"World Progression",
				);
				...
				$wpGenerateNow
					.prop("disabled", false)
					.html(
						'<i class="fa-solid fa-globe"></i> Generate Now (current period)',
					);
			}
		});

		const $wpFireWithInstructions = $(
			"#rpg_world_progression_fire_with_instructions",
		);
		$wpFireWithInstructions.on("click", async function () {
			...
			setTimeout(() => {
				const textarea = document.getElementById(
					"rt_wp_extra_instructions_settings",
				);
				if (textarea) {
					textarea.addEventListener("input", () => {
						extraInstructions = textarea.value.trim();
					});
				}
			}, 100);

			const { Popup } = SillyTavern.getContext();
			const choice = await Popup.show.confirm("World Progression", popupBody, {
				okButton: "Fire",
				cancelButton: "Cancel",
			});
			if (!choice) return;
		...
		$wpResetTimeline.on("click", function () {
			...
			if (typeof runtimeState.updateAgentWorldStatusRef === "function")
				runtimeState.updateAgentWorldStatusRef();
			toastr["info"](
				"World Progression timeline reset. Next report will start from the current time.",
				"World Progression",
			);
		});

		$("#rpg_world_progression_purge_history").on("click", () => {
		...
		$wpConsolidateNow.on("click", async function () {
			...
			if (count < 2) {
				toastr["warning"](
					"Please enter a count of at least 2 reports to consolidate.",
					"World Progression",
				);
				return;
			}
			if (
				!confirm(
					`Are you sure you want to consolidate the oldest ${count} raw reports right now?`,
				)
			) {
				return;
			}

			const { runWorldProgressionConsolidationPass } =
				await import("./router.js");
			...
			try {
				...
				$wpConsolidateNow
					.prop("disabled", false)
					.html('<i class="fa-solid fa-compress"></i> Consolidate Now');
			}
		});

		// ── World Skeleton wiring ───────────────────────────────────────────────
		const $wpSkeletonAtmosphere = $(
			"#rpg_world_progression_skeleton_atmosphere",
		);
		const $wpSkeletonAtmosphereLookback = $(
			"#rpg_world_progression_skeleton_atmosphere_lookback",
		);
		const $wpGenerateAtmosphere = $(
			"#rpg_world_progression_btn_generate_atmosphere",
		);
		const $wpSkeletonUseLorebooks = $(
			"#rpg_world_progression_skeleton_use_lorebooks",
		);
		const $wpSkeletonLorebookGroup = $(
			"#rpg_world_progression_skeleton_lorebook_filter_group",
		);
		const $wpSkeletonLorebookList = $(
			"#rpg_world_progression_skeleton_lorebook_list",
		);
		const $wpSkeletonLorebookOnly = $(
			"#rpg_world_progression_skeleton_lorebook_only",
		);
		const $wpSkeletonLorebookOnlyRow = $(
			"#rpg_world_progression_skeleton_lorebook_only_row",
		);
		const $wpSkeletonUseExisting = $(
			"#rpg_world_progression_skeleton_use_existing",
		);
		...
		const $wpSkeletonPrompt = $(
			"#rpg_world_progression_skeleton_system_prompt",
		);
		const $wpResetSkeletonPrompt = $(
			"#rpg_world_progression_btn_reset_skeleton_prompt",
		);
		const $wpGenerateSkeleton = $(
			"#rpg_world_progression_btn_generate_skeleton",
		);
		...
		function syncSkeletonLorebookOnlyAvailability() {
			...
			const lorebookOnlyActive =
				enabled && !!$wpSkeletonLorebookOnly.prop("checked");
			$wpSkeletonLorebookOnlyRow.css({
			...
			$("#rpg_world_progression_skeleton_counts")
				...
				.find("input")
				.prop("disabled", lorebookOnlyActive);
		}
		...
		async function refreshSkeletonLorebookList() {
			...
			try {
				...
				console.warn(
					"[RPG Tracker] Failed to refresh World Skeleton source lorebooks:",
					error,
				);
			}
			...
			const sourceBooks = [...new Set(worldNames)]
				.filter(
					(name) => name && !String(name).toLowerCase().endsWith("_skeleton"),
				)
				.sort((a, b) => String(a).localeCompare(String(b)));
			if (!sourceBooks.length) {
				$wpSkeletonLorebookList.append(
					$("<i>").css("opacity", "0.6").text("No lorebooks found."),
				);
				return;
			}
		...
		async function updateSkeletonStatus() {
			...
			try {
				...
				const locCount = entries.filter(
					(e) => e.extensions?.rpgCategory === "LOC",
				).length;
				const facCount = entries.filter(
					(e) => e.extensions?.rpgCategory === "FAC",
				).length;
				const conflictCount = entries.filter(
					(e) => e.extensions?.rpgCategory === "EVENT",
				).length;
				const count = locCount + facCount + conflictCount;

				$wpSkeletonStatus.text(
					count > 0
		...
		$wpSkeletonAtmosphere
			.val(settings.worldProgressionSkeletonAtmosphereSummary || "")
			.on("input", function () {
				getSettings().worldProgressionSkeletonAtmosphereSummary = String(
					$(this).val() || "",
				);
				saveSettings();
			});

		$wpSkeletonAtmosphereLookback
			.val(settings.worldProgressionSkeletonAtmosphereLookback ?? 30)
			.on("input", function () {
				getSettings().worldProgressionSkeletonAtmosphereLookback =
					parseInt(String($(this).val() || "")) || 30;
				saveSettings();
			});

		$wpSkeletonUseExisting
			.prop("checked", !!settings.worldProgressionSkeletonUseExisting)
			.on("change", function () {
				getSettings().worldProgressionSkeletonUseExisting =
					!!$(this).prop("checked");
				saveSettings();
			});

		$wpSkeletonUseLorebooks
			.prop("checked", !!settings.worldProgressionSkeletonUseLorebooks)
			.on("change", async function () {
				getSettings().worldProgressionSkeletonUseLorebooks =
					!!$(this).prop("checked");
				$wpSkeletonLorebookGroup.toggle(
					getSettings().worldProgressionSkeletonUseLorebooks,
				);
				syncSkeletonLorebookOnlyAvailability();
				if (getSettings().worldProgressionSkeletonUseLorebooks)
					await refreshSkeletonLorebookList();
				saveSettings();
			});
		$wpSkeletonLorebookOnly
			.prop("checked", !!settings.worldProgressionSkeletonLorebookOnly)
			.on("change", function () {
				getSettings().worldProgressionSkeletonLorebookOnly =
					!!$(this).prop("checked");
				syncSkeletonLorebookOnlyAvailability();
				saveSettings();
			});
		$wpSkeletonLorebookGroup.toggle(
			!!settings.worldProgressionSkeletonUseLorebooks,
		);
		syncSkeletonLorebookOnlyAvailability();
		$("#rpg_world_progression_skeleton_lorebook_refresh").on(
			"click",
			refreshSkeletonLorebookList,
		);
		if (settings.worldProgressionSkeletonUseLorebooks)
			void refreshSkeletonLorebookList();

		$wpGenerateAtmosphere.on("click", async function () {
			const ctx = SillyTavern.getContext();
			if (!ctx.chat || ctx.chat.length === 0) {
				toastr["warning"](
					"No chat history available. Please type some messages first.",
					"World Skeleton",
				);
				return;
			}
			const lookback =
				parseInt(String($wpSkeletonAtmosphereLookback.val() || "")) || 30;
			$wpGenerateAtmosphere
				.prop("disabled", true)
			...
			try {
				...
				saveSettings();
				toastr["success"](
					"Skeleton Source auto-generated successfully.",
					"World Skeleton",
				);
			} catch (e) {
				toastr["error"](
					`Failed to generate Skeleton Source: ${e.message}`,
					"World Skeleton",
				);
			} finally {
				$wpGenerateAtmosphere
		...
		$wpSkeletonFactions
			.val(settings.worldProgressionSkeletonFactions ?? 4)
			.on("input", function () {
				getSettings().worldProgressionSkeletonFactions =
					parseInt(String($(this).val() || "")) || 4;
				saveSettings();
			});

		$wpSkeletonLocations
			.val(settings.worldProgressionSkeletonLocations ?? 4)
			.on("input", function () {
				getSettings().worldProgressionSkeletonLocations =
					parseInt(String($(this).val() || "")) || 4;
				saveSettings();
			});

		$wpSkeletonConflicts
			.val(settings.worldProgressionSkeletonConflicts ?? 3)
			.on("input", function () {
				getSettings().worldProgressionSkeletonConflicts =
					parseInt(String($(this).val() || "")) || 3;
				saveSettings();
			});

		$wpSkeletonPrompt
			.val(settings.worldProgressionSkeletonSystemPrompt || "")
			.on("input", function () {
				getSettings().worldProgressionSkeletonSystemPrompt = String(
					$(this).val() || "",
				);
				saveSettings();
			});
		...
		$wpResetSkeletonPrompt.on("click", function () {
			...
			saveSettings();
			toastr["success"](
				"World Skeleton prompt reset to default.",
				"World Skeleton",
			);
		});
		...
		$wpGenerateSkeleton.on("click", async function () {
			...
			if (!atmosphere && !useLorebooks) {
				toastr["warning"](
					"Please enter a Skeleton Source or enable existing lorebook sources before generating.",
					"World Skeleton",
				);
				return;
			}
			...
			if (!prefix) {
				toastr["warning"](
					"No campaign prefix set. Set a prefix or open a chat in SillyTavern first.",
					"World Skeleton",
				);
				return;
			}
			...
			try {
				...
				toastr["success"](
					`World Skeleton generated: ${count} entries created.`,
					"World Skeleton",
				);
			} catch (e) {
				...
				$wpGenerateSkeleton
					.prop("disabled", false)
					.html(
						'<i class="fa-solid fa-wand-magic-sparkles"></i> Generate Skeleton',
					);
			}
		});
		...
		$wpAddSkeleton.on("click", async function () {
			...
			if (!useExisting && !useLorebooks && !atmosphere) {
				toastr["warning"](
					"Please enter a Skeleton Source or enable an existing source before adding entries.",
					"World Skeleton",
				);
				return;
			}
			...
			try {
				const { runSkeletonGenerationPass } = await import("./router.js");
				const count = await runSkeletonGenerationPass(
					atmosphere,
					true,
					useExisting,
				);
				await updateSkeletonStatus();
				toastr["success"](
					`World Skeleton updated: ${count} additional entries added.`,
					"World Skeleton",
				);
			} catch (e) {
				toastr["error"](`World Skeleton error: ${e.message}`, "World Skeleton");
				...
				$wpAddSkeleton
					.prop("disabled", false)
					.html('<i class="fa-solid fa-plus"></i> Add to Skeleton');
			}
		});
		...
		const customSyspromptCb = /** @type {HTMLInputElement|null} */ (
			document.getElementById("rpg_tracker_custom_sysprompt")
		);
		const narratorConfigBlock = document.getElementById(
			"rpg_narrator_config_block",
		);
		const syncNarratorBlockVisibility = () => {
			if (narratorConfigBlock)
				narratorConfigBlock.style.display = customSyspromptCb?.checked
					? "none"
					: "";
		};
		if (customSyspromptCb) {
		...
		$("#rpg_tracker_btn_update").on("click", async function () {
			...
			if (!chat || chat.length === 0)
				return toastr["info"]("No chat history found.", "RPG Tracker");
			...
			if (!lastAssistantMsg)
				return toastr["info"](
					"No assistant message with content found.",
					"RPG Tracker",
				);
		...
		$("#rpg_tracker_btn_clear").on("click", function () {
			if (
				confirm(
					"Are you sure you want to clear the memory history and wipe the tracker?",
				)
			) {
		...
		$("#rpg_tracker_btn_force_checkpoint").on("click", async function () {
			...
			btn.disabled = true;
			btn.innerHTML =
				'<i class="fa-solid fa-spinner fa-spin"></i> Writing checkpoint…';
			try {
				await forceDiskCheckpoint();
				toastr["success"](
					"Disk checkpoint written. Future rollbacks should land on this state.",
					"RPG Tracker",
				);
			} catch (err) {
				console.error("[RPG Tracker] Force disk checkpoint failed:", err);
				toastr["error"](
					`Checkpoint failed: ${err?.message || err}. Keep this tab open and try again.`,
					"RPG Tracker",
				);
			} finally {
				btn.disabled = false;
		...
		$("#rpg_tracker_btn_factory_reset").on("click", async function () {
			...
			try {
				...
				await saveSettings(true);
				toastr["success"](
					"Framework reset to factory defaults. Reloading in 2 seconds…",
					"RPG Tracker",
				);
				...
				toastr["error"](
					`Factory reset failed: ${err.message || err}`,
					"RPG Tracker",
				);
				setPortraitMigrationLocked(false);
			}
		...
		$("#rpg_tracker_profile_save").on("click", function () {
			const sel = /** @type {HTMLSelectElement} */ (
				document.getElementById("rpg_tracker_profile_select")
			);
			const name = sel.value;
			if (!name)
				return toastr["info"](
					'No profile selected to overwrite. Use "Save As" for new profiles.',
					"RPG Tracker",
				);
			saveProfile(name);
		...
		function syncSettingsUi() {
			...
			const narrativePacing = [
				...
				? s.narrativePacing
				: "normal";
			$(`input[name="rpg_narrative_pacing"][value="${narrativePacing}"]`).prop(
				"checked",
				true,
			);
			...
			$("#rpg_tracker_chat_setup_link_enabled").prop(
				"checked",
				!!s.chatSetupLinkEnabled,
			);
			$("#rpg_tracker_debug").prop("checked", !!s.debugMode);
			$("#rpg_tracker_daynight_cycle").prop(
				"checked",
				!!s.dayNightCycleEnabled,
			);
			...
			$("#rpg_tracker_auto_reset_prompts").prop(
				"checked",
				!!s.autoResetPromptsOnUpdate,
			);
			$("#rpg_main_sysprompt_backup_enabled").prop(
				"checked",
				isMainSyspromptBackupEnabled(s),
			);
			syncMainSyspromptBackupControlsUi();
			$("#rpg_tracker_legacy_dice").prop("checked", !!s.legacyDiceNaming);
			$("#rpg_tracker_dice_d100_mode").prop("checked", !!s.diceD100Mode);
			$("#rpg_tracker_enable_portraits").prop(
				"checked",
				s.enablePortraits !== false,
			);
			$("#rpg_portrait_generator_source").val(
				s.portraitGeneratorSource || "native",
			);
			...
			$("#rpg_tracker_horde_group").toggle(
				(s.portraitGeneratorSource || "native") === "horde",
			);
			$("#rpg_tracker_portrait_skip_prompt").prop(
				"checked",
				!!s.portraitSkipPromptDialog,
			...
			$("#rpg_tracker_portrait_story_lookback").val(
				s.portraitStoryLookback ?? 5,
			);
			$("#rpg_tracker_portrait_story_lookback_row").css({
				opacity: s.portraitUseStoryLookback ? "1" : "0.35",
				"pointer-events": s.portraitUseStoryLookback ? "auto" : "none",
			...
			$("#rpg_portrait_completion_preset").val(
				s.portraitCompletionPresetId || "",
			);
			$("#rpg_portrait_ollama_url").val(
				s.portraitOllamaUrl || "http://localhost:11434",
			);
			...
			$("#rpg_portrait_location_system_prompt").val(
				s.portraitLocationSystemPrompt ||
					getDefaultPortraitLocationSystemPrompt(
						!!s.portraitLocationIncludePresentNpcs,
					),
			);
			...
			applyMapRuntimeConnectionSettingsToUi(s);
			applyMapEvolutionConnectionSettingsToUi(s);
			$("#rpg_map_updater_enabled").prop(
				"checked",
				s.mapUpdaterEnabled !== false,
			);
			$("#rpg_map_updater_run_every").val(s.mapUpdaterRunEvery ?? 1);
			$("#rpg_map_updater_max_tokens").val(s.mapUpdaterMaxTokens ?? 25000);
			$("#rpg_map_updater_system_prompt").val(
			...
			$("#rpg_map_evolution_interval_hours").val(
			...
			$("#rpg_tracker_inventory_worth_mode").val(
				s.inventoryWorthMode || "hover",
			);
			$("#rpg_tracker_show_total_value").prop(
				"checked",
				s.showTotalInventoryValue !== false,
			);
			$("#rpg_tracker_full_review_mode").prop(
				"checked",
				!!s.fullReviewStateMode,
			);
			if (s.fullReviewStateMode) {
				$("#rpg_tracker_full_review_note").css("display", "block");
				$("#rpg_tracker_core_prompt")
					.val(FULL_REVIEW_STATE_SYSTEM_PROMPT)
					.prop("disabled", true)
					.css("opacity", "0.4");
				$("#rpg_tracker_user_prompt_suffix")
					.val(FULL_REVIEW_USER_PROMPT_SUFFIX)
					.prop("disabled", true)
					.css("opacity", "0.4");
				$("#rpg_tracker_btn_reset_prompt")
					.prop("disabled", true)
					.css("opacity", "0.4");
			} else {
				$("#rpg_tracker_full_review_note").css("display", "none");
				$("#rpg_tracker_core_prompt")
					.val(s.systemPromptTemplate || "")
					.prop("disabled", false)
					.css("opacity", "1");
				$("#rpg_tracker_user_prompt_suffix")
					.val(s.userPromptSuffix || "")
					.prop("disabled", false)
					.css("opacity", "1");
				$("#rpg_tracker_btn_reset_prompt")
					.prop("disabled", false)
			...
			$("#rpg_tracker_router_native_keyword_activation").prop(
				"checked",
				!!s.routerNativeKeywordActivation,
			);
			$("#rt-agent-router-native-kw").prop(
				"checked",
				!!s.routerNativeKeywordActivation,
			);
			$("#rpg_tracker_router_include_hidden").prop(
				"checked",
				!!s.routerIncludeHidden,
			);
			$("#rt-agent-router-include-hidden").prop(
				"checked",
				!!s.routerIncludeHidden,
			);
			$("#rpg_tracker_router_swipe_rollback").prop(
				"checked",
				s.routerSwipeRollback !== false,
			);
			$("#rt-agent-router-swipe-rollback").prop(
				"checked",
				s.routerSwipeRollback !== false,
			...
			$("#rpg_tracker_router_source").val(
				s.routerConnectionSource || "default",
			);
			updateRouterConnectionPanels();
			updateSettingsLorePrefixReadout();
			$("#rpg_tracker_router_prefix_override").val(
				s.routerCampaignPrefixOverride || "",
			);
			$("#rpg_tracker_router_connection_profile").val(
				s.routerConnectionProfileId || "",
			);
			$("#rpg_tracker_router_completion_preset").val(
				s.routerCompletionPresetId || "",
			);
			...
			$("#rpg_tracker_animate_all_custom_bars").prop(
				"checked",
				!!s.animateAllCustomBarChanges,
			);
			$("#rpg_sysprompt_mod_npc_rel_bars").prop(
				"checked",
				!!s.npcRelationshipBars,
			);
			$("#rpg_tracker_npc_card_import").prop(
				"checked",
				!!s.experimentalNpcImport,
			);
			$("#rpg_tracker_ignore_npc_limits").prop(
				"checked",
				!!s.ignoreNpcImportLimits,
			);

			// World Progression
			$("#rpg_world_progression_enabled").prop(
				"checked",
				!!s.worldProgressionEnabled,
			);
			$("#rpg_world_progression_locations_per_report").val(
				s.worldProgressionLocationsPerReport ?? 3,
			);
			...
			$("#rpg_world_progression_skeleton_use_existing").prop(
				"checked",
				!!s.worldProgressionSkeletonUseExisting,
			);
			$("#rpg_world_progression_skeleton_use_lorebooks").prop(
				"checked",
				!!s.worldProgressionSkeletonUseLorebooks,
			...
			syncSkeletonLorebookOnlyAvailability();
			if (s.worldProgressionSkeletonUseLorebooks)
				void refreshSkeletonLorebookList();
			$("#rpg_world_progression_consolidate_enabled").prop(
				"checked",
				!!s.worldProgressionConsolidateEnabled,
			);
			if (typeof runtimeState.updateAgentWorldStatusRef === "function")
				runtimeState.updateAgentWorldStatusRef();
			if (typeof runtimeState.updateAgentMapEvolutionStatusRef === "function")
			...
			if (typeof syncRouterPromptUi === "function") syncRouterPromptUi();
			$("#rpg_world_progression_system_prompt").val(
				s.worldProgressionSystemPrompt || "",
			);
			$("#rpg_world_progression_skeleton_system_prompt").val(
				s.worldProgressionSkeletonSystemPrompt || "",
			);
	...
	function updateAgentFontSize(size) {
		...
		for (const el of /** @type {NodeListOf<HTMLElement>} */ (
			document.querySelectorAll("#rpg-tracker-agent")
		)) {
	...
	_dbgWin.rpgDebug.testCleanToolCall = function (text) {
		if (text === undefined) {
			...
			const last =
				chat &&
				[...chat].reverse().find((m) => !m.is_user && m["role"] !== "user");
			text = last ? last.mes || last["content"] || "" : "";
			if (!text) {
				console.warn("[rpgDebug] No assistant message found in chat.");
				return;
			}
		}
		...
		console.group(
			"%c[rpgDebug] cleanToolCallMessage",
			"font-weight:bold;color:#7c4dff",
		);
