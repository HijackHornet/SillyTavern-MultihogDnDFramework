...
import {
	getLorebookManifest,
	scanRecentOutputForPresentNpcs,
} from "./router.js";
...
import {
	buildPortraitStoryContext,
	portraitStoryLookbackCount,
} from "./src/state/portrait-story-lookback.js";
...
export function imageGenToast(level, message, title = "RPG Tracker", options) {
	...
	if (getSettings().hideImageGenToasts && (lvl === "info" || lvl === "success"))
		return;
...
export async function applyPortraitData(entityName, src) {
	...
	if (!src) {
		...
		if (
			previous &&
			isManagedPortraitPath(previous) &&
			countPortraitPathRefs(s, previous) === 0
		) {
...
function migratePortraitMapKey(oldName, newName) {
	...
	return {
		moved: true,
		displaced: displaced && displaced !== src ? displaced : null,
		src,
	};
}
...
export async function renamePortraitEntity(oldName, newName) {
	...
	if (
		result.displaced &&
		isManagedPortraitPath(result.displaced) &&
		countPortraitPathRefs(s, result.displaced) === 0
	) {
...
function salvageOrphanPortraitKeysForNames(names) {
	...
	for (const name of names) {
		...
		if (result.moved) {
			...
			orphanKeys = orphanKeys.filter(
				(k) => k.toUpperCase() !== best.toUpperCase(),
			);
			console.log(
				`[RPG Tracker] Portrait key salvaged for rename: "${best}" → "${name}"`,
			);
		}
	}
...
export function reconcileMemoPortraitRenames() {
	...
	const allNames = [...roster, ...enemies, ...(character ? [character] : [])];
	...
	if (_lastMemoEntitySnapshot) {
		...
		if (
			_lastMemoEntitySnapshot.character &&
			character &&
			_lastMemoEntitySnapshot.character.toUpperCase() !==
	...
	for (const [oldName, newName] of pairs) {
		...
		if (result.moved) {
			...
			console.log(
				`[RPG Tracker] Portrait key migrated after memo rename: "${oldName}" → "${newName}"`,
			);
		}
...
const HORDE_CLIENT_AGENT =
	"SillyTavern-MultihogDnDFramework:1.0:https://github.com/Multihog-DnD-Framework";
...
export async function generateWithHordeAsync(prompt, opts = {}) {
	...
	console.log("[RPG Tracker] AI Horde: submitting generation request", {
		params,
	...
	if (!submitResp.ok) {
		...
		console.error(
			"[RPG Tracker] AI Horde: submit failed",
			submitResp.status,
			errBody,
		);
		throw new Error(
			`AI Horde ${submitResp.status}: ${errBody?.message || "Failed to submit generation request"}`,
		);
	}
	...
	console.log(
		`[RPG Tracker] AI Horde: request ${requestId} queued (est. ${submitData.kudos ?? "?"} kudos)`,
	);
	if (Array.isArray(submitData.warnings) && submitData.warnings.length) {
		console.warn(
			"[RPG Tracker] AI Horde: submit warnings (job may never find a worker) —",
			submitData.warnings,
		);
		...
		const fatal = submitData.warnings.filter((w) =>
			fatalCodes.includes(w.code),
		);
		if (fatal.length) {
			throw new Error(
				`AI Horde: ${fatal.map((w) => w.message || w.code).join(" | ")}`,
			);
		}
	}
	...
	while (Date.now() < deadline) {
		...
		const checkResp = await fetch(
			`${HORDE_API_BASE}/generate/check/${requestId}`,
			{
				headers: { "Client-Agent": HORDE_CLIENT_AGENT },
				signal: externalSignal,
			},
		...
		if (!checkResp.ok) {
			console.warn(
				`[RPG Tracker] AI Horde: check request failed with ${checkResp.status}, retrying...`,
			);
			continue;
		}
		const checkData = await checkResp.json();
		console.log(
			`[RPG Tracker] AI Horde: waiting=${checkData.waiting} processing=${checkData.processing} finished=${checkData.finished} queue_position=${checkData.queue_position} wait_time=${checkData.wait_time}s is_possible=${checkData.is_possible}`,
		);
		if (checkData.is_possible === false) {
			console.warn(
				"[RPG Tracker] AI Horde: no eligible workers currently available for this request (is_possible=false) — will keep waiting until timeout.",
			);
		}
		if (checkData.faulted) {
			console.error("[RPG Tracker] AI Horde: generation faulted", checkData);
			throw new Error(
				"AI Horde generation faulted (the worker reported an internal error)",
			);
		}
		if (checkData.done) {
			console.log(
				`[RPG Tracker] AI Horde: request ${requestId} done, fetching result...`,
			);
			break;
		}
	}
	...
	const statusResp = await fetch(
		`${HORDE_API_BASE}/generate/status/${requestId}`,
		{
			headers: { "Client-Agent": HORDE_CLIENT_AGENT },
			signal: externalSignal,
		},
	...
	if (!statusResp.ok)
		throw new Error(
			`AI Horde ${statusResp.status}: Failed to retrieve generation result`,
		);
	const statusData = await statusResp.json();
	const generation = statusData.generations?.[0];
	if (!generation?.img) {
		console.error(
			"[RPG Tracker] AI Horde: no image in status response after polling",
			statusData,
		);
		throw new Error(
			"AI Horde did not return an image (request may have timed out — try again or check your worker availability at aihorde.net)",
		);
	}
	console.log(
		`[RPG Tracker] AI Horde: image received from worker "${generation.worker_name || generation.worker_id}", downloading...`,
	);
	if (generation.censored) {
		throw new Error(
			"AI Horde image was censored by the worker's safety filter",
		);
	}
	...
	return await new Promise((resolve, reject) => {
		...
		reader.onerror = () =>
			reject(new Error("FileReader error while converting image blob"));
...
export async function generatePortraitPrompt(entityName) {
	...
	if (useStoryLookback) {
		...
		try {
			...
			if (worldInfo) {
				const entries =
					typeof ctx.getWorldInfoEntries === "function"
				...
				if (entries && Array.isArray(entries)) {
					...
					if (matchingEntries.length > 0) {
						const loreText = matchingEntries
							.map(
								(e) =>
		...
		try {
			if (s.activeRouterKeys?.length > 0) {
				const manifest =
					typeof getLorebookManifest === "function"
						? await getLorebookManifest(true)
						: null;
				if (manifest) {
					...
					if (matchingActive.length > 0) {
						const activeText = matchingActive
							.map(
								(e) =>
		...
		try {
			if (s.activeRouterKeys?.length > 0) {
				...
				for (const k of s.activeRouterKeys) {
					...
					if (!agentBooks[bookName])
						agentBooks[bookName] = await ctx.loadWorldInfo(bookName);
				}
				const agentEntries = [];
				for (const k of s.activeRouterKeys) {
					...
					if (entry && entry.content) {
						...
						agentEntries.push(
							`[Agent Entry: "${label}" | Keywords: ${keywords || "none"}]\n${(entry.content || "").substring(0, 600)}`,
						);
					}
				}
				if (agentEntries.length > 0) {
					contextParts.push(
						`Current Lorebook Agent (All Active Entries):\n${agentEntries.join("\n\n")}`,
					);
				}
			}
		} catch {
		...
		const storyContext = buildPortraitStoryContext(
			ctx,
			portraitStoryLookbackCount(s),
		);
	...
	const result = await sendStateRequest(
		getPortraitConnectionSettings(s),
		systemPrompt,
		userPrompt,
	);
	return (result || "").trim();
...
export async function generateNpcPortraitPrompt(entityName, npcContent) {
	...
	if (useStoryLookback) {
		try {
			...
			if (persona.trim()) {
				contextParts.push(
					`User Persona (for art style context):\n${persona.trim()}`,
				);
			}
		} catch {
			/* substituteParams may not exist */
		...
		try {
			...
			if (charData?.description) {
				contextParts.push(
					`Narrator Card Description (for world context):\n${charData.description.substring(0, 1500)}`,
				);
			}
		} catch {
			/* ignore */
 ...
 */
export async function showPortraitPromptPopup(
	prompt,
	entityName,
	localApply,
	refresh,
	...
	const popupOpts = {
		okButton: isNative
			? "🎨 Generate with ST Image Gen"
			: isHorde
				? "🎨 Generate with AI Horde"
	...
	setTimeout(() => {
		...
		if (ta) {
			ta.addEventListener("input", () => {
				finalPrompt = ta.value;
			});
			ta.focus();
			ta.setSelectionRange(ta.value.length, ta.value.length);
		}
		const chk = /** @type {HTMLInputElement|null} */ (
			document.getElementById(skipCheckboxId)
		);
		if (chk) {
			chk.addEventListener("change", () => {
				skipChecked = chk.checked;
			});
		}
	}, 0);
	...
	if (result === 3) {
		...
		if (isNative) {
			await generateWithNativeExtension(
				finalPrompt,
				entityName,
				localApply,
				refresh,
			...
			// Generate with Pollinations
			await generateWithPollinations(
				finalPrompt,
				entityName,
				localApply,
				refresh,
...
export async function generatePortraitDirect(prompt, entityName, opts = {}) {
	...
	if (isNative) {
		...
		const hasImagine =
			...
			SlashCommandParser.commands["imagine"];
		if (!hasImagine) {
			throw new Error(
				"ST Image Generation extension is not enabled. Please enable it in SillyTavern settings.",
			);
		}
		...
		const abortSlashCommand = () =>
			slashAbortController.abort("Real-Time Visualization stopped", true);
		if (externalSignal?.aborted) abortSlashCommand();
		else
			externalSignal?.addEventListener("abort", abortSlashCommand, {
				once: true,
			});
		...
		if (result && result.isError) {
			throw new Error(
				result.errorMessage || "ST Image Generation execution failed",
			);
		}
		const imageUrl = result && result.pipe;
		if (!imageUrl) {
			throw new Error(
				"No image URL returned from the ST Image Generation extension",
			);
		}
		return imageUrl;
	} else {
		...
		const doRequest = async (modelName) => {
			...
			if (!resp.ok) {
				...
				throw new Error(
					`Pollinations ${resp.status}: ${errText.substring(0, 300)}`,
				);
			}
		...
		try {
			...
			console.warn(
				`[RPG Tracker] Generation failed for model "${currentModel}": ${err.message}. Trying fallback models...`,
			);
			...
			if (currentModel !== "flux") {
				try {
					...
					console.warn(
						`[RPG Tracker] Fallback to "flux" failed: ${fallbackErr.message}. Trying "turbo"...`,
					);
				}
			}
			...
			try {
				...
				throw new Error(
					`All model options failed. Original error: ${err.message}`,
				);
			}
		}
	}
...
export async function generateWithPollinations(
	...
	const showPreview = async (preGeneratedUrl = null) => {
		...
		const genPromise = preGeneratedUrl
			? Promise.resolve(preGeneratedUrl)
			: generatePortraitDirect(prompt, entityName);
		genPromise
			.then((dataUrl) => {
		...
		setTimeout(() => {
			...
			if (sel)
				sel.addEventListener("change", () => {
					...
					s.pollinationsModel = currentModel;
					saveSettings();
				});
		}, 0);
		...
		if (result === 3) {
			...
			try {
				...
				toastr["error"](
					"Cannot crop — generation failed: " + err.message,
					"RPG Tracker",
				);
				await showPreview();
			}
			...
			try {
				...
				const finalUrl = dataUrl.startsWith("data:")
					? await scaleImageTo512Square(dataUrl)
					: dataUrl;
				await localApply(finalUrl);
				if (typeof refresh === "function") refresh();
				imageGenToast(
					"success",
					`Portrait applied for ${entityName}!`,
					"RPG Tracker",
				);
			} catch (err) {
				toastr["error"](
					"Cannot apply — generation failed: " + err.message,
					"RPG Tracker",
				);
			}
		}
 ...
 * @param {function} refresh
 */
export async function generateWithNativeExtension(
	prompt,
	entityName,
	localApply,
	refresh,
		...
		if (result === 3) {
			...
			try {
				...
				const finalUrl = imageUrl.startsWith("data:")
					? await scaleImageTo512Square(imageUrl)
					: imageUrl;
				await localApply(finalUrl);
				if (typeof refresh === "function") refresh();
				imageGenToast(
...
export function resolvePortraitSrcForPlayerCharacter(settings, pcName) {
	...
	for (const memoName of getCharacterBlockNames(s)) {
		if (
			!candidates.some(
				(c) => normalizeCharacterLabel(c) === normalizeCharacterLabel(memoName),
			)
		) {
...
function triggerPlayerPortraitAutoGenIfNeeded(settings, refresh, opts = {}) {
	if (
		!settings.portraitAutoGeneratePlayer ||
		settings.enablePortraits === false
	)
		return;
	const charName = getPrimaryCharacterBlockName(settings);
	if (!charName) return;
...
export async function autoGeneratePartyPortraits(refresh) {
	...
	if (partyMembers.length === 0) {
		toastr["warning"](
			"No party members or characters found in the current state memo.",
			"RPG Tracker",
		);
		return;
	}
	...
	if (toGenerate.length === 0) {
		imageGenToast(
			"info",
			"All party members and characters already have portraits.",
			"RPG Tracker",
		);
		return;
	}
	...
	imageGenToast(
		"info",
		`Starting auto-generation for ${toGenerate.length} party members...`,
		"RPG Tracker",
	);
	let successCount = 0;
	...
	if (successCount > 0) {
		imageGenToast(
			"success",
			`Finished! Applied ${successCount} party portraits.`,
			"RPG Tracker",
		);
	}
}
...
export async function autoGenerateEnemyPortraits(refresh) {
	...
	if (enemies.length === 0) {
		toastr["warning"](
			"No enemies found in the current COMBAT block.",
			"RPG Tracker",
		);
		return;
	}
	...
	imageGenToast(
		"info",
		`Starting auto-generation for ${toGenerate.length} enemies...`,
		"RPG Tracker",
	);
	let successCount = 0;
	...
	for (const name of toGenerate) {
		...
		try {
			...
			toastr["error"](
				`Failed for enemy ${name}: ${err.message}`,
				"RPG Tracker",
			);
		}
	}
	...
	if (successCount > 0) {
		imageGenToast(
			"success",
			`Finished! Applied ${successCount} enemy portraits.`,
			"RPG Tracker",
		);
	}
}
...
export async function removeAllPortraits(refresh) {
	...
	await saveSettings(true);
	toastr["success"](
		"All custom portraits removed (including saved chat copies).",
		"RPG Tracker",
	);
	if (typeof refresh === "function") refresh();
}
 ...
 */
export function triggerBackgroundPortraitGeneration(
	name,
	refresh,
	npcContent = null,
) {
	...
	console.log(
		`[RPG Tracker] triggerBackgroundPortraitGeneration for "${name}". alreadyHasPortrait:`,
		alreadyHas,
		`alreadyGenerating:`,
		alreadyGenerating,
	);
	...
	if (queuePos <= 0) {
		imageGenToast(
			"info",
			`Auto-generating portrait for ${name}...`,
			"RPG Tracker",
		);
	} else {
		imageGenToast(
			"info",
			`Queued portrait for ${name} (${queuePos} ahead)...`,
			"RPG Tracker",
		);
	}
	...
	enqueueImageGen(async () => {
		try {
			console.log(
				`[RPG Tracker] Generating prompt for "${name}" (NPC content provided: ${!!npcContent})`,
			);
			...
			if (!prompt) {
				console.warn(
					`[RPG Tracker] Could not generate prompt for ${name} - no context found.`,
				);
				return;
			}
			console.log(
				`[RPG Tracker] Calling generatePortraitDirect for "${name}"...`,
			);
			const dataUrl = await generatePortraitDirect(prompt, name);
			console.log(
				`[RPG Tracker] Successfully received portrait dataUrl for "${name}". Scaling...`,
			);
			const scaled = await scaleImageTo512Square(dataUrl);
			...
			imageGenToast(
				"success",
				`Portrait auto-generated and applied for ${name}!`,
				"RPG Tracker",
			);
			...
			console.error(
				`[RPG Tracker] Background portrait generation failed for ${name}:`,
				err,
			);
			...
			if (is524) {
				...
				toastr["error"](
					`Portrait generation failed for "${name}": ${errMsg.substring(0, 120)}`,
					"RPG Tracker",
				);
			}
		} finally {
			...
			console.log(
				`[RPG Tracker] Removed "${name}" from activeGenerations. Remaining:`,
...
export function resetAutoGenerationTracking() {
	console.log(
		"[RPG Tracker] resetAutoGenerationTracking called. Clearing knownEntities and resetting isFirstCheck to true.",
	);
	knownEntities.clear();
	isFirstCheck = true;
...
export async function forceCheckAutoGenerations(refresh) {
	const s = getSettings();
	console.log(
		"[RPG Tracker] forceCheckAutoGenerations called. Settings enablePortraits:",
		s.enablePortraits,
	);
	...
	if (s.portraitAutoGenerateParty) {
		...
		console.log(
			"[RPG Tracker] forceCheckAutoGenerations: checking party:",
			party,
		);
	...
	if (s.portraitAutoGenerateEnemies) {
		const enemies = getEnemyEntities();
		console.log(
			"[RPG Tracker] forceCheckAutoGenerations: checking enemies:",
			enemies,
		);
	...
	if (s.portraitAutoGenerateNpcs && s.npcPortraits !== false) {
		const ctx = SillyTavern.getContext();
		console.log(
			"[RPG Tracker] forceCheckAutoGenerations: checking NPCs. chatId:",
			ctx.chatId,
		);
		if (ctx.chatId) {
			...
			console.log(
				"[RPG Tracker] forceCheckAutoGenerations: bookName:",
				bookName,
			);
			try {
				if (typeof ctx.updateWorldInfoList === "function") {
					console.log(
						"[RPG Tracker] forceCheckAutoGenerations: updating world info list...",
					);
				...
				if (book && book.entries) {
					...
					console.log(
						"[RPG Tracker] forceCheckAutoGenerations: loaded book entries count:",
						entries.length,
					);
					for (const entry of entries) {
						...
						if (name) {
							console.log(
								"[RPG Tracker] forceCheckAutoGenerations: forcing NPC:",
								name,
							);
							knownEntities.add(name.toUpperCase());
					...
					console.log(
						"[RPG Tracker] forceCheckAutoGenerations: book not found or empty:",
						bookName,
					);
				}
			} catch (e) {
				console.error(
					"[RPG Tracker] forceCheckAutoGenerations NPC check error:",
					e,
				);
			}
		}
...
export async function checkAndTriggerAutoGenerations(refresh) {
	const s = getSettings();
	console.log(
		"[RPG Tracker] checkAndTriggerAutoGenerations invoked. enablePortraits:",
		s.enablePortraits,
		"isFirstCheck:",
		isFirstCheck,
	);
	if (s.enablePortraits === false) {
		console.log(
			"[RPG Tracker] checkAndTriggerAutoGenerations: enablePortraits is false. Exiting.",
		);
		return;
	}
	...
	console.log(
		"[RPG Tracker] checkAndTriggerAutoGenerations: currentParty:",
		currentParty,
		"currentEnemies:",
		currentEnemies,
	);
	...
	let npcEntries = [];
	console.log(
		"[RPG Tracker] checkAndTriggerAutoGenerations: portraitAutoGenerateNpcs settings:",
		s.portraitAutoGenerateNpcs,
	);
	if (s.portraitAutoGenerateNpcs && s.npcPortraits !== false) {
		const ctx = SillyTavern.getContext();
		console.log(
			"[RPG Tracker] checkAndTriggerAutoGenerations: ctx.chatId:",
			ctx.chatId,
		);
		if (ctx.chatId) {
			const prefix = getEffectiveRouterCampaignPrefix(ctx.chatId);
			const bookName = prefix ? `${prefix}_NPCs` : "NPCs";
			console.log(
				"[RPG Tracker] checkAndTriggerAutoGenerations: resolving bookName:",
				bookName,
			);
			try {
				const book = await ctx.loadWorldInfo(bookName);
				if (book && book.entries) {
					npcEntries = Object.values(book.entries).filter((e) =>
						(e.comment || "").trim(),
					);
					console.log(
						"[RPG Tracker] checkAndTriggerAutoGenerations: loaded npcEntries count:",
						npcEntries.length,
						npcEntries.map((e) => e.comment),
					);
				} else {
					console.log(
						"[RPG Tracker] checkAndTriggerAutoGenerations: no book or entries found for",
						bookName,
					);
				}
			} catch (e) {
				console.error(
					"[RPG Tracker] checkAndTriggerAutoGenerations NPC fetch error:",
					e,
				);
			}
		}
	...
	if (isFirstCheck) {
		console.log(
			"[RPG Tracker] checkAndTriggerAutoGenerations: isFirstCheck is true. Registering existing entities to knownEntities Set without generating.",
		);
		isFirstCheck = false;
		...
		await checkAndTriggerLocationAutoGenerations(refresh, {
			isFirstCheck: true,
		});
		console.log(
			"[RPG Tracker] checkAndTriggerAutoGenerations: knownEntities after first check:",
			Array.from(knownEntities),
		);
		return;
	}

	console.log(
		"[RPG Tracker] checkAndTriggerAutoGenerations: Checking for new entities against known list:",
		Array.from(knownEntities),
	);
	...
	if (s.portraitAutoGenerateParty) {
		for (const name of currentParty) {
			...
			if (!knownEntities.has(key)) {
				console.log(
					"[RPG Tracker] checkAndTriggerAutoGenerations: New party member detected:",
					name,
				);
				...
				console.log(
					"[RPG Tracker] checkAndTriggerAutoGenerations: Party member already known:",
					name,
				);
			}
		}
	...
	if (s.portraitAutoGeneratePlayer) {
		...
		if (charName) {
			...
			if (!knownEntities.has(key)) {
				console.log(
					"[RPG Tracker] checkAndTriggerAutoGenerations: [CHARACTER] block name detected:",
					charName,
				);
	...
	if (s.portraitAutoGenerateEnemies) {
		for (const name of currentEnemies) {
			...
			if (!knownEntities.has(key)) {
				console.log(
					"[RPG Tracker] checkAndTriggerAutoGenerations: New enemy detected:",
					name,
				);
				knownEntities.add(key);
				triggerBackgroundPortraitGeneration(name, refresh);
				...
				console.log(
					"[RPG Tracker] checkAndTriggerAutoGenerations: Enemy already known:",
					name,
				);
			}
		}
	...
	if (s.portraitAutoGenerateNpcs && s.npcPortraits !== false) {
		for (const entry of npcEntries) {
			...
			if (!hasPortrait(name)) {
				console.log(
					"[RPG Tracker] checkAndTriggerAutoGenerations: NPC has no portrait, triggering generation:",
					name,
				);
				triggerBackgroundPortraitGeneration(name, refresh, entry.content || "");
			} else {
				console.log(
					"[RPG Tracker] checkAndTriggerAutoGenerations: NPC already has portrait, skipping:",
					name,
				);
			}
		}
...
export function getLinkedPlayerCharacter(settings, ctx) {
	...
	const chatId =
		c.chatId ||
		(typeof globalThis._rpgCurrentChatId === "function"
...
async function loadPresentCharactersForLocationPrompt(settings, ctx) {
	...
	if (pc) {
		present.push({
			...
			content:
				pc.bio ||
				"(No PC bio — infer appearance from recent narrator output and scene context.)",
...
export async function generateLocationImagePrompt(locationPath, locContent) {
	...
	if (locContent && locContent.trim()) {
		contextParts.push(
			`Location Lorebook Entry (PRIMARY — depict this specific place):\n${locContent.trim()}`,
		);
	}
	...
	try {
		...
		if (narratorBlock) {
			contextParts.push(
				`Recent Narrator Output (last 2 replies — use for mood, staging, and moment):\n${narratorBlock.substring(0, 6000)}`,
			);
		}
	} catch {
		/* ignore */
	...
	try {
		...
		const presentCharacters = await loadPresentCharactersForLocationPrompt(
			s,
			ctx,
		);
		if (presentCharacters.length > 0) {
			const charBlocks = presentCharacters.map(
				(ch) =>
...
async function disableRealtimeLocationGenerationAfterFailure(err) {
	...
	const detail = String(
		err?.message || err || "Unknown image generation failure",
	).substring(0, 140);
	imageGenToast(
		"error",
		`Real-Time Visualization was disabled after one failed image request: ${detail}`,
		"RPG Tracker",
	);
	try {
		await saveSettings(true);
		...
		console.error(
			"[RPG Tracker] Failed to persist disabled Real-Time Visualization state:",
			saveErr,
		);
	}
}
 ...
 */
export function triggerBackgroundLocationGeneration(
	locationPath,
	refresh,
	locContent = "",
	opts = {},
	...
	if (
		isRealtimeArrival &&
		(!s.portraitAutoGenerateSceneView || realtimeLocationGenerationFailed)
	)
		return;
	...
	if (!isRealtimeArrival) {
		...
		if (queuePos <= 0) {
			imageGenToast(
				"info",
				`${forceReplace ? "Regenerating" : "Auto-generating"} location image for ${leaf}...`,
				"RPG Tracker",
			);
		} else {
			imageGenToast(
				"info",
				`Queued location image for ${leaf} (${queuePos} ahead)...`,
				"RPG Tracker",
			);
		}
	...
	enqueueImageGen(async () => {
		const realtimeAbortController = isRealtimeArrival
			? new AbortController()
			: null;
		if (realtimeAbortController)
			activeRealtimeLocationAbortController = realtimeAbortController;
		try {
			...
			if (
				isRealtimeArrival &&
				(!getSettings().portraitAutoGenerateSceneView ||
					realtimeLocationGenerationFailed)
			) {
				return;
			}
			...
			if (!prompt) {
				if (isRealtimeArrival) {
					await disableRealtimeLocationGenerationAfterFailure(
						new Error("No image prompt was returned"),
					);
				}
				return;
			}
			if (isRealtimeArrival && !getSettings().portraitAutoGenerateSceneView)
				return;
			...
			if (!isRealtimeArrival) {
				imageGenToast(
					"success",
					`${forceReplace ? "Location image regenerated" : "Location image auto-generated"} for ${leaf}!`,
					"RPG Tracker",
				);
			}
			...
			console.error(
				`[RPG Tracker] Background location image generation failed for ${normPath}:`,
				err,
			);
			const errMsg = String(err.message || err);
			if (isRealtimeArrival) {
				...
				toastr["error"](
					`Location image generation failed for "${leaf}": ${errMsg.substring(0, 120)}`,
					"RPG Tracker",
				);
			}
		} finally {
 ...
 */
export async function checkAndTriggerLocationAutoGenerations(
	refresh,
	opts = {},
) {
	const s = getSettings();
	...
	if (opts.isFirstCheck) {
		for (const entry of locEntries) {
			knownEntities.add(
				`LOC::${normalizeLocationPath(entry.label).toUpperCase()}`,
			);
		}
		return;
	}
